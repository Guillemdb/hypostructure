...
\newcommand{\NoResidual}{\mathsf{NoResidual}}
% SetupLabel: render each imported setup-paper result by final companion-paper
% theorem/lemma/proposition number, while retaining the stable source label as a
% parenthetical audit key.
\makeatletter
\newcommand{\SetupNum}[2]{\expandafter\gdef\csname SetupNum@#1\endcsname{#2}}
\SetupNum{p0:thm:singular-positive}{Theorem~5.7}
\SetupNum{p0:prop:concentration-package}{Proposition~5.9}
...
stationary-hull profiles are routed to a terminal concentration package.  The
...
depends only on the imported setup package stated below and on cited
...
stratum \(\Caff\), the log-diffuse and Young critical-tail alternatives
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \item \(\Crot\) is routed by a local Coriolis-flux identity into either
   setup-closed lower classes or the terminal stratification assembly
   (\Cref{prop:R2-routing-to-terminal}); its closure is obtained only
   ...
   (\Cref{prop:R3S-R3-terminal-routing}).
...
(\Cref{thm:first-bad-mesoscopic-routing}) which excludes the
...
\label{subsec:imported-setup-package}

\begin{theorem}[Imported setup package]
\label{thm:imported-setup-package}
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \SetupLabel{p0:prop:concentration-package}
   \end{quote}
   ...
   \end{quote}
\end{enumerate}
The purpose of the present paper is exactly to prove
\(\NoResidual\).  This verifies the setup residual hypothesis
\[
   \SetupLabel{p1:hyp:no-remainder}.
\]
\end{theorem}
...
Below, the phrase ``by the imported setup package'' means that the terminal
...
\Cref{thm:imported-setup-package}.  Paper~IV proves only the residual closure
\(\NoResidual\).
\end{remark}
...
\Cref{thm:imported-setup-package}: velocity nonvanishing, local suitability,
...
\label{def:retained-vs-support-state-descendant}
...
\]
A \emph{normalized support profile}, also called a \emph{support-state
...
retained threshold.  Log-window and Young/variance normalizations are called
...
\label{tab:ancestor-heredity-ledger}\\
\hline
...
\hline
\endfoot
Imported setup package &
Setup paper only: local concentration, local Type~I bridge, normalized
...
R2 closure &
R2 routing, terminal stratification assembly, finite separated-family exclusion,
...
R3 closure &
R3 routing, terminal stratification assembly, sequence-\(L^3\), and endpoint
atomic contradiction. &
Premature stationary-hull exclusion.\\
Concentration-set extraction &
Local compactness, local defect measures, compact observer windows, and pressure
atlas. &
...
Bounded-origin realization, affine quotient, hidden-scale compactness, and
coherent-tail alternatives. &
Final residual exclusion.\\
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   \item \emph{Setup interface and Type-I bridge}
   (\Cref{subsec:imported-setup-package,sec:local-typeI-bridge}). Records the
   setup outputs used here as black boxes and verifies that local pointwise
   Type-I singularities enter the admissible Seregin framework.
   ...
   \]
   The inherited ledger of allowed limiting operations is recorded in
   \Cref{thm:ancestor-realization-discharge}.
   ...
   dichotomy, routing proposition
   (\Cref{prop:R2-routing-to-terminal}).  Closure deferred until the terminal
   stratification assembly.
   \item \emph{Class \(\Cstat\)}. Stationary phase space, hull realization,
   centered scale reselection, routing
   (\Cref{prop:R3-stationary-hull-routing,prop:R3S-R3-terminal-routing}).
...
theorem (\Cref{thm:first-bad-mesoscopic-routing}).  Everything else is
either bookkeeping for the residual decomposition or routing of the four
analytic classes into this terminal core.
...
local-window condition are routed by the compact-window-to-expanding-region
...
\begin{theorem}[Setup local Type-I bridge]
...
\Cref{thm:imported-setup-package}\,(2).  Paper~IV does not reprove the
...
The setup proof of
\Cref{thm:AB-weak-serrin-typeI-package} relies on the Albritton--Barker
...
velocity lower bound, and since the local defect measure dominates the velocity
...
\Cref{thm:no-mixed-compact-diffuse}, and infinite chains of retained concentrating descendants are excluded
...
\begin{theorem}[Local Type I singularities enter the terminal framework]
...
remaining single retained profile is exactly the terminal case closed in
\Cref{sec:final-assembly}.  Thus the present theorem supplies the entry into
the residual framework; the contradiction is the final two-paper corollary
proved after \(\NoResidual\).
\end{proof}
...
\begin{proof}
The entry assertion is \Cref{thm:local-typeI-entry-or-discharge}.  The
contradiction is deferred to the final assembly, where \(\NoResidual\) is
...
\section{Decision tree for the residual profile decomposition}
...
\Cref{thm:imported-setup-package}.}
...
\textbf{Routing/closure.}  Localized Coriolis-flux reduction routes the profile
into setup-closed lower classes or the terminal velocity-concentration
...
profiles to keep positive non-affine oscillation after affine normalization.
...
\end{equation}
The local CKN concentration is
...
\begin{definition}[Retained local velocity concentration and CKN activity]
\label{def:retained-active-main}
Fix \(0<\eta_*,\eps_*<\infty\).  A terminal profile \((U,\Pi)\) has retained local
velocity concentration if, after the terminal normalization under consideration,
\[
   \Vel_1(U;0)\ge \eta_* .
\]
It has retained CKN activity if
\[
...
the imported setup package and the local concentration input of the companion
...
\]
The pressure-inclusive retained CKN activity threshold \(\eps_*\) is then
chosen with
\[
...
threshold depending on a previously extracted profile tree.
\end{definition}
...
\begin{itemize}[leftmargin=1.6em]
   ...
   \item \(\calE_1\): pressure-normalized unit CKN concentration, fixed at
      \(r=1\); used for local regularity, CKN smallness, and pressure-inclusive
      activity after the retained velocity lower bound has been identified.
   ...
   \item \(\Tcrit\): critical-tail compactification of triples
      \((\bar W,\mathcal Y,\Lambda)\), \Cref{def:critical-diffuse-tail-strata}.
...
\begin{itemize}[leftmargin=1.6em]
   ...
   \item \(\eta_*\): retained descendant velocity threshold,
      ...
      \Cref{def:retained-active-main} and in concentration-set extraction.
   \item \(\eps_*\): retained CKN activity threshold,
      \(0<\eps_*\le\eps_{\rm sd}/2\), used only for pressure-inclusive
      CKN activity and smallness bookkeeping.
...
nonzero descendants.\\
\(\mu_n\) & Local defect measure for concentration-set and compactness
arguments.\\
\(\mathcal A_n(z,r)\) & Spatial/log-tail averaged affine-quotient oscillation.\\
\(\calH_\infty=\calA_\infty\) & Full covariant spacetime asymptotic hull.\\
...
\(\Cyoung\) & Young/variance critical-tail residual alternative.\\
...
Normalized support profile / support-state descendant & Normalized defect
...
Profile-level recentering & Covariant recentering of an already extracted
...
neighborhoods, below unit velocity thresholds after routing.\\
...
Critical-tail defect & Nonzero normalized annular/logarithmic residual state
...
Since the estimate is on unit cylinders, the corrected normalization
\(\calE_1\) agrees with unit atlas mass up to the fixed pressure atlas versus
quotient gauge convention in \Cref{def:local-critical-quantity}.
\end{proof}
...
\(L^3\)-mass is still one of the terminal concentration-set alternatives listed in
\Cref{thm:terminal-exhaustion-main}.
\end{proof}
...
\subsection{Routing of the rotational relative-equilibrium class}

\begin{proposition}[Rotational residual routing]
\label{prop:R2-routing-to-terminal}
...
\Cref{prop:R3-stationary-hull-routing}.  We therefore assume
\(\Omega\ne0\).
...
This is exactly a terminal input realized from the original blow-up sequence for the terminal stratification
theorem; no R2 closure is used in this routing step.
\end{proof}
...
\begin{definition}[Retained local velocity concentration]
...
zero local velocity mass on the relevant compact cylinders.  Excluding that stationary
...
\subsection{Routing of the degenerate stationary-hull concentration class}

\begin{proposition}[R3 stationary-hull routing]
\label{prop:R3-stationary-hull-routing}
Every retained stationary-hull residual profile is either a setup-closed
stationary, tight, or structured profile, or it produces a stationary terminal
...
\begin{enumerate}[label=(\roman*)]
   ...
   \item Mixed compact--diffuse and compact--noncompact residual profiles, and
   infinite chains of retained concentrating descendants, are impossible in the refined residual
   class.
...
imported setup package: local positive concentration
...
\eqref{eq:R3S-centered}.  Suppose that \((U,\Pi)\) has retained local velocity
...
\Cref{lem:atomic-sequence-L3}.  The hypotheses listed here are exactly the
...
\Cref{lem:atomic-sequence-L3} gives \eqref{eq:R3S-atomic-L3}.
\end{proof}
...
concentration statement used by the stationary-hull routing.
\end{remark}
...
\label{prop:R3S-R3-terminal-routing}
...
concentration-set extraction uses \(\actthree\), because after parasitic-free
normalization it detects genuine non-affine oscillation.  Mesoscopic cylinders use
...
subsequence.  For bounded smooth centered profiles, interior parabolic
estimates for \eqref{eq:centered-main} upgrade the velocity convergence to
\(C^\infty_{\loc}\); the pressure representatives converge weakly in
...
\Cref{thm:imported-setup-package}.  The affine statement follows by substituting
...
time function.  This is exactly the quotient convention in
...
used for \(|(a,s)|_{\rm cov}\).  We write
\(\calA_\infty(V):=\calH_\infty(V)\) below for compatibility with the tail
dichotomy notation.  Thus the recurrent tail hull is invariant under the full
covariant action, not merely under ordinary spatial translations.
\end{definition}

\begin{theorem}[Compactness of the asymptotic hull]
\label{thm:asymptotic-hull-compactness}
Let \(V\) be a bounded smooth ancient solution of \eqref{eq:centered-main}.
Then \(\calA_\infty(V)=\calH_\infty(V)\) is nonempty and compact in
...
\(L^\infty\) bound by
\Cref{lem:covariant-translation-invariance,lem:semidirect-covariance}.  On every compact cylinder, the
...
bounded ancient solution.  Nonemptiness follows by applying this to any
escaping sequence in \(G\).
...
with the same local \(C^k\)-bounds as above, so it has a locally smooth
convergent subsequence.  Along that subsequence, \(W_j\) converges to the same
limit.  The closedness argument places the limit in \(\calH_\infty(V)\).
...
\begin{lemma}[Full covariant invariance of the asymptotic hull]
\label{lem:asymptotic-hull-covariant-invariance}
For every \(g\in G\),
\[
...
the imported setup package supplies the local pressure representatives; the present
...
the imported setup package.  Then
...
formulation recorded in the imported setup package.  For a spatially constant
...
\begin{definition}[Raw local defect measure and concentrating recentering sequences]
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \item the local pressure representation on a slightly larger cylinder
   converts the pressure lower bound into a positive velocity lower bound on
...
limit has zero \(D_1\)-mass, a contradiction.  Hence a genuine pressure-only
lower bound with compact pressure gauges can occur only if \(U\) is nonzero on
the finite enlargement used by the local pressure representation, even though
its velocity mass on \(Q_1\) vanishes.
...
therefore harmonic in space on the unit cylinder.  Its affine part is exactly
...
standard interior estimates for harmonic functions give a nonzero second
spatial derivative on a smaller cylinder.  The pressure Poisson equation then
forces a nonzero Riesz-pressure source on a finite enlargement, returning to
\textup{(iii)}.  This proves that a retained terminal profile is never
certified by pressure alone.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \(\psi\).
   \item \emph{Affine quotient ledger.} The quotient of \(h_n\) by spatial
   affine functions
   \[
   ...
   \item \emph{Non-affine harmonic detection.} If \([h_n]\) has a nonzero
   limit \([h]\) in the quotient \(D_1\), then either
   \([h]\) is purely affine (case \textup{(ii)} of
   \Cref{lem:no-pressure-only-retained-profile}), or there is a finite
   collection \(\{\psi_\ell\}_{\ell\le L}\) of compactly supported
   Schwartz test functions and a multi-index \(\alpha_\ell\) with
   \(|\alpha_\ell|=2\) such that
   \(\int (\partial^{\alpha_\ell}h_n)\psi_\ell\,dx\) is bounded away from
   zero on a positive-measure set of times, in which case the pressure
   Poisson equation
   \(-\Delta(p_n-a_n)=\partial_i\partial_j(u_{n,i}u_{n,j})\) on \(Q_4\)
   forces a nonzero Riesz-pressure source on a finite enlargement of
   \(B_2\), and the resulting source is detected on a neighboring unit
   cylinder by the local Riesz strong continuity of \textup{(i)}.
\end{enumerate}
The conclusion of \textup{(iv)} is the precise mechanism behind the routing
of the pressure-only case to alternative \textup{(iii)} of
\Cref{lem:no-pressure-only-retained-profile}: a non-affine harmonic
representative cannot survive in normalized whole-space Riesz representatives
outside the affine quotient because such a representative is exhibited as a
nonzero Riesz source by the displayed Poisson equation, and that source is
then detected as a nonzero velocity-product mass on a neighboring unit
cylinder.
\end{lemma}
...
(\Cref{thm:imported-setup-package}\,(3)).
...
\(D_1\).  Then \([h]\) has a nonzero second spatial derivative as an
element of the local distribution space; by the standard density of
compactly supported smooth test functions there is a finite collection
\(\{\psi_\ell\}_{\ell\le L}\) and multi-indices \(|\alpha_\ell|=2\) with
\(\int (\partial^{\alpha_\ell}h)\psi_\ell\,dx\ne0\) on a positive-measure
set of times; by stability \(\int (\partial^{\alpha_\ell}h_n)\psi_\ell\,dx\)
is bounded away from zero for large \(n\) on the same set.  Since
\(\partial_i\partial_j h_n=0\) on \(B_2\) (harmonicity), the same
combination applied to the full pressure
\(-\Delta(p_n-a_n)=\partial_i\partial_j(u_{n,i}u_{n,j})\) forces a nonzero
contribution from the velocity-product source on a finite enlargement of
\(B_2\).  By \textup{(i)}, this contribution is realized as a nonzero
\(L^{3/2}\)-mass of the local Riesz pressure on a neighboring unit cylinder,
which by the same Riesz boundedness corresponds to a nonzero
\(L^3\)-mass of \(u_n\) on a slightly larger cylinder.  This is exactly
alternative \textup{(iii)} of
...
\[
   \Vel_1(U;0)>0
\]
after lowering the retained threshold once.
\end{lemma}
...
cylinder for all large \(n\).  The imported setup package therefore gives the
...
\begin{lemma}[Covariant compactness for descendants of a bounded ancient profile]
\label{lem:profile-level-covariant-compactness}
Let \((U,\Pi)\) be a bounded pressure-normalized ancient profile in the
setup-generated class \(\mathfrak X_M\).  For every covariant recentering
sequence \(g_n\in G\), the transforms
\[
   \mathscr G_{g_n}(U,\Pi)
\]
admit a locally compact pressure-gauge subsequence in the Seregin topology.  If
\[
...
\(\mathbb R^3\times\mathbb R\).  Interior parabolic estimates and the setup
pressure atlas give local \(C^\infty\)-compactness for the velocity and local
\(L^{3/2}\)-compactness of pressure modulo gauges on every compact cylinder.
A diagonal subsequence gives a bounded ancient suitable descendant.  Strong
local \(L^3\) convergence transfers any velocity lower bound to the limit.
\end{proof}

\begin{lemma}[Escaping first-generation recenterings are routed]
\label{lem:escaping-first-generation-routing}
Let \((V_n,P_n)\) be a first-generation terminal sequence and let
\(\mathfrak c=(z_n)\) carry retained velocity mass:
\[
   \liminf_{n\to\infty}\Vel_1(V_n;z_n)>0 .
\]
If \(\mathfrak c\) fails the local-window condition, then it is not discharged
...
velocity mass gives a positive residual measure in the corresponding selected
...
critical-tail scale.  These are routing alternatives, not compactness
conclusions.
\end{proof}

\begin{corollary}[Local compactness or routing for retained recenterings]
\label{lem:rough-stratum-discharge}
Let \((V_n,P_n)\) be a terminal sequence obtained either as a first-generation
setup sequence or as a profile-level descendant of an already extracted bounded
ancient profile.  Any recentering sequence carrying retained velocity mass is
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \item It is profile-level; then
   \Cref{lem:profile-level-covariant-compactness} gives a nonzero locally
   compact descendant.
   \item It is first-generation and fails the local-window condition; then
   \Cref{lem:escaping-first-generation-routing} routes it to an exterior,
   diffuse, pressure/noncompact, or critical-tail terminal branch.
\end{enumerate}
Thus retained mass under an arbitrary recentering is never used by itself as a
...
\Cref{lem:first-generation-local-window-compactness,lem:escaping-first-generation-routing}.
Profile-level recenterings act on a bounded ancient profile defined on the full
centered ancient space, so they are handled by
\Cref{lem:profile-level-covariant-compactness}.  These cases exhaust the
recenterings used in the terminal profile tree.
\end{proof}
...
\(\mu_n\) be the local defect measures of a terminal sequence, with local finite
...
compact retained velocity profile whenever the local-window/profile-level
compactness hypotheses of \Cref{lem:rough-stratum-discharge} hold.  Declare two
...
distinct after the CKN-active deletion, with \textup{(b)} being the only
compact-window alternative in which no unit cylinder is CKN-active above
...
activity above the retained threshold, it is a retained active recentering and
its observer cylinder is CKN-active by
\Cref{lem:oscillation-active-implies-ckn-active}; but every CKN-active
...
\Cref{lem:no-pressure-only-retained-profile} routes it either to \textup{(d)},
to the affine/parasitic quotient, or to a neighboring velocity observer.  The
remaining sub-alternative is exactly \textup{(b)}: every unit cylinder in
...
tail region by dyadic annular/logarithmic windows \(\mathcal A_{n,k}\) with
bounded overlap, ordered by increasing \(|k|\) and then by sign.  Choose
\(k(n)\) to be the first index for which
\[
   \mu_n\!\left(\mathcal A_{n,k(n)}
      \setminus U_\rho(A_{n,m}^{\eta})\right)
   \ge \frac12
   \sup_k
   \mu_n\!\left(\mathcal A_{n,k}
      \setminus U_\rho(A_{n,m}^{\eta})\right)>0 .
\]
Normalize the residual measure on this selected window.  By
\Cref{lem:annular-tight-vs-log-escape-dichotomy}, after passing to a further
subsequence either the normalized log-radius measures have a nonzero vague
limit on the logarithmic compactification (case \textup{(e)}), or a fixed
multiplicative annulus captures a positive fraction of the mass at macroscopic
scales \(R_n\), again giving case \textup{(e)} via the macroscopic
critical-tail input of \Cref{def:critical-diffuse-tail-strata}.  This
selection is local in the chosen windows and uses no global mass budget.
\end{proof}

\begin{lemma}[Compact subthreshold regular residual cannot carry retained mass]
...
of \Cref{thm:imported-setup-package}\,(1).  Then the corresponding portion of
...
CKN-active locus has been deleted at radius \(\rho\) and the cylinders are
chosen at distance \(\rho/2\) from it.

By the compact subthreshold hypothesis, every such cylinder satisfies
\[
   \calE_1\bigl(\mathcal Q_{1+\delta}^{\rm cov}(z_i)\bigr)
   <\eta\le \eps_{\CKN}/2 .
\]
...
\Cref{thm:imported-setup-package}\,(1), each cylinder
\(\mathcal Q_{1+\delta}^{\rm cov}(z_i)\) is \(\eps_{\CKN}\)-regular for the
underlying suitable solution \((U_n,\Pi_n)\) along the diagonal extraction:
the velocity is bounded by an absolute constant on
\(\mathcal Q_1^{\rm cov}(z_i)\) and is locally smooth there.  Taking the
...
\(\eps_{\CKN}\), at least one CKN-active unit cylinder of mass exceeding
...
or to zero in the radial sense.  Then after passing to a subsequence exactly
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \(\Lambda_n:=(\log|\cdot|)_*\lambda_n\) on \(\mathbb R\) have a nonzero
   vague limit \(\Lambda\) on the two-point compactification
   \(\overline{\mathbb R}_{\log}=[-\infty,+\infty]\), with \(\Lambda\)
   charging at least one of the points \(\pm\infty\) or having no
   exponentially decaying mass on every fixed bounded log-window.
\end{enumerate}
\end{lemma}

\begin{proof}
For each \(n\) and each \(L>0\), set
\[
   \alpha_n(L):=\lambda_n\bigl(\{e^{-L}\le|x|\le e^L\}\bigr)
   =\Lambda_n([-L,L]).
\]
Since \(\alpha_n(L)\) is non-decreasing in \(L\) and bounded above by \(1\),
the limits
\(\alpha(L):=\liminf_n\alpha_n(L)\) and
\(\bar\alpha(L):=\limsup_n\alpha_n(L)\) exist and are non-decreasing in
\(L\).  Pass to a subsequence so that \(\alpha_n(L)\to\alpha_\infty(L)\) for
every rational \(L\), with \(\alpha_\infty\) non-decreasing and bounded by
\(1\); set
\[
   m:=\sup_L \alpha_\infty(L)
   =\lim_{L\to\infty}\alpha_\infty(L)\in[0,1] .
\]

If \(m<1\), a positive amount \(1-m\) of mass escapes every fixed bounded
log-window in the limit; hence \(\Lambda_n\) restricted to
\(\overline{\mathbb R}_{\log}\) has weak-\(*\) limit points charging at least
one of \(\pm\infty\), and we are in alternative \textup{(2)}.

If \(m=1\) but \(\alpha_\infty\) is not eventually constant on every bounded
window, the same conclusion holds in the form ``no exponentially decaying
mass on every fixed bounded log-window,'' i.e.\ \textup{(2)}.

Otherwise \(m=1\) and \(\alpha_\infty\) is eventually constant: there is
\(L_0>0\) with \(\alpha_\infty(L_0)=1\), so the \(\lambda_n\) are tight in
the log-radius variable.  Cover \([-L_0,L_0]\) by finitely many sub-intervals
\(I_k=[\log a_k,\log b_k]\) of length \(\le1\) with \(0<a_k<b_k<\infty\).
At least one of these intervals carries mass at least \(1/(2N)\) along a
further subsequence, where \(N\) is the number of intervals; that is,
\[
   \lambda_n\bigl(\{a_k<|x|<b_k\}\bigr)\ge \frac{1}{2N}
\]
for all sufficiently large \(n\).  Since the original measures live on
escaping windows, the supports translate by index-dependent macroscopic
scales \(R_n\); after factoring out this translation, the captured fixed
annulus is exactly \(\{a_kR_n<|x|<b_kR_n\}\), giving alternative
\textup{(1)}.
\end{proof}
...
\begin{lemma}[Oscillatory observers are CKN-concentrating]
\label{lem:oscillation-active-implies-ckn-active}
Let \(\mu_n\) be the local defect measure used in
...
\]
then the same observer is CKN-concentrating at threshold \(\eta\):
\[
   \mu_n(\mathcal Q_1^{\rm cov}(z))\ge\eta .
\]
Consequently, after the CKN concentration sets of
...
\]
The local defect measure contains the velocity contribution, hence
\[
   \iint_{\mathcal Q_1^{\rm cov}(z)}|U_n|^3
   \le
   \mu_n(\mathcal Q_1^{\rm cov}(z)).
\]
Thus oscillation implies CKN concentration with the same threshold.  The
...
the local velocity contribution, hence by the local defect measure.  The proof of
...
sequence that remains in these sets at a fixed positive threshold and has no
...
\Cref{def:active-removed-residual-measure}.  The residual remainder is then of
one of the following priority types:
\[
...
\(\mathcal R_{\rm diff}\) means positive concentration-deleted residual mass remains
while every fixed unit cylinder is below threshold; and
...
\Cref{lem:rough-stratum-discharge} gives a bounded retained terminal profile:
the diagonal compactness is supplied there either by the first-generation
local-window compactness lemma or by profile-level covariant compactness.  If such an observer
...
states are mutually exclusive once the concentration sets and selected protocol
...
\begin{corollary}[Persistent local velocity mass forces retained velocity concentration]
...
\begin{proof}
The local defect measure \(\mu_n\) dominates the velocity contribution.  Hence the
...
supplied by the imported setup package.  Then the retained
...
the imported setup package, failure of local vanishing at a singular terminal
...
that the residual profile has local CKN mass below \(\eps_{\rm sd}\) on each fixed
...
contained the compact lower-bound threshold from the imported setup package, the
...
the imported setup package, and fixed compact terminal cylinders.
\end{proof}
...
each defined on diffuse-defect-state candidates produced by
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   defect-state representative on the unit covariant cylinder
   ...
   and equal to \(1\) on \(K_m\), recording the mass that the defect state
...
\begin{theorem}[Construction of the diffuse-defect state space]
\label{thm:diffuse-defect-state-space-construction}
...
\(\|\mathcal O_k\|_\infty\le C_k\).  Define the embedding
\[
   \iota\colon \omega\longmapsto
   \bigl(\mathcal O_k(\omega)\bigr)_{k\in\mathbb N}
   \in
   \prod_{k\in\mathbb N}[-C_k,C_k] ,
\]
where \(\omega\) ranges over the diffuse-defect-state candidates produced by
\Cref{thm:diffuse-defect-compactness}.  By Tychonoff's theorem, the product
space
\[
   \mathcal X_0:=\prod_{k\in\mathbb N}[-C_k,C_k]
\]
is compact and metrizable (the metric
\(d(x,y)=\sum_k 2^{-k}\frac{|x_k-y_k|}{1+|x_k-y_k|}\) generates the product
topology).  Set
\[
   X_{\rm def}:=\overline{\iota(\text{all defect-state candidates})}
   \subset \mathcal X_0 .
\]
\(X_{\rm def}\) is compact metrizable as a closed subset of a compact
metrizable space.

The covariant action: for each \(g=(a,s)\in G\), the pullback of every
observable \(\mathcal O_k\) by the covariant observer map \(T_aZ_s\) is
again an observable in the chosen family
(\Cref{def:diffuse-defect-observables} is closed under the rational
\(G\)-action and is dense enough that the closure under \(G\) lies in the
same product space after a permutation of coordinates).  Hence the map
\(\sigma_g\colon \iota(\omega)\mapsto\iota(g\cdot\omega)\) extends
continuously to \(X_{\rm def}\), and \((g,x)\mapsto\sigma_g(x)\) is jointly
continuous because the rational orbit on the countable family of
observables is dense in the natural \(G\)-orbit map and convergence in
\(X_{\rm def}\) is coordinatewise.

The ancestor-realized class:
\[
   X_{\rm def}^{\rm anc}
   :=
   \overline{\iota(\text{ancestor-realized defect states})}
   \subset X_{\rm def} ,
\]
where ``ancestor-realized'' means realized from the original blow-up
sequence in the sense of
\Cref{def:ancestor-realized-residual-object}.  This gives a closed subset of
...
\(G\)-invariant ancestor-realized subset \(X_{\rm def}^{\rm anc}=
\mathfrak D_M^{\rm anc}\) are those constructed in
\Cref{thm:diffuse-defect-state-space-construction}.  The
...
\Cref{thm:diffuse-defect-state-space-construction}.
\end{definition}
...
defect state space \(X_{\rm def}=\mathfrak D_M\).  Let \(\nu_0\) be a Borel
probability measure supported on the closed ancestor-realized class
...
\(\mathbb P\) is supported on the closed ancestor-realized diffuse-defect class.
...
\]
converges to \(\mathcal T=(\bar W,\mathcal Y,\Lambda)\) in \(\Tcrit\) if:
\begin{enumerate}[label=\textup{(\roman*)}]
   \item \(\bar W_n\rightharpoonup^\ast\bar W\) in
   \(L^\infty_{\loc}\) on macroscopic annular cylinders;
   \item the Young measures converge against every bounded continuous velocity
   observable with compact support in the macroscopic variables;
   \item the log-scale defect measures \(\Lambda_n\) converge vaguely on the
   logarithmic annular compactification;
   \item pressure gradients converge distributionally after the time-dependent
   gauges and affine pressure quotients fixed in
   \Cref{def:pressure-gauge-affine-quotient};
   \item the affine quotient data converge in the finite-dimensional local
   quotient topology; and
   \item when the states are ancestor-realized, the limiting state remains in
   the closed ancestor-realized support class generated by the original terminal
   sequence.
\end{enumerate}
...
\begin{theorem}[Compactness of the critical-tail state space]
\label{thm:critical-tail-state-space-compactness}
Every bounded ancestor-realized family of critical-tail states has a convergent
subsequence in \(\Tcrit\).  If all states in the sequence are ancestor-realized,
then the limit is ancestor-realized.
\end{theorem}
...
subsequential limits.  The affine quotient data are finite-dimensional on each
fixed time window, and the quotient topology was chosen so that bounded
sequences have subsequential limits.  Diagonalizing over the countable annular
exhaustion gives convergence in all components listed in
\Cref{def:critical-diffuse-tail-strata}.  For ancestor-realized sequences,
choose for each approximating state a realizing subsequence from the original
...
to the same limit, so the limiting critical-tail state remains
ancestor-realized.
\end{proof}
...
\(M\).  A sequence of Young measures \(\mathcal Y_n\) on the macroscopic
spacetime annulus \(\{a<|x|<b\}\times I\) with values in \(\overline{B_M}\)
converges to \(\mathcal Y\) if for every \(\psi\in C_c^\infty(\{a<|x|<b\}\times
I)\) and every \(F\in C(\overline{B_M})\),
\[
...
at \((x,s)\).  Equivalently, \(\mathcal Y_n\to\mathcal Y\) in the
...
A sequence of pressure profiles \(Q_n\), each defined on the same macroscopic
annular cylinder up to the affine and time-only gauges of
\Cref{def:pressure-gauge-affine-quotient}, converges \emph{annularly
distributionally} to \(Q\), written \(\nabla Q_n\to\nabla Q\) in
\(\mathcal D'_{\rm ann}\), if for every compactly supported divergence-free
test vector field \(\Psi\in C_c^\infty(\{a<|x|<b\}\times I,\mathbb R^3)\)
\[
   \langle\nabla Q_n,\Psi\rangle
   =-\langle Q_n,\nabla\cdot\Psi\rangle\to0
\]
and for every general compactly supported \(\Phi\in C_c^\infty(\{a<|x|<b\}\times
I,\mathbb R^3)\) one has
\[
   \langle\nabla Q_n,\Phi\rangle\to\langle\nabla Q,\Phi\rangle ,
\]
where the left-hand sides are computed for representatives of \(Q_n\) chosen
inside the affine quotient.
\end{definition}
...
\(R_n\to\infty\), affine pressure quotient fixed, and pressure gradients
satisfying the gauge of
\Cref{def:critical-pressure-gradient-convergence}.  Let
\((\bar W,\mathcal Y,\Lambda)\) be the limiting critical-tail state.  After
passing to the subsequence defining \(\Tcrit\), the pressure and viscous
pairings
\[
   \mathscr P_{n,F}[\psi]
   :=
...
the critical-tail topology.  Denote the limits by \(\mathscr P_F[\psi]\) and
\(\mathscr D_F[\psi]\).  Then the Young measure satisfies the exact weak
balance
...
\begin{aligned}
   ...
   &\qquad =
   \frac12\iint \psi
   \int \xi\cdot\nabla_\xi F(\xi)\,d\mathcal Y_{x,s}(\xi)\,dx\,ds
   +\mathscr P_F[\psi]+\mathscr D_F[\psi].
\end{aligned}
\end{equation}
...
\(\mathscr P_F[\psi]=\mathscr D_F[\psi]=0\), then
\[
   \iint
   \Bigl(\partial_s\psi+\tfrac12x\cdot\nabla_x\psi
      +\tfrac{3-p}{2}\psi\Bigr)
   \int F(\xi)\,d\mathcal Y_{x,s}(\xi)\,dx\,ds=0 .
\]
...
\(\rho\mapsto\rho+\tfrac12 s\) on \(\overline{\mathbb R}_{\log}\).
\end{lemma}
...
\[
   R_n^{-2}\!\iint F'(V_n)\Delta V_n\,\psi\,dx\,ds
   =-R_n^{-2}\!\iint
   \Bigl[
      \psi\,\nabla_xV_n^T\nabla_\xi^2F(V_n)\nabla_xV_n
      +\nabla_\xi F(V_n)\nabla_xV_n\cdot\nabla\psi
   \Bigr]\,dx\,ds .
\]
Since \(F'\) and \(F''\) are bounded on \(\overline{B_M}\), \(V_n\) is bounded
by \(M\), and the local energy bound from
\Cref{lem:local-energy-blow-down} gives \(R_n^{-2}\iint
|\nabla V_n|^2\psi\,dx\,ds\le C\), the Hessian part has a subsequential
distributional limit on the countable test family; this is
\(\mathscr D_F[\psi]\).  The remaining first-derivative part equals
\(-R_n^{-2}\iint\nabla_xF(V_n)\cdot\nabla\psi\), which after integration by
parts is \(R_n^{-2}\iint F(V_n)\Delta\psi=O(R_n^{-2})\) and tends to \(0\).
...
and keeping the pressure term explicitly, we obtain
\[
   \iint
   F(V_n)
...
choosing more general \(F\) and passing to the Young-measure limit using
...
\begin{aligned}
   ...
   &\qquad =
   \iint \tfrac12 \psi
   \int F'(\xi)\xi\,d\mathcal Y_{x,s}(\xi)\,dx\,ds
   +\mathscr P_F[\psi]+\mathscr D_F[\psi].
\end{aligned}
\]
...
\(\Tcrit\) and is routed by the pressure/noncompact, affine quotient, or Young
critical-tail alternatives rather than silently discarded.
...
\Cref{lem:recurrent-virial-equality-routing} is automatic for realized
recurrent defects, after either ordinary minimal mesoscopic-scale reduction or
...
origin and satisfy the scaled bounds
\(|\nabla\phi|\lesssim |y|^{-1}\), \(|\Delta\phi|\lesssim |y|^{-2}\) from
\eqref{eq:log-cutoff-derivative-bounds}.  Combined with
...
\Cref{lem:homogeneous-gauge-compactness} assigns the descendant to \(\Caff\) and
the assertion is recorded as the affine alternative.  In the complementary
case the observability hypothesis of
\Cref{lem:recurrent-virial-equality-routing} is verified, and the reduction
...
mesoscopic-scale reduction or assignment to \(\Caff\).  Let \(\mathbb P\) be an
...
the exact blow-down ledger of \Cref{def:macroscopic-critical-tail-blowdown}.
...
limit in the exact ungauged equation
...
pressure gradients converge in annular distributions.  The limit therefore satisfies
\[
...
called a log-window support-state descendant, is obtained by choosing
...
original defect even if their unnormalized mass tends to zero.
...
converge to a probability measure supported in \(K\).  This gives
a nonzero log-window normalized support profile outside \(\mathcal F\).

This contradicts the hypothesis that every nonzero log-window normalized support profile lies
in \(\mathcal F\).  Hence \(\Lambda\) is supported on \(\mathcal F\).  Since
\(\mathcal F\) is closed under retained descendants and normalized support profiles and is already excluded,
the original defect has no admissible surviving normalized support profile.  The
fact that \(\lambda_n(I_n)\) may tend to zero does not affect the argument: the normalized
restriction records a support profile of the original probability measure, and a
positive original defect measure cannot be supported entirely on excluded
closed alternatives.
\end{proof}
...
nonzero log-diffuse critical-tail defect generated after concentration-set removal
and affine normalization has a nonzero normalized support profile in
\[
...
\]
The observer metric used here is the fixed parabolic metric on covariant
observer space,
\[
   d_{\rm obs}\bigl((h,\theta),(0,0)\bigr)\simeq |h|+|\theta|^{1/2}.
\]
Multiplication by \(R_n\) converts this macroscopic observer displacement into
the corresponding centered spatial/logarithmic tail scale.  This is an
observer blow-down scale, not a Navier--Stokes parabolic rescaling of the
...
positive constant-in-time oscillation lower bound, alternative (i) holds.
...
The pressure normalization is chosen so that \(\nabla_xQ_n\) is the rescaled
pressure-gradient term appearing in the centered blow-down equation.
\end{definition}
...
\Cref{def:ancestor-realized-residual-object}, and let
\[
   V_n(x,s)=U_n^{z_n}(r_nx,s),
   \qquad
   Q_n(x,s)=r_n^{-1}\Pi_n^{z_n}(r_nx,s),
\]
be the minimal-scale spatial/logarithmic blow-downs of
\Cref{def:first-bad-spatial-blowdown}.  Then \((V_n,Q_n)\) satisfy the local
energy inequality associated to
\eqref{eq:first-bad-scaled-equation} on every relatively compact spacetime
domain \(K\Subset\mathbb R^3\times\mathbb R\): for every nonnegative
\(\phi\in C_c^\infty(K)\),
...
\begin{equation}\label{eq:blow-down-local-energy-bound}
   r_n^{-2}\!\iint |\nabla V_n|^2\phi\,dx\,ds
   \le
   C\!\left(M,\|\phi\|_{C^2},|K|,r_n\right)
\end{equation}
with a finite constant uniform in \(n\), and the right-hand side of
\eqref{eq:blow-down-local-energy} converges, as \(n\to\infty\), to the
distributional drift--pressure identity for the limiting bounded profile.
\end{lemma}

\begin{proof}
The setup-paper local energy inequality (recalled inside the imported package
\Cref{thm:imported-setup-package}\,(3)) states that for every locally suitable
...
\[
   ...
   +\iint Q_nV_n\cdot\nabla_x\phi\,dx\,ds ,
\]
using \(Q_n=r_n^{-1}\Pi_n^{z_n}(r_nx,s)\),
...
The uniform bound \eqref{eq:blow-down-local-energy-bound} follows from the
\(L^\infty\) bounds \(|V_n|\le M\), the local \(L^{3/2}\)-pressure compactness
in \Cref{lem:first-bad-pressure-compactness}, H\"older's inequality on the
compact support of \(\phi\), and the explicit dependence of the right-hand
side on \(\|\phi\|_{C^2}\), \(|K|\), and \(r_n\).  Since \(r_n\to\infty\), the
viscous and convective terms carry the vanishing factors \(r_n^{-2}\) and
\(r_n^{-1}\).  After passing to the pressure-gauge subsequence supplied by
\Cref{lem:first-bad-pressure-compactness} and to the bounded-velocity
subsequence, the remaining drift and pressure-flux terms pass to their
distributional limits.  This is the local energy identity used in the proof of
\Cref{lem:first-bad-young-variance}.
\end{proof}
...
blow-down, or concentrating descendant profile used in the residual profile class is
...
associated profile is assigned to \(\Caff\); in the parasitic-free representative
...
By \Cref{prop:normalized-pressure-representation}, the only remaining pressure
component in the normalized profile class is
\[
   \beta(s)\cdot y+\alpha(s).
\]
After scaling,
...
Let \(Q_n\) be as in \Cref{lem:first-bad-blowdown-equation}.  Assume in
addition the \emph{normalized whole-space Riesz-pressure hypothesis}: each
\(Q_n\) is the whole-space Riesz-pressure representative of the
minimal-scale rescaled solution, normalized modulo the affine pressure
gauges and modulo time-only functions in the sense of
\Cref{def:pressure-gauge-affine-quotient} and the local pressure atlas
imported in \Cref{thm:imported-setup-package}\,(3); equivalently, each
\(Q_n\) is the canonical normalized representative produced by
\Cref{lem:local-pressure-decomposition} after subtracting affine gauges.
...
minimal-scale scaling.  If the affine gauges retain nonzero descendant concentration,
...
\(\Ccohcrit\).  Therefore every minimal mesoscopic oscillation scale is assigned to
...
concentration-set removal; the affine case is removed by
\Cref{thm:oscillatory-entry-normalization}; and the coherent critical-tail
...
rescaled.  The scaling ledger is
\[
\begin{array}{c|c}
...
\begin{equation}\label{eq:critical-tail-exact-blowdown-equation}
   ...
   =0 .
\end{equation}
If \(Q_n\) is used instead of \(\widehat Q_n\), the equation is understood in
the affine/parasitic quotient; the missing term is the spatially homogeneous
force \(b_n(s)\), which is either assigned to \(\Caff\) or removed by the
...
\((\mathcal Y,\Lambda)\) satisfies the Young/log balance of
\Cref{lem:critical-tail-young-log-transport}; in particular the log-scale
component is transported by the induced log-dilation flow, while nonlinear
velocity observables retain the pressure and viscous defect pairings explicitly
recorded in \eqref{eq:young-transport-identity}.
\end{theorem}
...
observers \(z_n=(y_n,\sigma_n)\), affine velocity gauges \(c_n(s)\), and
pressure gauges.  On fixed annular cylinders use the exact blow-down map of
\Cref{def:macroscopic-critical-tail-blowdown}:
\[
...
It remains to identify the equation satisfied by the barycenter.  The ungauged
...
limit for \(V_n\) and then projected to the affine/parasitic quotient.  If the
homogeneous component represented by \(c_n\) is retained, it is assigned to
\(\Caff\); otherwise the canonical affine normalization removes it from the
non-affine quotient barycenter.
...
barycentric limit of \(V_n\), then applying the affine quotient just described,
yields the displayed linear critical-tail transport equation for the
non-affine barycenter \(\bar W\).
Applying \Cref{lem:critical-tail-young-log-transport} gives the exact
Young/log balance: the log-scale component is transported by the induced
log-dilation flow, while nonlinear velocity observables retain the pressure and
viscous defect pairings displayed in
...
set of positive macroscopic measure, the profile is a Young critical tail, giving
\emph{(d)}.
...
used in both reductions is \MesoscopicReductionTheorem.  These are local
...
recorded in the companion setup paper have been removed.  Then
...
A profile in \(\Caff\) therefore cannot carry retained normalized residual
...
\begin{proof}
Let a retained residual profile generate a nonzero Young critical-tail alternative.
...
\begin{enumerate}[label=\textup{(\roman*)}]
   \item It is a profile-level window of a bounded ancient descendant
...
\begin{proof}
Profile-level windows are obtained after an ancient bounded profile has already
...
\Cref{lem:escaping-first-generation-routing}, it must be routed through the
...
sequence is profile-level, or because the first-generation local-window
...
\[
   ...
   \le 2M+C_I
\]
for a.e. \((x,s)\in K_{R,I}\).  The domain realization dichotomy
\Cref{lem:critical-tail-domain-realization-dichotomy} is exactly the hypothesis
...
\emph{Scope clarification.}  The bounded-origin realization recorded in the
statement and used throughout this lemma is taken in the precise sense of
\Cref{thm:bounded-origin-realization-coherent-tails} (the \emph{bounded-origin
realization} theorem) in the present paper: every macroscopic tangent of an element of
\(\Ccohcrit\) admits a representative that is locally bounded on every
compact macroscopic cylinder, including cylinders meeting the macroscopic
origin \(\{x=0\}\), with constants stable under local hull limits.  The
hypothesis is local in the centered variables and uses no global
critical-norm or global tail budget; it propagates under logarithmic
translation by the same argument as in
...
same coherent tail realized from the original blow-up sequence.  Logarithmic translation corresponds to
...
\(Z_*(\rho,\cdot,\cdot)\to0\) locally as \(\rho\to-\infty\), and the zero profile
...
\(\Capcrit\).  Choose \(Z_*\in\mathcal M\).  By
\Cref{lem:minimal-coherent-hull-origin-vanishing}, \(0\in\mathcal H(Z_*)\).
Minimality gives \(\mathcal H(Z_*)=\mathcal M\).  Since the zero state is a
...
\begin{proof}
Let \(\mathcal T\in\Tcrit\) be a remaining nonzero realized critical-tail profile.
By \Cref{thm:realized-critical-tail-rigidity}, the ordered priority alternative
is one of: regenerated concentration, affine/parasitic collapse, log-diffuse defect,
...
\[
   ...
   \qquad
   \hbox{one compact concentration core}+\mathcal R_{\rm noncomp},
\]
...
successor, or a chain of retained concentrating descendants.  The mixed successors are excluded
by \Cref{thm:no-mixed-compact-diffuse}; infinite chains of retained concentrating descendants and finite
...
\]
   ...
   topology modulo gauges, with retained local velocity concentration at the origin.  An ordinary
  ...
  \]
   ...
   \]
   has velocity concentration below the retained threshold, unless that
   recentering sequence has loss of local compactness.  If a pressure-inclusive
   CKN lower bound remains without velocity concentration, it is routed by
...
retained velocity concentration profile is terminally indecomposable if it has no
retained concentrating tail limit, no diffuse exterior concentration, and no
escaping non-compact recentering sequence.
\end{definition}

\begin{lemma}[Time-slice concentration gives parabolic velocity concentration]
...
\[
   ...
   \qquad -\delta^2<\tau<0 .
\]
Integrating in time gives a positive lower bound for \(\Vel_1(U;0)\).  The
pressure contribution is not used to certify nontriviality.  The constants are unchanged under
...
\begin{lemma}[Quantitative exterior slice-to-defect routing]
\label{lem:exterior-slice-to-diffuse-defect}
Let \((U,\Pi)\) be a bounded smooth solution of \eqref{eq:centered-main} with
\(\|U\|_{L^\infty}\le M\).  Let \(\tau_n\to-\infty\), \(R_n\to\infty\), and
\[
   E_n:=\{|y|>R_n\}
\]
satisfy
...
\]
Fix \(\alpha>0\), and let \(c_{\rm sl}(M,\alpha)>0\) be the parabolic lower
bound supplied by \Cref{lem:timeslice-to-ckn-activity} for a unit ball
time-slice mass \(\alpha\).  Define
\[
   m_n^{\rm vel}
   :=
...
\]
If \(m_n^{\rm vel}<c_{\rm sl}(M,\alpha)\), then every unit ball with center in
the exterior collar satisfies
\[
   \int_{B_1(x)}|U(y,\tau_n)|^3\,dy<\alpha
   \qquad (|x|>R_n-1).
\]
Consequently there are measurable exterior packets \(F_n\subset E_n\), escaping
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   carried by velocity, and is routed by
   \Cref{lem:no-pressure-only-retained-profile};
...
choose a measurable packet \(F_n\subset E_n\) with
\[
...
packets escape every compact set.
...
solution \(\mathscr T_{(x_{n,j},\tau_n)}(U,\Pi)\).  At local time \(0\) the ball
\(B_1(0)\) is exactly the original ball \(B_1(x_{n,j})\).  Hence
\[
   \Vel_1\!\left(\mathscr T_{(x_{n,j},\tau_n)}U;0\right)
   \ge c_{\rm sl}(M,\alpha),
\]
so the parabolic velocity part of the local defect measure on that unit
cylinder has mass at least \(c_{\rm sl}(M,\alpha)\).  This is the only
...
\(m_n^{\rm vel}<c_{\rm sl}(M,\alpha)\).  This proves the stated unit-ball
time-slice bound.
...
logarithmic or normalized-tail compactification, it is exactly the already
listed critical-tail alternative \textup{(iii)}.  In the complementary case the
escaping exterior slice packet persists, all unit exterior velocity observers are
below the retained threshold fixed by \(c_{\rm sl}(M,\alpha)\), pressure-only
observers have been routed, and no noncompact recentering remains.  This is the
...
Let \((U,\Pi)\) be a bounded smooth ancient solution of
\eqref{eq:centered-main}.  Suppose \(U\) has retained local velocity concentration and
...
Fix once and for all a small \(\alpha_0>0\), and let
\(c_{\rm sl}(M,\alpha_0)\) be the corresponding constant in
\Cref{lem:exterior-slice-to-diffuse-defect}.  By the threshold hierarchy
\Cref{def:threshold-hierarchy}, lower the retained descendant threshold once,
before reading terminal indecomposability for this bounded class, so that
\[
   4\eta_*<c_{\rm sl}(M,\alpha_0).
\]
This only makes the retained-tail test more sensitive and is independent of any
later profile tree extraction.

First suppose \(\limsup_{n\to\infty}m_n^{\rm vel}\ge4\eta_*\).  Passing to a
...
\[
   \Vel_1(W;0)\ge\eta_* .
\]
Thus \(W\) is a retained concentrating covariant tail limit, contradicting
terminal indecomposability.

It remains to consider the case in which \(m_n^{\rm vel}<4\eta_*\) for
all large \(n\).  If some unit exterior
...
indecomposability is contradicted by the noncompact defect alternative.  If the
...
\[
   \int_{|y|>R_n}|U(y,\tau_n)|^3\,dy\ge1
\]
together with the parabolic velocity smallness \(m_n^{\rm vel}<4\eta_*\)
is now
handled quantitatively by \Cref{lem:exterior-slice-to-diffuse-defect}.  Apply
that lemma with \(m_0=1\), \(\alpha=\alpha_0\), and the inequality
\(m_n^{\rm vel}<4\eta_*<c_{\rm sl}(M,\alpha_0)\).
If the lemma produces local compactness loss, pressure-gauge loss, pressure-only
mass, or a critical-tail normalization, the branch is one of the already routed
...
\(\mu_{n(m)}^{\mathfrak d}\) denote the local defect measure of the original
terminal sequence pulled back by this combined parabolic-covariant recentering and by
the pressure gauges used in the diagonal convergence.  The velocity part is
continuous under the strong local \(L^3\) convergence, so
\[
   \liminf_{m\to\infty}
      \iint_{Q_1(0,0)}|V_{n(m)}^{\mathfrak d}|^3\,dy\,d\tau
   \ge \Vel_1(W;0)\ge\eta_* .
\]
Thus \(\mathfrak d\) has retained local velocity concentration.
\end{proof}
...
by the imported setup package.  The fact that the diffuse or
...
\Cref{lem:no-nonactive-successor}, while coexistence of a compact core with a
diffuse or noncompact remainder is handled separately by
\Cref{thm:no-mixed-compact-diffuse}.  This proof is graph/recurrence based and
...
the imported setup package give compactness of \(\calK\) on every fixed terminal
...
is excluded by \Cref{thm:no-mixed-compact-diffuse}.  Hence no retained
...
the already routed exterior, diffuse, pressure/noncompact, and critical-tail
alternatives.

\begin{longtable}{@{}L{0.27\textwidth}L{0.43\textwidth}L{0.22\textwidth}@{}}
\caption{Acyclic dependency ledger for the terminal concentration argument.}
\label{tab:terminal-dependency-ledger}\\
\hline
...
\]
Assume all setup-closed lower classes have been removed.  The compact
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \item an affine/parasitic quotient profile occurs;
...
\textup{(2)}, \textup{(3)}, a mixed diffuse/noncompact/critical-tail branch, or
the atomic case \textup{(7)}.  Alternatives \textup{(2)}--\textup{(6)} are
excluded by the preceding branch closures and local terminal arguments.
Alternative \textup{(7)} is excluded by the endpoint bounded-mild ancient
...
routing, and closure results in the order recorded in
\Cref{tab:terminal-dependency-ledger}.  The pure radiative alternatives enter
only as sole-source exclusions, and are used only in that form through
\Cref{thm:terminal-nonactive-discharge}.  Coexistence of a compact concentration core
...
\Cref{thm:realized-critical-tail-rigidity,cor:critical-tail-discharge-complete}.  Infinite concentration chains and finite
...
\begin{proof}
By \Cref{prop:R2-routing-to-terminal}, every R2 residual either belongs to a
...
The lower classes are empty by the imported setup package, and the terminal
...
\begin{proof}
By
\Cref{prop:R3-stationary-hull-routing,prop:R3S-R3-terminal-routing}, every R3
stationary-hull residual either lies in a setup-closed lower class or produces
a stationary terminal concentration profile realized from the original blow-up
sequence.  The lower classes are empty by the imported setup package.  The stationary terminal
...
normalization and after removal of the setup-closed lower alternatives.  Then a
...
setup package supplies retained compact velocity mass.
...
compactness, or a scale-critical tail window.  These alternatives are exactly
...
\Cref{thm:compact-active-descendant}.  Diffuse and noncompact remainders are
excluded by
\Cref{thm:terminal-nonactive-discharge,thm:no-mixed-compact-diffuse}; critical
tails are excluded by
\Cref{thm:realized-critical-tail-rigidity,cor:critical-tail-discharge-complete}.
The pressure-only case is routed by
...
\Cax
   ...
   \varnothing,\\
\Crot
   &\xrightarrow{\;\hbox{local flux routing + terminal assembly}\;}
   \varnothing,\\
\Cstat
   &\xrightarrow{\;\hbox{stationary-hull routing + terminal assembly}\;}
   \varnothing,\\
\Caff
...
\begin{enumerate}[label=\textup{(\arabic*)}]
   ...
   log-window support-profile selection plus hidden-scale compactness; see
   ...
   critical-tail rigidity, descendant heredity, recurrence for concentration chains, finite
...
By the refined ordered decomposition, after extraction it belongs to one of
\[
   \Cax,\Crot,\Cstat,\Caff,\Clogdiff,\Cyoung,
   \Chomcrit,\Clogper,\Capcrit,\Cgen .
\]
The axisymmetric class is empty by \Cref{thm:R1-exclusion}.  The rotational
class is reduced to the terminal stratification assembly theorem by \Cref{prop:R2-routing-to-terminal} and
is empty by \Cref{cor:R2-closure}.  The stationary-hull class is reduced to
the terminal stratification assembly theorem by
\Cref{prop:R3-stationary-hull-routing,prop:R3S-R3-terminal-routing} and is
empty by \Cref{cor:R3-closure}.  No retained normalized representative
remains in \(\Caff\) by \Cref{thm:oscillatory-entry-normalization}.  The
log-diffuse and Young branches are empty by
\Cref{cor:log-diffuse-branch-discharge,cor:young-branch-discharge}.  The
...
decomposition.  Hence
\(\calR^\#(\calS;I,J)=\varnothing\), which is \(\NoResidual\).
\end{proof}
...
\label{cor:setup-residual-hypothesis-discharge}
The residual-class hypothesis
\(\SetupLabel{p1:hyp:no-remainder}\) of the setup paper holds.
\end{corollary}

\begin{proof}
By \Cref{thm:imported-setup-package}, the setup residual hypothesis is exactly
...
\Cref{cor:setup-residual-hypothesis-discharge}, no local pointwise Type~I
singularity exists under the stated admissibility hypotheses.
\end{corollary}

\begin{proof}
This is exactly the conditional final assembly imported in
\Cref{thm:imported-setup-package}, with the residual hypothesis verified by
\Cref{cor:setup-residual-hypothesis-discharge}.
\end{proof}
...
\Cref{thm:imported-setup-package}.  All remaining case-closure steps are proved in this
...
The imported setup results displayed in \Cref{thm:imported-setup-package} are
cited by their companion-paper theorem numbers, with stable source labels kept
as parenthetical audit keys.  This table records the same dictionary in one
place so that Paper~IV can be read without relying on private file names or
...
\SetupLabel{p0:prop:concentration-package} & Terminal concentration package.\\
...
\SetupLabel{p1:hyp:no-remainder} & Residual hypothesis discharged here.\\
\SetupLabel{p1:thm:final-assembly} & Conditional final assembly in the setup
paper.\\
\end{longtable}
\endgroup
Paper~IV uses these results only through the imported package; it does not
invoke private source-file conventions.
...
\((M,\mu,\Phi_t)\) of the centered translation hull, where \(\mu\) is an
\(\Phi_t\)-invariant Borel probability measure (existence of such a
measure is the standard Krylov--Bogolyubov consequence of
\Cref{thm:diffuse-defect-compactness}; uniqueness is not used).  The acting group is
\(G:=\mathbb R^3\rtimes\mathbb R\) (covariant translations and parabolic
time shift).

\subsection{Hilbert framework and unitary representation}
Let \(H:=L^2(M,\mu;\mathbb C)\).  The Koopman representation
\(U_g f:=f\circ\Phi_g^{-1}\) (\(g\in G\)) is a strongly continuous unitary
representation of \(G\) on \(H\), by \(\Phi_t\)-invariance of \(\mu\) and
the dominated convergence theorem.  Write \(\Pi_0:H\to\ker(\nabla_G)\) for
the orthogonal projection onto the \(G\)-invariant subspace of \(H\).  If the
chosen invariant measure is ergodic on the minimal core, this projection is the
spatial mean \(f\mapsto\int f\,d\mu\); the arguments below require only the
projection \(\Pi_0\), not ergodicity or uniqueness of \(\mu\).
...
\(U_{e^{tX_k}}=e^{itA_k}\).  Define the non-negative self-adjoint operator
\(\mathcal L\) by the closed quadratic form
\[
   \mathfrak q(f)
   :=
   \sum_{k=0}^3\|A_k f\|_{L^2(M,\mu)}^2,
   \qquad
   f\in\bigcap_{k=0}^3\mathcal D(A_k).
\]
Thus \(\mathcal L\) is the Friedrichs generator associated with the formal
operator
\[
   \mathcal L=\sum_{k=0}^3 A_k^2,
\]
and \(\mathcal L\Pi_0=\Pi_0\mathcal L=0\).  All inverses below are understood
as resolvent limits of \((\varepsilon+\mathcal L)^{-1}(I-\Pi_0)\).
...
For each \(\varepsilon>0\) and each generator \(A_k\), define the
...
\((\varepsilon+\mathcal L)^{-1}\) is bounded on \(H\) with
norm \(\le\varepsilon^{-1}\) and commutes with \(\Pi_0\); each \(D_k\) is
closed on \(\mathcal D(A_k)\), and
\((\varepsilon+\mathcal L)^{-1}D_k^*F\) lies in the form domain.  Therefore the
...
\begin{proof}
The spectral theorem applied to \(\mathcal L\) gives
\[
   D_k(\varepsilon+\mathcal L)^{-1}(I-\Pi_0)
   \xrightarrow{\;\varepsilon\downarrow0\;}
   D_k\mathcal L^{-1}(I-\Pi_0)
\]
in the weak resolvent sense on the form domain, after projecting away the
invariant kernel.  For \(F\in\mathcal B\), the test
\((\varepsilon+\mathcal L)^{-1}D_k^*F\) converges in the form topology to
\(\mathcal L^{-1}D_k^*F\) modulo \(\Pi_0H\).  Hence the displayed weak pairing
...
distributional spatial derivatives along the chart and \(A^{-1}(I-\Pi_0)\)
restricts to the inverse Laplacian on the centered ancient solution
modulo the kernel of \(G\)-invariant functions; this restriction is the
...
\Cref{thm:diffuse-defect-state-space-construction} and
