...
No hidden-scale variance and annular tightness by definition of \(\Cyoung\). &
Log-diffuse exclusion.\\
...
The covariant action is constructed in two steps.  First let
\(G_{\mathbb Q}\subset G\) be the fixed countable dense subgroup from
\Cref{def:diffuse-defect-observables}.  Because the observable family has been
enlarged by all rational pullbacks, for every \(q\in G_{\mathbb Q}\) one has
\[
   (\mathcal O_k\circ q')(q\cdot\omega)=\mathcal O_k((q'q)\cdot\omega),
   \qquad q'\in G_{\mathbb Q},
\]
so \(G_{\mathbb Q}\) acts on \(\mathcal X_0\) by coordinate permutation and
hence continuously on \(X_{\rm def}\).

Now fix a compact set \(C\Subset G\) and a basic observable \(\mathcal O_k\).
By \Cref{lem:state-space-local-compactness,thm:covariant-observer-calculus},
the orbit maps
\[
   g\longmapsto \mathcal O_k(g\cdot\omega)
\]
are continuous on \(C\) in the local state-space topology; since the realized
normalized defect class is compact in that topology after embedding into the
product compactification, these orbit maps are equicontinuous on \(C\),
uniformly in realized states \(\omega\).  Hence, if \(q_j\to g\) in
\(G_{\mathbb Q}\).

Fix \(x\in X_{\rm def}\), a sequence \(q_j\to g\) in \(G_{\mathbb Q}\), and
\(\varepsilon>0\).  Choose a finite set of coordinates \(k\le K\) so that the
tail of the product metric beyond \(K\) is \(<\varepsilon/3\).  Choose realized
states \(\omega_m\) with \(\iota(\omega_m)\to x\).  By the uniform
equicontinuity of the first \(K\) orbit maps on \(C\), there exist \(m\) and
\(J\) such that for all \(j,\ell\ge J\),
\[
   d_K\bigl(\sigma_{q_j}x,\sigma_{q_\ell}x\bigr)
   \le
   d_K\bigl(\sigma_{q_j}x,\sigma_{q_j}\iota(\omega_m)\bigr)
   +d_K\bigl(\sigma_{q_j}\iota(\omega_m),\sigma_{q_\ell}\iota(\omega_m)\bigr)
   +d_K\bigl(\sigma_{q_\ell}\iota(\omega_m),\sigma_{q_\ell}x\bigr)
   <\varepsilon ,
\]
where \(d_K\) denotes the finite-coordinate metric on the first \(K\) entries.
Together with the metric-tail bound this shows that \(\sigma_{q_j}x\) is Cauchy
in the product metric on \(\mathcal X_0\).  Define
\[
   \sigma_gx:=\lim_{j\to\infty}\sigma_{q_j}x .
\]
Applying the same finite-coordinate estimate to two rational approximating
sequences \(q_j\to g\) and \(q_j'\to g\) gives independence of the limit from
the chosen approximation.  The group law follows by approximating both factors
from \(G_{\mathbb Q}\), and the same finite-coordinate argument, carried out
uniformly on compact subsets of \(G\times X_{\rm def}\), gives joint continuity
of \((g,x)\mapsto\sigma_gx\).
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \Cref{def:pressure-gauge-affine-quotient}, and for every pair
   \((F,\psi)\) in the fixed countable \(C^2\)-test family of
   \Cref{def:critical-pressure-gradient-convergence} the scalar defect
   coordinates \(\mathscr P_{n,F}[\psi]\) and \(\mathscr D_{n,F}[\psi]\)
   converge;
   \item in the realized recurrent setting, the local virial, dissipation, and
   pressure-flux coordinates on the fixed rational family of log-windows are
   recorded as part of the compactification data rather than asserted to be
   continuous for the weak velocity-pressure topology alone;
...
\(\mathscr P_{n,F}[\psi]\) and viscous-defect coordinates
...
on the fixed countable family they are uniformly bounded by the finite-\(n\)
Young identity and the local energy bound, so a diagonal extraction gives
convergent subsequences for all of them simultaneously.  For realized recurrent
profiles, the virial coordinates are likewise uniformly bounded on the
countable rational family by \Cref{lem:realized-log-window-virial-compactness},
hence admit simultaneous limits as coordinate data of the recurrent virial
hull.  The affine quotient data
...
chosen countable annular exhaustion.
...
inside the affine quotient.  In addition, for every
\((F,\psi)\in\mathfrak F_{\rm crit}\) the scalar defect coordinates
\[
   \mathscr P_{n,F}[\psi]
   :=
   \iint \psi\,\nabla_\xi F(V_n)\cdot\nabla Q_n
...
\[
   ...
   \psi\,\nabla V_n^T\nabla_\xi^2F(V_n)\nabla V_n
\]
are required to converge along the selected compactification subsequence.
\end{definition}
...
\(\mathscr D_F[\psi]\).  For each finite \(n\), the exact identity
...
\begin{aligned}
   ...
   +\mathscr P_{n,F}[\psi]+\mathscr D_{n,F}[\psi]+\mathscr E_{n,F}[\psi].
\end{aligned}
\end{equation}
holds, and \(\mathscr E_{n,F}[\psi]\to0\) as \(n\to\infty\).  Consequently the Young measure satisfies the exact weak
balance
...
identity also gives a uniform bound for the pressure pairings
\(\mathscr P_{n,F}[\psi]\) on the fixed countable family.  These scalar
...
These quantities are not claimed to be continuous under weak
\(L^\infty\)-velocity and distributional-pressure convergence alone: quadratic
energy is only weakly lower semicontinuous, dissipation is not weakly
continuous, and the pressure flux requires more than distributional convergence
against fixed test fields.  Instead, one enlarges the recurrent compactification
by these bounded scalar coordinates.  Tychonoff compactness and diagonal
extraction on the countable rational family of \((\rho,A)\) then produce limits
for all virial coordinates simultaneously.  By construction these coordinate
maps are continuous on the resulting compact virial hull.  If at any stage the
homogeneous gauge sequence loses compactness,
...
\Cref{def:first-bad-spatial-blowdown}.  Then \((V_n,Q_n^{\rm act})\) satisfy the local
...
\(L^\infty\) bounds \(|V_n|\le M\), the actual-pressure compactness modulo
time-only gauges recorded in
\Cref{lem:actual-pressure-minimal-scale-energy}, H\"older's inequality on the
...
\(r_n^{-1}\).  After passing to the time-only pressure-gauge subsequence
supplied by \Cref{lem:actual-pressure-minimal-scale-energy} and to the bounded-velocity
...
Let \(V_n,W_n,c_n,Q_n^{\rm act},Q_n^\sharp\) be as in
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   assigned to \(\Caff\).
   \item The affine-quotient pressure \(Q_n^\sharp=Q_n^{\rm act}-L_n\), with
...
particular, the actual minimal-scale pressure \(Q_n^{\rm act}\) used in the
local energy inequality carries only the scaled Riesz part and time gauges; the
affine term \(\beta(s)\cdot y\) appears only after passing to an affine-quotient
representative.
\end{proposition}
...
the only possible surviving harmonic component is affine.  After scaling,
\[
...
\[
   L_n(x,s):=\beta(s)\cdot x+a_n(s)+r_n^{-1}\alpha(s),
\]
...
\Cref{lem:scaled-pressure-compactness-affine} implies strong
\(L^{3/2}_{\rm loc}\)-compactness modulo time-only gauges, with limit zero, on
every compact cylinder.  If an affine pressure/velocity mode survives after the
quotient normalization, then by definition that mode is routed to \(\Caff\),
which is alternative \textup{(i)}.
...
\Cref{lem:pressure-velocity-product-convergence}.  The time-only gauge itself
does not contribute to the local energy flux, since for every compactly
supported \(\phi\),
\[
   \iint a_n(s)V_n\cdot\nabla\phi
   =-\iint a_n(s)\phi\,\nabla\cdot V_n=0.
\]
Thus the pressure flux is determined by the actual non-affine pressure alone and
converges to zero in the non-affine quotient.  This is alternative
\textup{(ii)}.
\end{proof}
...
\Cref{lem:first-bad-pressure-compactness} applied to \(Q_n^\#\) gives either a
retained descendant in \(\Caff\), or a quotient pressure limit \(Q\) whose
gradient enters the affine-normalized quotient equation.
...
\(Q_n^{\rm act}\to0\) strongly in \(L^{3/2}_{\rm loc}\) after time-only gauges.
In the latter case the pressure flux converges distributionally to
...
\[
   |W_n|\le 2M
\]
on every selected macroscopic cylinder, and Banach--Alaoglu gives weak-* compactness of
\(W_n\) in \(L^\infty_{\rm loc}\).  The fundamental theorem of Young
...
an element of \(\Tcrit\).  The pressure-defect coordinates
\(\mathscr P_{n,F}[\psi]\), the viscous-defect coordinates
\(\mathscr D_{n,F}[\psi]\), and, in the realized recurrent setting, the
virial coordinates are extracted simultaneously on the fixed countable dense
families entering the compactification.  Since the normalized defect mass is one,
\(\mathcal T\ne0\).
...
\Cref{thm:no-hidden-scale-variance-realized-defects}.  The Young alternative is
reduced to that compactness statement by the
Fr\'echet--Kolmogorov/Young-measure equivalence in
\Cref{lem:hidden-variance-young-obstruction}.  The log-diffuse alternative is
...
\(\Lambda=0\), and the annular Young measure is non-Dirac on a set of positive
macroscopic measure.  Let \(W_n\) be the corresponding annular blow-down and
...
Hence alternative \textup{(ii)} of
\Cref{lem:realized-amplitude-log-hull-heredity} is not an admissible
\(\Capcrit\) object: if it occurred, the construction would have been routed
earlier to regenerated concentration, the affine/parasitic quotient,
log-diffuse escape, a critical-tail defect, or hidden-scale variance, and the
profile would not have entered the coherent aperiodic row.  Therefore, for a
profile genuinely lying in \(\Capcrit\), only alternative \textup{(i)}
remains.  In that case \(0\in\mathcal H(Z_*)\).
