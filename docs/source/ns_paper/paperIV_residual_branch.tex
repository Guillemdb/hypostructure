...
\(\mathcal D\).  If \(\Lambda(\Tcrit\setminus\mathcal F)>0\), the concrete
support-transfer lemma \Cref{lem:log-window-support-transfer} gives a nonzero
log-window normalized support profile outside \(\mathcal F\).  This contradicts
the hypothesis that every nonzero log-window normalized support profile lies in
...
and let \(X_Y(K)=K\times\mathcal P(\overline{B_M})\), where
\(\overline{B_M}\) is the compact velocity ball fixed by the local
\(L^\infty\) bound.  Let
\[
...
\(\mathcal P(\overline{B_M})\) is compact metrizable in the weak topology.
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \item the failure of such realized bounded-origin heredity produces one of
   the already stratified lower alternatives:
   \[
      \Cactive,\qquad \Caff,\qquad \Clogdiff,\qquad
      \hbox{critical-tail defect},\qquad
      \hbox{or hidden-scale variance}.
   \]
\end{enumerate}
\end{lemma}
...
It remains to identify the complementary case.  If the
amplitude-normalized translated windows cannot be realized with the same
bounded-origin control, then one of the realized extraction inputs has failed.
If positive unit concentration is regenerated, the branch is \(\Cactive\).  If
the failure is purely homogeneous or affine in the velocity/pressure quotient,
it is \(\Caff\).  If mass escapes in logarithmic radius before a tight
amplitude-normalized profile is obtained, it is a log-diffuse or critical-tail
defect.  If tightness holds but the Young measure is not Dirac on an annular
window, it is hidden-scale variance.  These are precisely the lower rows in the
ordered critical-tail compactification, and all are sequence-realized support
profiles by \Cref{lem:critical-tail-support-transfer,lem:log-window-support-transfer,lem:young-variance-support-transfer}.
This gives alternative \textup{(ii)}.
\end{proof}
...
\[
   ...
   \qquad\hbox{as }\rho\to-\infty .
\]
The lower-stratum alternatives are already removed before the coherent
aperiodic row is closed.  In the remaining case \(0\in\mathcal H(Z_*)\).
