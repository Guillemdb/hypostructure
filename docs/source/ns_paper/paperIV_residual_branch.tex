...
The setup paper \(\texttt{proof\_setup.tex}\) proves the following facts.
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   \item Positive terminal local concentration at every local pointwise Type~I
   singular point.
   \begin{quote}\small
   ...
   sequence with pressure gauges, no-escape, and retained unit-scale
   concentration.
   \begin{quote}\small
...
unit CKN concentration bound
\[
   \liminf_n\calE_1(\cdot;0)\ge\eta_*>0 .
\]
A \emph{normalized support profile} is a normalized profile in the support of a
defect measure or normalized window measure associated with the original
blow-up sequence.  It may arise from
...
retained only when the displayed unit CKN concentration bound is explicitly proved;
...
\Cref{thm:refined-residual-closure-complete-decomposition} is realized from the original blow-up sequence in the
...
limit remains in the normalized compactness class and keeps the fixed retained CKN
...
R2 reduction &
Rotational flux identities, finite unit-cylinder concentration cover, and terminal
concentration extraction. &
R2 closure.\\
R3 reduction &
Hull realization, centered scale reselection, and setup heredity. &
R3 closure.\\
...
Terminal stratification, diagonal lifting, finite directed graph recurrence,
and recurrence for concentration chains. &
Global \(L^3\) bound.\\
Atomic sequence-\(L^3\) &
...
structured/decay classes.  This is the pre-quotient residual class
\(\calR_{\rm pre}(\calS;I,J)\).  Passing to the canonical affine/parasitic
...
\begin{equation}\label{eq:local-energy-main}
   \calE_r(V,P;z_0):=C_r(V;z_0)+D_r(P;z_0).
\end{equation}
...
\(\Cstat\) & Stationary-hull residual class with retained local concentration.\\
...
Retained descendant & Descendant with inherited positive unit CKN concentration.\\
Normalized support profile & Normalized defect support profile, not necessarily
retained at the original threshold.\\
\hline
\end{longtable}
...
satisfying the hypotheses of the terminal stratification theorem.
\end{proposition}
...
\(\varepsilon_*\)-concentrating if it has retained concentration in the normalized
unit sense: there is a recentering/rescaling realized from the original blow-up
sequence
\(\mathscr T\) such that
\begin{equation}\label{eq:R3-active-ckn}
   \calE_1\bigl(\mathscr T(W,\Pi);0\bigr)
   \ge \varepsilon_* .
\end{equation}
Equivalently, before the final unit normalization one may require
\(\calE_r(W,\Pi;z_0)\ge\varepsilon_*\) for some parabolic cylinder
\(Q_r(z_0)\).  Unnormalized large-cylinder mass is not a retained concentration
criterion.
\end{definition}
...
concentration.  Its proof is deferred until the terminal stratification theorem
...
The proof is given after the terminal stratification theorem in
...
concentration theorem is available.
\end{proof}
...
contradiction is deferred to the terminal stratification theorem.  Thus the
...
terminal stratification theorem has been proved.
\end{remark}
...
only after the terminal stratification theorem:
\[
\begin{gathered}
...
% -----------------------------------------------------------------------------
\section{Reduction of degenerate stationary-hull profiles with retained concentration}
...
\subsection{Deferred terminal concentration theorem for R3}

We record the following specialization of the terminal local concentration
theorem.  It is stated here exactly in the form needed for the stationary-hull
closure with retained local concentration, with proof deferred to the terminal
section.

\begin{theorem}[Deferred terminal concentration theorem for R3]
...
\begin{enumerate}[label=(\roman*)]
   ...
   of \Cref{thm:terminal-exhaustion-main}: a retained concentration set
...
\Cref{thm:terminal-local-stratification-package}.  It is stated here only as a
...
sequence-\(L^3\) lemma used after the terminal theorem.
...
terminal indecomposability hypotheses used there: retained unit concentration, no
...
structure; once the terminal theorem is available, the stationary concentration
argument applies to these profiles in particular.
\end{proof}

\begin{proposition}[Stationary-hull reduction to the terminal concentration theorem]
\label{prop:R3S-R3-terminal-routing}
Let \(\Cstat(\mathcal S;I,J;\mathfrak B)\) be the degenerate
stationary-hull residual class with retained local concentration.  Every
...
\Cref{thm:terminal-local-stratification-package}.
\end{proposition}
...
theorem in the stationary case.  The actual emptiness conclusion is recorded
after that terminal theorem as \Cref{thm:R3S-R3-empty}.
\end{proof}
...
profile carrying retained concentration or non-affine oscillation.
\end{remark}
...
\begin{lemma}[Gauge compatibility and retained-concentration semicontinuity]
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   for lifting concentrating limits:
   \[
      \calE_1(U,\Pi;0)
      \le
   ...
   \item If retained concentration is carried by the velocity part, then it is
...
\[
   \hbox{retained concentration set}
   \quad+\quad
   \hbox{residual remainder}.
\]
More precisely, after passing to a subsequence, the compact observer-window
concentration sets \(A_m^\eta\) of \Cref{lem:maximal-active-family} exist
...
the local Seregin compactness or pressure-gauge control.  Thus mixed configurations such
...
It remains to classify what is outside the concentration sets.  If
\(\mu_{n,m,\eta,\rho}^{\rm rem}(K_m)\to0\) for every compact observer window
and then no residual mass survives on the selected expanding terminal regions,
the remainder is \(\mathcal R_{\rm van}\).  If a positive amount of residual
mass persists on expanding regions while every fixed compact observer window is
below every concentration threshold after removing neighborhoods of the concentration sets,
the remainder is \(\mathcal R_{\rm diff}\).  These three residual states are
mutually exclusive once the concentration sets are fixed, and they exhaust the
residual possibilities.  No step asserts that a diffuse or exterior remainder
cannot coexist with compact concentration profiles.
\end{proof}
...
therefore carried by the retained concentration set of
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   the fixed smoothing convention of \Cref{lem:maximal-active-family}.
...
positive-threshold concentration sets from \Cref{lem:maximal-active-family}.  By
...
positive normalized spatial quotient oscillation mass, no unit-scale concentration
...
closed under retained descendants and normalized support profiles.  Suppose that every nonzero normalized
...
non-affine diffuse content and does not regenerate unit-scale concentration.
...
\]
   ...
   topology modulo gauges, with retained local concentration at the origin.  An ordinary
...
\begin{lemma}[Time-slice concentration gives parabolic CKN concentration]
...
terminal extraction would be a mixed profile configuration consisting of the retained compact
...
Define the parabolic exterior concentration level
\[
   m_n^{\rm par}
   :=
...
\begin{definition}[Closed retained concentration successor relation]
...
\begin{lemma}[Closedness of the retained concentration successor relation]
...
\begin{theorem}[Recurrence for retained concentration chains]
...
\begin{lemma}[Recurrent path produces a compact retained concentration core]
\label{lem:recurrent-path-compact-core}
Let \(\mathbb P\) be a shift-invariant probability measure on the retained concentration path
...
lemma preserves realization from the original sequence and retained concentration,
...
\begin{theorem}[No infinite retained concentration chain]
...
\begin{theorem}[Terminal stratification theorem]
...
a retained unit CKN concentration lower bound
\[
   \liminf_{n\to\infty}\calE_1(V_n,P_n;z_n)\ge\eta_*>0 .
\]
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \item a finite separated family of retained concentration profiles occurs;
...
profile satisfying the retained unit CKN concentration lower bound remains.
\end{theorem}
...
\Cref{lem:atomic-sequence-L3,hyp:mildness-inheritance-main,lem:no-atomic-active}
...
\begin{remark}[No global budget in the terminal theorem]
\label{rem:no-global-budget-terminal-theorem}
The terminal stratification theorem assumes no global \(L^3\), global CKN
...
realized from the original blow-up sequence with retained unit CKN concentration.
...
profile satisfies the retained unit CKN concentration hypotheses of
...
class is reduced to the terminal concentration theorem by \Cref{prop:R2-routing-to-terminal} and
is empty by \Cref{thm:R2-exclusion-safe}.  The stationary-hull class is reduced to
the terminal concentration theorem by
\Cref{prop:R3-stationary-hull-routing,prop:R3S-R3-terminal-routing} and is
empty by \Cref{thm:R3S-R3-empty}.  No retained normalized representative
...
The residual-class hypothesis
\(\textup{\texttt{p1:hyp:no-remainder}}\) of the setup paper holds.
\end{corollary}
...
This is \Cref{thm:refined-residual-closure-complete-decomposition}.
\end{proof}
...
\(\textup{\texttt{p1:thm:final-assembly}}\) and
...
\item recurrence for chains of retained concentration profiles;
...
\Cref{thm:R2-exclusion-safe} proceeds through the localized Coriolis-flux
