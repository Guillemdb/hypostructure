...
\newcommand{\SetupLabel}[1]{\textup{\texttt{#1}}}
...
stationary-hull profiles with retained local velocity concentration, and the generic
terminal residual.  The terminal case is then refined into the
...
and still carries local velocity concentration; it is then excluded by the terminal
stratification assembly theorem.  The final class is reduced by covariant tail
normalization, concentration-set extraction, diffuse-defect compactness,
critical-tail compactification, recurrence for chains of concentration profiles,
exclusion of finite separated concentration profiles, indecomposable-profile sequence-\(L^3\)
...
depends only on the imported setup theorem stated below and on cited
...
\begin{theorem}[Imported setup theorem]
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   \SetupLabel{p0:cor:paper0-local-typeI-bridge-admissible}\\
...
Below, the phrase ``by the imported setup theorem'' means that the terminal
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \(\Vel_1\ge\eps_*\) after the fixed admissible pressure gauges;
...
\begin{enumerate}[label=\textup{(\alph*)}]
   ...
   the fixed descendant lower bound \(\Vel_1\ge\eps_*\).
...
\begin{theorem}[Imported weak-Serrin local Type I estimate]
...
\[
   ...
   (t_0-t)^{1/2}\|v(t)\|_{L^\infty(B_\rho(x_0))}<\infty .
\]
The setup paper imports the Albritton--Barker weak-Serrin estimate in the exact
form needed here: for every \(0<\theta<1\), the scale-invariant local Type~I
bounds are finite on \(Q_{\theta\rho}(z_0)\).  That is, there is
\[
   K_{\theta}<\infty
\]
...
Albritton--Barker.  The constant is allowed to depend on the local
suitable-solution background quantities on \(Q_\rho(z_0)\), including the local
\(C\)- and \(D\)-bounds.
\end{theorem}

\begin{proof}
This theorem is quoted from the companion setup paper; Paper~IV does not
reprove the weak-Serrin-to-local-Type~I estimate.  Its only role here is to put
first-generation terminal sequences inside the admissible local-window class and
to supply the local energy, pressure, and no-escape bounds used by
\Cref{def:first-generation-local-window-condition}.
The local pointwise Type~I envelope is the endpoint weak-Serrin hypothesis
\(L^{2,\infty}_tL^\infty_x\) on the cylinder.  The setup theorem cited in
\Cref{thm:imported-setup-package} imports the Albritton--Barker local estimate
in precisely this endpoint form and supplies the constants needed for terminal
no-escape.  Paper~IV uses only this endpoint consequence, not a broader
restatement of the weak-Serrin theorem.
\end{proof}
...
the imported setup theorem and the local concentration input of the companion
...
\begin{itemize}[leftmargin=1.6em]
   ...
   \item \(\Tcrit\): terminal profile class, \Cref{def:terminal-state-space}.
   \item \(\Tcrit^\#\): compact normalized descendant profile class,
...
\begin{itemize}[leftmargin=1.6em]
   ...
   \item \(\eps_{\rm sd}\): local small-data threshold,
      \(0<\eps_{\rm sd}\le\min\{\eta_0,\eta_v,\eps_{\CKN}\}\),
      \Cref{def:threshold-hierarchy}.
   \item \(\eps_*\): retained descendant velocity threshold,
...
\(\varepsilon_*\)-concentrating if it has retained velocity concentration in the normalized
...
\(\Vel_r(W;z_0)\ge\varepsilon_*\) for some parabolic cylinder
...
closure with retained local velocity concentration, with proof deferred to the
terminal section.

\begin{theorem}[Deferred terminal stratification corollary for R3]
...
compact parabolic cylinder \(Q_r(z_0)\) and some \(\varepsilon_*>0\),
...
imported setup theorem: local positive concentration
...
\begin{proof}[Proof deferred]
The proof is the stationary case of the R3 closure after
\Cref{thm:terminal-stratification}; see \Cref{cor:R3-closure}.  This theorem is
kept here to name the exact stationary concentration statement used by the
stationary-hull routing.
\end{proof}
...
\begin{proof}[Proof deferred]
The proof is included in the post-terminal R3 closure,
\Cref{cor:R3-closure}.  The stationary-family condition in (i) is additional
structure; once the terminal stratification assembly theorem is available, the stationary concentration
...
\]
Hence \(W\in\calH_\infty(V)\).  Sequential compactness in the metrizable local
smooth topology gives compactness.
\end{proof}
...
the imported setup theorem supplies the local pressure representatives; the present
...
the imported setup theorem.  Then
...
formulation recorded in the imported setup theorem.  For a spatially constant
...
\begin{definition}[Atlas-dependent defect measure and concentrating recentering sequences]
...
\Cref{def:local-critical-quantity}.  It is used for local compactness and
concentration-set extraction.  Scale-invariant threshold and regularity statements
are written with \(\calE_r\).
...
limit, and the remaining lower bound is pressure-only.  Exactly one of the
...
cylinder for all large \(n\).  The imported setup theorem therefore gives the
...
\Cref{lem:compact-window-expanding-region-protocol} to that region gives exactly
one of the listed alternatives: a selected unit observer with velocity
...
\begin{lemma}[Compact-window to expanding-region terminal protocol]
...
measures.  Fix a countable family of selected expanding terminal regions
\[
   E_n^{(j)}\subset Z,\qquad j\in\mathbb N,
\]
where each \(E_n^{(j)}\cap K_m\) is Borel and each selected exterior or tail
window used later is one of these regions after passing to a subsequence.  After
one diagonal subsequence, for every \(j\), every rational \(\eta>0\), and every
rational \(\rho>0\), exactly one of the following alternatives occurs for the
concentration-deleted measures
\[
...
\(E_n^{(j)}\) and rational deletion radii \(\rho\).  If alternative
...
activity above the retained threshold, it is a retained active recentering.  If
the local compactness or pressure gauges fail along the selected observer
sequence, this is \textup{(c)}.  If compactness holds but the lower bound is
pressure-only, \Cref{lem:no-pressure-only-retained-profile} routes it either to
\textup{(c)}, to the affine/parasitic quotient, or to a neighboring velocity
observer.  If every unit observer remains below threshold and compactness holds,
the residual mass is diffuse on that compact window; after normalization it is
the same measure-theoretic object as in \textup{(b)}, restricted to the compact
part of \(\overline Z\).
...
\textup{(b)}.  Finally suppose that the residual mass is invisible at every
...
\]
Normalize the residual measure on this selected window.  If the selected
windows stay in a fixed logarithmic annulus, the normalized measure has a
compact diffuse limit already covered above; otherwise the corresponding
scales \(R_n=2^{k(n)}\) tend to \(0\) or \(\infty\), and the normalized
annular/logarithmic measures give the critical-tail alternative
\textup{(d)}.  This selection is local in the chosen windows and uses no global
mass budget.
\end{proof}
...
supplied by the imported setup theorem.  Then the retained
...
the imported setup theorem, failure of local vanishing at a singular terminal
...
contained the compact lower-bound threshold from the imported setup theorem, the
...
the imported setup theorem, and fixed compact terminal cylinders.
\end{proof}
...
Let
\[
   X_{\rm def}:=\mathfrak D_M
\]
be the compact metrizable observer-defect state space, defined as the compact
closure, in the weak topology of
...
diffuse-defect profiles produced by \Cref{thm:diffuse-defect-compactness}.  The
...
\(\Theta_s\).  The closed subset
\[
   \mathfrak D_M^{\rm anc}\subset\mathfrak D_M
\]
is the closure of diffuse-defect states realized from the original blow-up
sequence in the sense of \Cref{def:ancestor-realized-residual-object}.  It is
closed by definition and \(G\)-invariant because the covariant observer maps are
among the allowed realized operations.
\end{definition}
...
Convergence in \(\Tcrit\) means local weak-\(*\) convergence of the bounded
velocity representatives, pressure convergence modulo time and affine gauges in
the distribution topology needed for the limiting equation, vague convergence
of the defect measures, weak convergence of the Young measures against bounded
continuous test functions, convergence of the logarithmic annular coordinates,
and convergence in the affine pressure quotient fixed in
...
\(W_n=V_n-c_n(s)\) changes only the spatially homogeneous quotient component;
if that component is retained, it is assigned to \(\Caff\), and otherwise the
canonical affine normalization removes it.  Therefore, in the non-affine
quotient, on fixed macroscopic annuli,
\[
   \partial_s W_n
   -R_n^{-2}\Delta_xW_n
   +R_n^{-1}V_n\cdot\nabla_xV_n
   ...
   =\operatorname{Err}_n,
\]
where \(\operatorname{Err}_n\to0\) in distributions and consists only of the
discarded homogeneous affine/parasitic quotient terms.
...
terminal sequence after terminal concentration-set removal.  Then exactly one of the
following ordered profile alternatives occurs:
\begin{enumerate}[label=\textup{(\alph*)}]
...
occurs: regenerated concentration, affine/parasitic collapse, log-diffuse defect,
...
\]
for every integer \(k\).  If \(L>0\), send \(k\to+\infty\); if \(L<0\), send
\(k\to-\infty\).  In both cases the orbit approaches \(r=0\), where
bounded-origin realization gives zero.  Hence \(Z(\rho,\omega,s)=0\) for every
\(\rho\), so \(W\equiv0\), again contradicting nonzero critical tail mass.
Thus \(\Clogper=\varnothing\).
...
It remains to consider the case in which \(m_n^{\rm vel}<4\eps_*\) for
...
\[
   \int_{|y|>R_n}|U(y,\tau_n)|^3\,dy\ge1
\]
together with the parabolic velocity smallness \(m_n^{\rm vel}<4\eps_*\)
is now
...
\(m_n^{\rm vel}<4\eps_*<c_{\rm sl}(M,\alpha_0)\).
...
by the imported setup theorem.  The fact that the diffuse or
...
the imported setup theorem give compactness of \(\calK\) on every fixed terminal
...
\]
Assume all setup-closed lower classes have been removed.  Then the paired
...
\Cref{thm:terminal-exhaustion-main}.  This theorem does not introduce an
...
The lower classes are empty by the imported setup theorem, and the terminal
...
sequence.  The lower classes are empty by the imported setup theorem.  The stationary terminal
...
normalization eliminates non-affine oscillation, then
...
oscillation threshold survives.

Apply the paired terminal concentration-set/remainder decomposition to a terminal
extraction of this tail-normalized candidate.  The distinguished compact
velocity mass from the setup paper still forces the terminal concentration set to be
nonempty by \Cref{cor:persistent-forces-active}; after affine normalization the
surviving residual oscillation is measured by \(\Phi\).  The pure radiative
...
\Cref{thm:no-mixed-compact-diffuse}.  Infinite chains of retained concentrating descendants are excluded by
\Cref{thm:compact-active-descendant}.  Thus the concentration set reduces, in the
...
profile possibilities are impossible.  Hence no generic residual profile
exists.
\end{proof}
...
The labels displayed in \Cref{thm:imported-setup-package} are the companion
setup paper's theorem labels.  In a standalone arXiv submission, these labels
should be replaced by the final theorem numbers of the companion paper or kept
as a label dictionary in the following form.
\begingroup
\scriptsize
...
bridge.\\
\SetupLabel{p0:cor:paper0-local-typeI-bridge-admissible} & Admissibility of
