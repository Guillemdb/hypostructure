...
\(G_{\mathbb Q}\), then for every \(x\in X_{\rm def}\) the sequence
\(\sigma_{q_j}x\) is Cauchy in the product metric on \(\mathcal X_0\).  Define
\[
   \sigma_gx:=\lim_{j\to\infty}\sigma_{q_j}x .
\]
The equicontinuity makes the limit independent of the approximating rational
sequence and gives the group law by approximation from \(G_{\mathbb Q}\).
The same argument, applied uniformly on compact subsets of \(G\times X_{\rm def}\),
gives joint continuity of \((g,x)\mapsto\sigma_gx\).
...
|\nabla V_n|^2\psi\,dx\,ds\le C\), the Hessian term has a subsequential
distributional limit on the countable test family; this is
\(\mathscr D_F[\psi]\).  The second term equals
...
For spatial/logarithmic blow-down limits with scale \(r_n\to\infty\) or
\(R_n\to\infty\), \(\kappa=0\), because the finite-\(n\) representatives have
coefficient \(r_n^{-1}\) or \(R_n^{-1}\).  Outside the affine/parasitic
...
blow-down \(x\mapsto r_nx\), the pressure is normalized as
\[
   Q_n(x,s)=r_n^{-1}\Pi_n(r_nx,s),
\]
...
\(Q_n\) is the scaled whole-space Riesz-pressure representative, with
blow-down coefficient \(r_n^{-1}\), of the minimal-scale rescaled solution,
normalized modulo the affine pressure
...
\Cref{thm:oscillatory-entry-normalization} by the same routing argument; and
...
Equivalently, the final exclusion is the composition of seven ordered steps:
\begin{enumerate}[label=\textup{(\arabic*)}]
