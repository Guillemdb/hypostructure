...
velocity threshold.  Log-window and Young/variance normalizations are called
...
measure has no mass outside \(\mathcal A\); this is the mechanism made
...
\begin{definition}[Velocity retention and unit CKN activity]
...
\begin{theorem}[Compactness of the escaping covariant hull]
\label{thm:escaping-covariant-hull-compactness}
Let \(V\) be a bounded smooth ancient solution of \eqref{eq:centered-main}.
Then \(\calH_\infty(V)\) is nonempty and compact in
...
normalized class \(\mathfrak X_M\).  For every covariant recentering
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \item It acts on an already extracted bounded ancient profile; then
   ...
   \Cref{lem:escaping-first-generation-alternatives} assigns it to an exterior,
...
\Cref{lem:ancient-profile-covariant-compactness}.  These cases exhaust the
...
annulus is \(\{a_kR_n<|x|<b_kR_n\}\), giving alternative
\textup{(1)}.
\end{proof}
...
\Cref{lem:retained-recentering-alternatives}.  Local vanishing, exterior escape, and
diffuse exterior concentration are removed by
\Cref{lem:radiative-stratum-exclusion}.  The proof uses only the positive
concentration input and the local compactness hypotheses in
the imported setup results, and fixed compact terminal cylinders.
\end{proof}
...
\]
where \(\omega\) ranges over the diffuse-defect candidates produced by
...
\]
with pressure representatives \(Q_n\) in the time/affine quotient converges to
\(\mathcal T=(\bar W,\mathcal Y,\Lambda)\), with pressure representative \(Q\),
in \(\Tcrit\) if:
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   observable with compact support in the macroscopic variables: for every
...
Every bounded sequence-realized family of critical-tail profiles has a convergent
subsequence in \(\Tcrit\).  If all profiles in the sequence are sequence-realized,
then the limit is sequence-realized.
\end{theorem}
...
\((\bar W,\mathcal Y,\Lambda)\) be the limiting critical-tail profile.  After
...
Young critical-tail alternatives rather than silently discarded.
...
\Cref{lem:recurrent-virial-equality-reduction} is automatic for realized
recurrent defects, after either ordinary minimal mesoscopic-scale reduction or
membership in the affine/parasitic stratum \(\Caff\).
\end{lemma}
...
\Cref{lem:recurrent-virial-equality-reduction} is verified, and the reduction
provided by the minimal mesoscopic-scale theorem
\Cref{thm:first-bad-mesoscopic-reduction} closes the diagram.
\end{proof}
...
\Cref{def:sequence-realized-residual-object}, and let
\[
   V_n(x,s)=U_n^{z_n}(r_nx,s),
   \qquad
   Q_n(x,s)=r_n^{-1}\Pi_n^{z_n}(r_nx,s),
...
\(\beta(s)\cdot y\) is the affine pressure paired with the spatially
...
remaining pressure is the scaled Leray representative already estimated
...
This is the affine/parasitic lower stratum.  If this alternative does
...
inequality; the homogeneous gauge has not been subtracted.  In
...
The strong pressure convergence gives
\[
...
hand, the barycenter equation for \(\bar V\) is an equality for a bounded
smooth macroscopic limit, so testing it with \(\bar V\) gives
\[
...
rescaled.  The scaling identities is
\[
\begin{array}{c|c}
...
\[
   ...
   \nabla\cdot\bar W=0
\]
in distributions on macroscopic annular cylinders, and the Young/log state
\((\mathcal Y,\Lambda)\) satisfies the balance identity of
\Cref{lem:critical-tail-young-log-transport}; in particular the log-scale
component satisfies the induced log-dilation balance, while nonlinear
...
Young/log balance: the log-scale component satisfies the induced
log-dilation balance, while nonlinear velocity observables retain the pressure and
...
By \Cref{cor:log-diffuse-branch-exclusion}, the log-diffuse alternative is already
...
escaping recentering with retained unit velocity concentration loses the local dissipation or
pressure-gauge bounds in \Cref{def:terminal-measures}, then after its own local
normalization it is a retained unit-velocity recentering sequence.  This
contradicts \Cref{lem:retained-recentering-alternatives}.  Thus it remains to exclude a
...
\(B_1(0)\) is the original ball \(B_1(x_{n,j})\).  Hence
\[
...
below the velocity threshold fixed by \(c_{\rm sl}(M,\alpha)\), pressure-only
...
later iterated extraction of descendant profiles.
...
excluded by
\Cref{thm:terminal-inactive-exclusion,thm:no-mixed-compact-diffuse}; critical
...
\begin{enumerate}[label=\textup{(\arabic*)}]
   ...
   \Cref{cor:log-diffuse-branch-exclusion}.  The alternative \(\Cyoung\) is
   excluded by the same hidden-scale compactness; see
   \Cref{cor:young-branch-exclusion}.  The alternatives
...
\[
   \mathcal L=-\sum_{k=0}^3D_k^2
\]
on the common cylinder core.  This is the operator denoted by
\(-\sum D_k^2\) in the recurrent-core pressure argument.
