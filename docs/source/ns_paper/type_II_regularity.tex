...
local compactness position, positive finite critical annulus, critical tightness,
...
compactness, critical-annulus, critical-tightness, local windowed \(H^1\),
...
localized-energy closure criterions are automatic for every compactly supported fixed
cutoff and every moving cutoff with \(R(\tau_0)<\infty\).
\end{lemma}

\begin{proof}
The pre-carrier regime includes the positive finite critical \(L^3\) annulus.
...
in the pre-carrier regime gives the finite initial localized energy required
by either localized-energy criterion through
...
\par
Since \(\omega\) is in the pre-carrier regime, the positive finite critical
...
in the pre-carrier regime, the analytic, representation, critical-annulus,
compact-tightness, local \(H^1\), modulation, and cost-compatibility rows in
...
\par
The positive inner concentration needed for (R2) is inherited from the
pre-carrier row itself, not inferred from the \(J_7\)-density.  That regime
has already passed the positive finite critical \(L^3\)-annulus test.  Since
the present occupation state has no exterior/leakage label, the corresponding
critical mass remains inside the retained compact single-core region.  Cover a
...
quantity along the represented sequence, then the retained positive critical
annulus would either vanish on the compact core or exit through the exterior
face; the first contradicts the positive-annulus row and the second is exactly
the exterior/leakage label excluded above.  Thus, after passing to a
...
local Caccioppoli estimates, exterior regularity, and the stationary rigidity
input for the compact scale-collapse limit.
...
\begin{assumption}[Perturbative estimates for strict outer profiles]
...
\begin{assumption}[Terminal windowed state-space completeness]
...
conclusion is derived from the terminal local compactness hypotheses
(\cref{paper6a:def:terminal-critical-profile-hypotheses}), namely local compact
terminal-window estimates, local perturbative residual stability, and local
...
freely replaced by that conditional hypothesis set whenever the terminal local
compactness hypotheses are available.
\end{remark}
...
completeness, local perturbative residual stability, the exterior annulus
alternative, repaired-gauge representation, local Caccioppoli regularity, and
the compact scale-collapse stationary-rigidity input, no active multibubble
...
regime), and the renormalized Lebesgue control
\(\sup_{\tau\ge\tau_0}\|V(\tau)\|_{L^3(\mathbb R^3)}\le M<\infty\)
that is part of the pre-carrier validated compact-cost regime data
...
by the regime.  Assume further that
...
\begin{assumption}[Local terminal state-space completeness]
...
nonretained residual \(S_n\).  Local terminal state-space completeness is the
following purely local assertion.
\begin{enumerate}[label=\textup{(\roman*)}]
\item On every \(Q_{2R,I}\), the represented fields satisfy the repaired-gauge
...
\item If, after passing to a subsequence,
\[
  \limsup_{n\to\infty}\|S_n\|_{L^\infty(I;L^3(B_R))}>0,
\]
then a selected subwindow and subcylinder carry a nonzero local occupation
state.  Its first closed label in the local state space is one of the named
alternatives in \(\cref{paper6:thm:state-decomposition}\): multibubble or
multicore splitting, same-point cascade, exterior leakage or critical
tightness failure, loss of local windowed \(H^1\) control, finite-cost
scale-collapse, scale-rigid residual behavior, or a representation,
modulation, pressure, or cost-compatibility failure recorded earlier in the
ordered validation.
\item If all named adjacent alternatives are closed by their local arguments
and comparable retained classes have been grouped before selecting the
terminal frame, then no nonretained residual with the preceding positive local
mass can remain on the retained terminal branch.  Consequently
\[
  S_n\to0
...
Kato estimate is part of this assumption.
\end{assumption}
...
In the local topology of
\cref{paper6a:lem:l3-profile-extraction-cited}, the ordered alternatives in
\cref{paper6:thm:state-decomposition} are closed under subsequential limits of
selected compact terminal windows.
\end{lemma}

\begin{proof}
Each label is expressed by a local compact-cylinder condition.  Multibubble and
multicore splitting are detected by positive CKN mass in two separated or
scale-separated subcylinders.  Cascade and scale-rigid labels are closed under
the corresponding scale-center limits.  Loss of local windowed \(H^1\) control
is the failure of a uniform compact-cylinder bound.  Finite-cost scale collapse
and negative-drift labels are determined by lower semicontinuous nonnegative
localized costs.  Representation, modulation, pressure, and cost-compatibility
failures are ordered failures of the local hypotheses used to enter the
retained compact branch.  These conditions are stable under the compact local
convergence above, after passing to subsequences and shrinking cylinders when
needed.
\end{proof}
...
\(\|S_n(\tau_n)\|_{L^3(B_R)}\ge\delta\).  By the local energy inequality,
pressure reconstruction, and compact-cylinder estimates, the selected
subwindows are admissible for the local alternative decomposition.  Apply
\cref{paper6:thm:state-decomposition} to the selected local concentration
sequence generated by this residual mass.  The ordered first failure or
retained local state is closed by
\cref{paper6a:lem:critical-mass-decoupling}; hence, after passing to a
subsequence, the residual has one of the named labels in
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(ii)}.
\end{proof}
...
retained classes and that every adjacent label in
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(ii)} is closed by
its local argument.  Then a nonretained residual satisfying the hypotheses of
...
label.  If this label is an adjacent alternative, it is handled by its local
closure theorem and is not part of the retained branch.  If the label is the
retained compact class, then the corresponding component is comparable to the
selected retained terminal frame; by the terminal selection rule, comparable
retained classes based at the same physical point were already grouped into
\(U_n\).  It therefore cannot remain in \(S_n\).  Both cases contradict the
...
\Cref{paper6a:ass:critical-ns-profile-decomposition} holds on every retained
terminal branch satisfying the compact-cylinder estimates, the ordered local
state-space decomposition, closure of adjacent alternatives, and terminal
grouping of comparable retained classes.
\end{theorem}
...
identifies the closed local labels.  \Cref{paper6a:lem:heat-flow-kato-smallness}
...
such a label inside the retained branch once adjacent labels have been closed
and comparable retained classes have been grouped.  Hence
\cref{paper6a:lem:profile-exhaustion-by-diagonal-extraction} gives
\(S_n\to0\) on compact terminal cylinders.  These are exactly the clauses of
\cref{paper6a:ass:critical-ns-profile-decomposition}.
\end{proof}

\begin{remark}[Status of the terminal completeness assumption]
...
the pressure normalized modulo functions of time.
\end{proof}
...
\begin{definition}[Terminal local compactness hypotheses]
\label{paper6a:def:terminal-critical-profile-hypotheses}
The terminal local compactness hypotheses consist of the local compact
terminal-window hypothesis
\cref{paper6a:def:bounded-critical-terminal-sequences}, local perturbative
stability
\cref{paper6a:def:small-data-critical-stability}, and local terminal
state-space completeness
\cref{paper6a:ass:critical-ns-profile-decomposition}.
\end{definition}
...
Assume the terminal local compactness hypotheses.  Then, on every retained
terminal branch and every compact terminal cylinder, all nonretained residual
components vanish in the local perturbative topology of
\cref{paper8:def:terminal-nonlinear-decoupling}.
\end{theorem}

\begin{proof}
The local compact terminal-window hypothesis gives the compact-cylinder
estimates.  Local terminal state-space completeness gives
\[
  S_n\to0\quad\text{in }L^\infty(I;L^3(B_R))
\]
after all adjacent local alternatives have been closed and comparable retained
classes have been grouped.  Local perturbative stability then converts this
velocity convergence into the nonlinear-stress, pressure, and residual
...
The terminal profile completeness hypothesis in the final argument may be
replaced by the terminal local compactness hypotheses of
...
cylinders, bounded modulation, local perturbative residual stability, and
closure of the adjacent alternatives in the local state-space decomposition.
Then the terminal profile completeness hypothesis in the final argument holds.
\end{corollary}
...
\cref{paper6a:cor:kato-gives-small-data-stability}.  The local state-space
closure hypotheses give \cref{paper6a:ass:critical-ns-profile-decomposition}
through \cref{paper6a:thm:critical-ns-profile-decomposition-discharged}.
Therefore the terminal local compactness hypotheses of
...
cost is compatible with the cost-divergence theorem; multibubble and
...
Caccioppoli estimate; and every divergent compatible localized cost on the
canonical windowwise schedule is handled by
\cref{paper6a:thm:cost-divergence-exclusion-discharged}.
\end{remark}
...
adjacent-stratum closure for the compact-cost regime in the sense of
...
cost-divergence argument or by the treated adjacent-stratum alternatives is either
...
alternatives, the first alternative is unavailable.  Therefore the
...
regularity problem; it is a precise local Type II exclusion theorem whose
