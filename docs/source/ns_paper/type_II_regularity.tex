...
A profile \(V\) belongs to \(\mathcal A^{\rm tr,C^1}_{p,R}\) if there exists
\(\delta_0>0\) such that the following maps are finite and continuous for
\[
...
\end{aligned}
\]
The analogous continuity requirements are imposed on the remaining moment
functionals used to fix the translation and orientation gauges.
\end{definition}
...
space, the interior harmonic estimates imply compactness of \(H_n\) in
\(L^{3/2}_tC^m_x(B_{2r}(x_0))\) for each fixed finite \(m\), after passing to a
subsequence.  Hence
\[
   H_n\to H_*
   \qquad\text{strongly in }L^{3/2}_{\mathrm{loc}}(Q_{2r}(z_0)).
\]
Set \(P_*:=P_*^{\mathrm{loc}}+H_*\).  Passing to the limit in the represented equation
...
on the same subcylinders.  Therefore the scaled CKN quantities vanish
uniformly on compact subcylinders as \(\rho\downarrow0\), and the strong
convergence of the local pressure pieces has already been proved above.
\end{proof}
...
cylinders and pass to the pressure-defect compactification.  The realized
support-profile transfer theorem gives a nonzero diffuse pressure-only defect.
This is alternative \textup{(v)}.  The alternatives exhaust all infinite nested
...
\begin{proof}
Let \(\pi\) be the normalized diffuse pressure-defect measure.  By realized
support-profile transfer, every nonzero support profile of \(\pi\) is realized
from the original pressure-only sequence.
...
\]
Use the logarithmic shell compactification of the tail together with realized
support-profile transfer.  After passing to a subsequence, \(\nu_n\) converges
to a probability measure on the compactified tail state space.  Since no
support profile carries positive local retained velocity activity, the
normalized defect is a diffuse critical-tail defect, giving item
\textup{(vi)}.
...
nonzero.  Apply realized support-profile transfer to the logarithmic tail
compactification.

If some normalized support profile of \(\nu\) has positive retained local CKN
activity, then support-profile heredity realizes this activity as a retained
local carrier in the original sequence.  This is one of the carrier rows in
...
extraction theorem applies to this realized defect.  The Young-variance,
strong-compactness, and coherent critical-tail closure theorems then imply that
the defect either generates a retained coherent critical-tail carrier or falls
into the affine, active, pressure, or hidden-scale rows.  All of these are
lower named rows already closed in the terminal ledger.
...
selected cylinder with the previous selected core.  There are only finitely many
geometric possibilities after passing to subsequences.  If the new center is
separated from the old center at a comparable physical scale, the branch is a
separated exterior or multibubble row.  If the new center remains at the same
physical point and the new scale is strictly smaller, the process either
terminates after finitely many steps or generates a same-point scale cascade.
If the new scale is strictly larger in the renormalized variables but still
collapses physically to the same point, the process gives the same-point
larger-scale cascade row.  If the relative centers or relative scales fail to
have a compact limit, the process gives the scale-collapse or noncompact
exterior row.

Finally, if the CKN mass persists but no subsequence produces a fixed local
carrier, then the normalized remaining mass is diffuse.  By the tail-support
compactification and \cref{paper9:lem:diffuse-critical-tail-row-closed}, this
is the diffuse critical-tail row, already named and closed.  Thus infinite
reselection is not an unclassified possibility: it is exactly one of the named
infinite rows.
\end{proof}
