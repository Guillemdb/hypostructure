...
concentration, retained active-core loss, and noncanonical carrier/cost failure
...
\]
and there exists \(\delta_0>0\) such that the following maps are finite and
continuous for
\[
...
\[
   ...
   \qquad j=1,2,3,
\]
and require finiteness and continuity of
\[
...
\[
   ...
   |V(c+\lambda Y)|^2\,dY,
\]
for \(j,k\in\{1,2,3\}\).  These are the derivative maps used by the
translation and orientation gauge conditions.
\end{definition}
...
\(V\in\mathcal A^{\rm tr,C^1}_{p,R}\).  Define
\[
   \mathcal J(c,\lambda;V)
   :=
   \int_{\RR^3}\chi_R(Y)|Y|^{-p}|V(c+\lambda Y)|^3\,dY
\]
for \(|c|+|\lambda-1|<\delta_0\).  Then \((c,\lambda)\mapsto\mathcal J(c,\lambda;V)\)
is \(C^1\) on this neighborhood.  In particular,
\[
\partial_{c_k}\mathcal J(c,\lambda;V)
=
\mathcal J_{c_k}(c,\lambda;V),
\qquad
\partial_{\lambda}\mathcal J(c,\lambda;V)
=
\mathcal J_{\lambda}(c,\lambda;V).
\]
At \((c,\lambda)=(0,1)\),
\[
\partial_{c_k}\mathcal J(0,1;V)
=
...
\]
\[
\partial_\lambda \mathcal J(0,1;V)
=
-\int \left[(3-p)\chi_R(Y)+Y\cdot\nabla\chi_R(Y)\right]
|Y|^{-p}|V(Y)|^3\,dY.
\]
\end{lemma}

\begin{proof}
First assume \(V\) is smooth.  Differentiating under the integral sign and
integrating by parts in the \(Y\)-variable gives the displayed formulas.  The
factor \(\lambda^{-1}\) appears because
\[
   \partial_{Y_k}|V(c+\lambda Y)|^3
   =
   \lambda\,\partial_{x_k}|V(c+\lambda Y)|^3 .
\]
Thus
\[
   \partial_{c_k}|V(c+\lambda Y)|^3
   =
   \lambda^{-1}\partial_{Y_k}|V(c+\lambda Y)|^3 .
\]
Similarly,
\[
   \partial_\lambda |V(c+\lambda Y)|^3
   =
   Y\cdot\nabla_x |V(c+\lambda Y)|^3
   =
   \lambda^{-1}Y\cdot\nabla_Y |V(c+\lambda Y)|^3 .
\]
Integrating by parts gives the formulas for
\(\mathcal J_{c_k}\) and \(\mathcal J_\lambda\).

For general \(V\in\mathcal A^{\rm tr,C^1}_{p,R}\), mollify \(V\) in space.
The smooth formulas hold for the mollified profiles.  The defining continuity
and finiteness properties of \(\mathcal A^{\rm tr,C^1}_{p,R}\) allow the
mollification parameter to be sent to zero in the functionals and in their
displayed derivative expressions.  Thus the displayed derivatives exist for
\(V\), and the derivative maps are continuous by definition of the strong chart
class.
\end{proof}
...
\partial_{c_k}V_{\lambda,c}=\lambda\,\partial_kV(c+\lambda Y).
\]
...
scale entry satisfies the signed transversality bound
\[
\alpha(\tau)=
...
\(Q_R\).  Moreover, \(V_*\) and \(P_*\) are smooth on every
\(Q_{R''}\Subset Q_R\).  Consequently, for every \(Q_{R''}\Subset Q_R\),
\[
   \lim_{\rho\downarrow0}
...
Since \(V_*\in L^\infty(Q_R)\), local Serrin regularity for bounded suitable
solutions gives smoothness of \(V_*\) on every \(Q_{R''}\Subset Q_R\).  The
pressure relation then gives smoothness of \(P_*\), modulo functions of time,
on the same subcylinders.  Therefore the scaled CKN velocity quantity vanishes
uniformly on compact subcylinders as \(\rho\downarrow0\).  For the pressure,
fix \(Q_{R''}\Subset Q_{2r}(z_0)\).  Since \(P_*\) is smooth,
\[
   \rho^{-2}\iint_{Q_\rho(z)}
   |P_*-(P_*)_{B_\rho(x_z)}(t)|^{3/2}
   \longrightarrow 0
\]
uniformly for \(Q_\rho(z)\Subset Q_{R''}\) as \(\rho\downarrow0\).
...
limit has a smooth pressure representative on smaller cylinders, and its CKN
velocity--pressure quantity is uniformly small at sufficiently small scales.
...
subsequence would lose the retained active mass.  This is exactly the
...
such that
\[
  \sup_n\|V_n\|_{L^\infty(J;L^3(B_\rho))}\le C_{\rho,J}.
\]
...
If the retained active lower floor fails, the selected candidate is not the
...
rebased velocity/pressure tuple on a fixed model cylinder, and the harmonic jets
of the rebased harmonic remainder on a slightly smaller model cylinder.  The
closure of this family in the product topology is a compact metrizable
...
occupation-state space obtained from the compact-cylinder suitable bounds and
the local pressure atlas on the rebased model cylinder, and
...
the compact occupation-state space furnished by the usual local compact-cylinder
bounds, while the rebased harmonic remainders have compact jet coordinates by
interior harmonic estimates.  Together with the compactified relative
scale-center chart, this gives the compact metrizable state space
\(\mathfrak X_{\rm pres}\).
...
\item a pressure-atlas, gauge, or pressure-reconstruction failure.
\end{enumerate}
\end{theorem}
...
which admissible pressure reconstruction, gauge normalization, or the pressure
atlas breaks down, then by definition we are in alternative \textup{(iv)}.
...
If the limiting Calder\'on--Zygmund component is nonzero, its source is a
nontrivial local velocity stress, which is alternative \textup{(i)}.  If the
...
alternative \textup{(ii)}, while an affine or pure-gauge harmonic profile is
alternative \textup{(iii)}.  Because \(\mathfrak p\) belongs to the support of a
normalized nonzero defect measure, the realized profile cannot vanish
identically on the model cylinder.  Thus one and only one of
\textup{(i)}--\textup{(iv)} occurs.
\end{proof}
...
Finally, if the pressure lower bound persists but no support profile carries a
positive local pressure atom, normalize the pressure measures on the nested
cylinders.  By
\cref{paper8:thm:pressure-defect-compactification-support-transfer}, the
normalized defects have a nonzero realized diffuse pressure-only limit, giving
alternative \textup{(v)}.  The alternatives exhaust all infinite nested
pressure-only selections.
\end{proof}
...
\cref{paper8:thm:pressure-defect-compactification-support-transfer}, every
nonzero support profile of \(\pi\) is realized from the original pressure-only
sequence and satisfies one of the four alternatives listed there.

If a support profile has nonzero local Calder\'on--Zygmund source, then the
source is generated by a nontrivial velocity stress.  The fixed-annular stress
dichotomy produces a velocity carrier, contradicting pressure-only terminality.

If the support profile is harmonic and non-affine after quotienting by
time-dependent constants and affine gauges, then the far-field harmonic-tail
row occurs.

If the harmonic profile is affine or pure gauge, then it is removed by the
pressure gauge and cannot carry terminal pressure mass.

If the pressure gauge, pressure atlas, or compactness needed to make one of
these conclusions fails, the branch is exactly the pressure-gauge or
pressure-reconstruction failure row.  Thus no diffuse pressure-only defect is
terminal.
\end{proof}
...
bounded modulation are the other local inputs used in
...
\cref{paper6a:def:critical-ns-profile-decomposition-local}\textup{(i)} on
...
\cref{paper8:thm:exterior-stratification}.  Local \(H^1\)-loss is recorded as
the rough-core row of \cref{paper6:thm:state-decomposition}.  Multibubble,
multicore, cascade, and gauge-degenerate rows are precisely the non-single-core
structural rows of \cref{paper6:thm:state-decomposition}.  Finite-cost
scale-collapse is recorded as the finite-cost scale-collapse row, and
scale-rigid residual labels are recorded as the scale-rigid row governed by
...
\[
  (V_n,P_n,U_n,S_n,a_n,b_n)
\]
on a selected compact subcylinder.  For any other residual diagnostic covered
by the statement, visibility in the same augmented coordinates is part of the
hypothesis.  By
...
mass, then \cref{paper6a:thm:residual-mass-admissible-state} shows that the
same positive defect is visible in the full represented branch and hence is
again assigned to an adjacent named row unless it is carried by a retained
comparable class.  By \cref{paper6a:lem:retained-frame-maximality}, every such
retained comparable class was already grouped into the selected retained
component before the terminal frame was fixed.
...
Fix a canonical dyadic cover of the unit shell by compact model cylinders.  For
each occupied shell
\[
  A_{n,k}=\{2^kR_n<|y|\le 2^{k+1}R_n\}
\]
record its logarithmic index, the compactified relative center-scale data of the
chosen model cylinder inside the unit shell, the rebased local velocity/pressure
state on that model cylinder, and the boundary faces where representation,
pressure-atlas, gauge, local-energy, or rough-core admissibility fails.  The
closure of this family is a compact metrizable shell-state space
\[
...
\]
After passing to a subsequence, the normalized shell measures induce a weak-*
probability limit on \(\mathfrak X_{\rm shell}\).  Every support point is
realized by an actual sequence of occupied shells and corresponding rebased
unit-shell extractions from the original represented branch.
...
inequality, and compact-cylinder bounds on a fixed unit-shell model cylinder;
...
\begin{proof}
Translate each occupied shell to its shell center and dilate by its shell radius
\(2^kR_n\).  Translation, Navier--Stokes scaling, and subtraction of the
appropriate pressure mean preserve the represented equation, divergence-free
condition, local energy inequality, and local pressure reconstruction on any
fixed compact unit-shell cylinder.  Thus every rebased shell either remains in a
uniform local compactness class on the chosen model cylinders or hits one of the
explicit failure faces recorded in \(\mathfrak F_{\rm shell}\).  The logarithmic
shell index belongs to the one-point compactification of \(\mathbb N\), the
relative center-scale chart is compactified exactly as in the local state-space
package, and the rebased local-state factor is compact by the corresponding
local compact-cylinder bounds.  This gives the compact metrizable space
\(\mathfrak X_{\rm shell}\).

Push forward the normalized shell measures to \(\mathfrak X_{\rm shell}\) and
...
\(m\) we may select an actual occupied shell and a rebased model-cylinder state
lying in \(U_m\).  These genuine rebased shell states converge to
...
\[
  \nu_n
  :=
  M_n^{-1}|V(y,\tau_n)|^3\mathbf 1_{\{|y|>R_n\}}\,dy
\]
be the normalized tail measures associated to a critical-tightness failure
sequence, where \(M_n\ge\varepsilon_0>0\) and \(R_n\to\infty\).  After passing
to a subsequence, the logarithmic shell compactification of the tail carries a
probability limit state.  Every point in the support of that limit is realized
by a rebased unit-shell extraction of the original represented branch, and each
realized support profile has a first closed local row in the ordered local
...
rebased unit-shell realizing sequence for every support point.
...
unit-shell model cylinder.  If \(\mathfrak z\) lies on one of the failure faces
...
retained local velocity activity at any fixed level.  Then, after passing to a
subsequence, there exists a realized diffuse critical-tail state whose first
closed local row is minimal among the support profiles.  This first row is one
...
\cref{paper9:thm:logarithmic-tail-support-transfer} and choose a support
profile whose first closed row is minimal in that finite ordered list.  By the
assumption that no support profile carries positive compact retained local
velocity activity, the carrier rows of
\cref{paper9:lem:critical-tightness-failure-row} do not occur for this minimal
profile.  Therefore its first row is one of the remaining named local rows:
pressure/gauge/reconstruction failure, rough-core or local-energy failure,
affine/pure-gauge behavior, hidden-scale/scale-collapse behavior, or the
diffuse coherent critical-tail row.  This realized profile is the required
minimal mesoscopic extraction.
\end{proof}
...
spacetime \(L^3\) floor is lost, then the branch enters the loss-of-active-core / pressure-only
...
cylinder, and assume it lies away from the pressure/gauge/local-energy failure
faces.  Suppose moreover that case \textup{(ii)} of
\cref{paper9:thm:young-variance-strong-compactness} holds on a compact
subcylinder \(Q_{2R,I}\), so that after passing to a subsequence
\[
  W_n\to W_\infty
  \quad\text{strongly in }L^3(Q_{2R,I}).
\]
Set
\[
  U_n:=W_\infty,
  ...
  S_n:=W_n-W_\infty.
\]
Then on every smaller compact subcylinder the augmented tuple
\[
...
Any positive nonretained local \(L^3\), pressure, or compactness defect of the
shell sequence is therefore visible either as a residual coordinate/selected-time
residual concentration measure in that augmented state space, or as an already
named pressure/gauge/rough-core failure row.
\end{lemma}

\begin{proof}
By \cref{paper9:thm:logarithmic-tail-support-transfer}, the realized shell
sequence is a represented local sequence on the fixed model cylinder: it
satisfies the repaired-gauge equation, the local energy inequality, pressure
reconstruction modulo functions of time, and the compact-cylinder bounds.  Since
\(U_n\equiv W_\infty\) is constant in \(n\) and
\(S_n=W_n-W_\infty\to0\) strongly in local \(L^3\), the same compact-cylinder
estimates place \((W_n,U_n,S_n,a_n,b_n)\) in the augmented local topology of
\cref{paper6a:lem:l3-profile-extraction-cited} on every smaller compact
subcylinder.
...
compact cylinder.  By
\cref{paper5:lem:selected-compactness-aubin-lions}, either
\[
...
the same augmented local state space.

Consequently, if a positive nonretained local diagnostic remains after passing
to the strong local limit, then on a smaller compact subcylinder it is either a
nonzero residual coordinate/selected-time concentration measure for \(S_n\), or
one of the already named pressure/gauge/rough-core failure faces.  This is the
required compatibility with
\cref{paper6a:thm:local-residual-exhaustion}.
\end{proof}

\begin{lemma}[Strong diffuse-tail limits create a named local row]
\label{paper9:lem:strong-diffuse-tail-limit-row}
Let \((W_n,\widetilde P_n)\) be a realized diffuse critical-tail extraction on a
rebased unit-shell model cylinder, and suppose case \textup{(ii)} of
\cref{paper9:thm:young-variance-strong-compactness} holds on a compact
subcylinder \(Q_{2R}\).  Let
\[
  W_n\to W_\infty
  \quad\text{strongly in }L^3(Q_{2R}).
\]
Assume that the pressure/gauge/local-energy structure required to test the local
CKN and bounded-window rows has not already failed.  If
\(W_\infty\not\equiv0\), then after shrinking to a smaller compact subwindow one
...
\item \(W_\infty\) is not locally bounded on any smaller compact subwindow.
Then the contrapositive of
...
subcylinder of \(Q_{2R}\) on which \(W_\infty\) has positive \(L^3\) mass.  If
\(W_\infty\) is bounded on a slightly smaller compact subwindow, then
...
Assume next that \(W_\infty\) is not locally bounded on any such smaller compact
subwindow.  If every sufficiently small compact subcylinder had CKN quantity
below the threshold of \cref{paper8:thm:terminal-ckn-regularity}, then that
theorem would imply local boundedness of \(W_\infty\), contrary to assumption.
Hence there is a compact subcylinder on which the local CKN quantity of the
limit is positive.  By the strong local \(L^3\) convergence and the pressure
compactness conclusion of \cref{paper5:lem:selected-compactness-aubin-lions}, the
same positive CKN quantity transfers to \((W_n,\widetilde P_n)\) for all large
\(n\), unless pressure compactness fails first; such a pressure failure is
alternative \textup{(iii)}.  In the non-failure case,
...
Finally, if during these local tests the pressure atlas, repaired gauge, or
local-energy structure breaks, then by definition the first failed row is one of
the named local failure rows.  This is \textup{(iii)}.
\end{proof}
...
that subwindow if necessary, either a named local row already appears, or the
limit is bounded-window regularizable and any positive activity left outside the
regularized region is one of the named remaining rows in
\cref{paper6:lem:bounded-window-local-regularization}.
...
\begin{lemma}[Failure of critical tightness is a named local row]
...
variables, with scale function \(\lambda(\tau)\).  Suppose critical tightness
fails: there exist \(\varepsilon_0>0\), times \(\tau_n\to+\infty\), and radii
...
\item a rough-core or time-slice-to-activity failure row;
...
Set
\[
   M_n:=\int_{|y|>R_n}|V(y,\tau_n)|^3\,dy.
\]
After passing to a subsequence, assume \(M_n\ge\varepsilon_0\).  Decompose the
tail into dyadic shells
\[
   A_{n,k}
   :=
   \{2^kR_n<|y|\le 2^{k+1}R_n\},
   \qquad k\ge0.
\]
Cover each occupied shell \(A_{n,k}\) by finitely overlapping unit balls in the
\(y\)-variables.  Suppose first that for some \(\eta>0\) there are shells
\(A_{n,k_n}\) and unit balls \(B_1(y_n)\subset A_{n,k_n}\) such that
\[
   \int_{B_1(y_n)} |V(y,\tau_n)|^3\,dy\ge \eta .
\]
By the local time-slice-to-activity conversion, this produces a parabolic
retained carrier unless that conversion fails.  If it fails, the branch is in
the rough-core or local-energy/pressure failure row, giving item
\textup{(v)} or \textup{(iv)}.

Assume therefore that a parabolic carrier is produced.  Let
\[
   d_n:=\lambda(\tau_n)|y_n|
\]
be the physical distance of the carrier from the selected core.  Passing to a
subsequence, exactly one of the following occurs.  If \(d_n\to0\), the carrier
remains at the same physical point but at a larger renormalized radius, giving
item \textup{(i)}.  If \(d_n\to d\in(0,\infty)\), the carrier is separated by a
positive physical distance from the selected core, giving item \textup{(ii)}.
If \(d_n\to\infty\), the carrier escapes every compact physical neighborhood in
the selected chart, giving item \textup{(iii)}.

The only remaining possibility is that no positive local carrier exists at any
fixed level.  In that case the tail mass is diffuse.  Define the normalized
tail measures
\[
   \nu_n
   :=
   M_n^{-1}|V(y,\tau_n)|^3\mathbf 1_{\{|y|>R_n\}}\,dy .
\]
By \cref{paper9:thm:logarithmic-tail-support-transfer}, after passing to a
subsequence the measures \(\nu_n\) define a probability limit state on the
compactified logarithmic shell space, and every support profile is realized by
rebased unit-shell extractions of the original branch.  Since we are in the
case where no fixed-level local carrier exists, none of those support profiles
carries positive compact retained velocity activity.  Hence the normalized
defect is a diffuse critical-tail defect, giving item \textup{(vi)}.

If the pressure atlas, gauge normalization, or represented equation needed in
this extraction fails, then the first failed test is one of the named
pressure/gauge/reconstruction rows, giving item \textup{(iv)}.  This exhausts
all possibilities.
\end{proof}
...
nonzero.  Apply
\cref{paper9:thm:logarithmic-tail-support-transfer} to the logarithmic tail
compactification.

If some normalized support profile of \(\nu\) has positive retained local CKN
activity, then the support-transfer theorem realizes this activity as a
retained local carrier in the original sequence.  This is one of the carrier
rows in
\cref{paper9:lem:critical-tightness-failure-row}, contrary to the diffuse case.

If a support profile loses the pressure atlas, gauge normalization, compact
upper estimates, or local-energy structure, then the first failed test is the
corresponding pressure/gauge/rough-core row, already named in the ledger.

We are therefore in the case where every nonzero support profile is a realized
diffuse critical-tail profile with the pressure/gauge/local-energy structure
intact and with no retained compact carrier.  The minimal mesoscopic
extraction theorem
\cref{paper9:thm:minimal-mesoscopic-extraction} applies to this realized
defect.  The local strong-compactness theorem
\cref{paper9:thm:young-variance-strong-compactness} and the coherent-tail
closure theorem \cref{paper9:thm:coherent-critical-tail-closure} then imply
that the defect falls into one of the affine, active, pressure, hidden-scale,
or other already named lower rows.
...
compact-cylinder critical bound
\[
  \sup_{\tau\in I_n^*}\|V(\tau)\|_{L^3(B_{4R})}\le M_{3,4R},
\]
...
\[
  \int_{I_n}\int_{B_R}|\nabla V|^2\,dy\,d\tau
  \le
  C_R\bigl(1+A_{n,R}\bigr)
  \bigl(1+M_{3,4R}^3\bigr),
\]
...
equal to one on \(I_n\times B_R\).  The velocity terms are bounded by the
finite-measure embeddings
\[
  \|V(\tau)\|_{L^2(B_{2R})}^2\le C_R\|V(\tau)\|_{L^3(B_{2R})}^2,
  \qquad
  \|V(\tau)\|_{L^3(B_{2R})}^3\le M_{3,4R}^3 .
\]
The pressure term is controlled, after subtracting spatial means, by the local
pressure reconstruction and the same critical bound:
\[
  \|P(\tau)-c_{4R}(\tau)\|_{L^{3/2}(B_{2R})}
  \le C_RM_{3,4R}^2 .
\]
...
\]
up to cutoff constants; the local \(L^3\) bound controls the inner integral by
\(C_RM_{3,4R}^2\).  Combining these estimates gives the displayed bound.  No
pointwise-in-time modulation bound is used.
\end{proof}
...
bounds, and the local Caccioppoli estimate on the compact cylinders used in
...
\[
  \sup_n A_{n,R}<\infty .
\]
The compact-cylinder critical bound and local pressure reconstruction are
retained-stratum inputs.  Applying
...
critical control, local Caccioppoli regularity, or the uniform integrated
modulation-window bound.  By the ordered tests
...
rough-core rows are closed by the corresponding first-failure theorems.  The
...
Then either the loss-of-retained-active-core row occurs, or, after passing to a
...
lower active mass floor fails, the selected candidate is not a retained active
component; this is exactly the loss-of-retained-active-core row.
\par
If the lower active mass floor does not fail, then
...
windows.  Apply
\cref{paper6a:lem:admissible-drift-window-active-core}.  If the active-core
floor is lost, the branch is in \textup{(T1)}.  Otherwise a fixed compact
subcylinder carries a positive retained active local mass floor, so the
...
occupation blocks, the retained active-core floor, and the ordered local
...
The expanded terminal data imply uniform critical tightness.
\end{lemma}
...
By \cref{paper3:lem:bounded-window-vmo-pressure}, the bounded selected-window
limit has a smooth pressure representative on smaller cylinders, and its
velocity--pressure CKN quantity is uniformly small at sufficiently small
scales.  The strong convergence of \(V_n\), the strong convergence of the local
Calder\'on--Zygmund pressure pieces, and harmonic pressure decay therefore
...
cylinder and the usual failure faces for representation, pressure, gauge, and
rough-core breakdown.  After passing to a subsequence in the generation index,
...
local activity on some fixed model cylinder, then undoing the rebasing gives a
fixed local carrier on the original reselection branch.
\end{lemma}

\begin{proof}
The compactified relative scale-center chart is compact by construction, and the
rebased local-state factor is compact by the same local occupation-state
compactness used throughout the bounded-window analysis once one records failure
faces for loss of represented structure.  Hence the normalized tree measures are
tight and possess weak-* limits.
...
same lower bound for all large \(m\).  Undoing the rebasing sends those
descendants back to physical subcylinders of the original bounded window, and
these subcylinders form a fixed local carrier on the reselection branch.
\end{proof}
...
scale-collapse row, noncompact exterior row, or diffuse critical-tail row.
\end{enumerate}
\end{lemma}
...
Finally, if the CKN mass persists but no subsequence produces a fixed local
carrier, normalize the remaining CKN mass on the reselection tree at the first
scale on which each descendant cylinder is selected.  Apply
\cref{paper10:lem:reselection-tree-support-realization}.  It gives a weak-* limit
state on a compact reselection space and realizes every support point by an
actual descendant sequence with the corresponding rebased local PDE data.

If some support point carried positive compact local activity, then the same
lemma would convert that support point into a fixed local carrier on the
original reselection branch, contrary to the case under consideration.  Hence
every realized support profile is diffuse.  Since these support profiles are
genuine rebased local states, \cref{paper9:lem:diffuse-critical-tail-row-closed}
applies and identifies the limit as the diffuse critical-tail row, already named
and closed.  Thus infinite reselection is not an unclassified possibility: it
is exactly one of the named infinite rows.
\end{proof}
...
one of the alternatives listed in \cref{paper6:thm:state-decomposition}.
\end{theorem}
...
local pressure regularity of the bounded limit, and
...
scale-collapse, noncompact exterior, or diffuse critical-tail rows.
\par
...
\cref{paper9:lem:critical-tightness-failure-row}.  The diffuse case is closed
by \cref{paper9:lem:diffuse-critical-tail-row-closed}.  When pressure-only
exterior selection nests indefinitely, apply
\cref{paper8:lem:nested-pressure-only-well-founded}; the diffuse pressure case
is closed by \cref{paper8:lem:diffuse-pressure-only-not-terminal}.
\par
...
retained terminal state.  The retained terminal states are precisely those
listed in \cref{paper6:thm:state-decomposition}.
\end{proof}
...
\item the first failed test is one of the named cost-side adjacent first
failures listed in
\cref{paper7:lem:no-unrecorded-cost-escape}\textup{(b)}: cost-compatibility
failure, carrier-subidentity failure, finite fixed-cutoff error failure,
summable moving-annulus error failure, carrier-comparison failure for the
validated cost, modulation-integrability failure, or failure of compact cost
divergence for the validated cost; each of these first failures is a closed
adjacent row, routed in the ambient adjacent wrapper
\cref{paper7:cor:ambient-adjacent-wrapper-discharged}.
\end{enumerate}
...
\smallskip\noindent\emph{Cost-side first failures (the irreducible
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item the lower active mass-floor failure, in which case the candidate is
...
\cref{paper6a:lem:terminal-local-critical-mass-routing}.  A lower-mass failure
means the candidate was not a retained active core; an upper local
