...
\(L^3(B_R\times(-r^2,0))\), the Calder\'on--Zygmund estimate and harmonic
...
\begin{lemma}[Nonzero constant exits the retained Type II row]\label{paper1:lem:nonzero-contradiction}
...
assigned to the bounded-window alternative rather than the retained local Type
II row.
\end{proof}
...
branch exits the retained local Type II row through the bounded-window
alternative.  The two cases exhaust all
...
\(F(\cdot,s)\in L^1_{\rm loc}(I_*;\mathbb R^4)\) by (i), hence \(F\) is a Carath\'eodory
...
Uniqueness follows from Gr\"onwall on the difference of two AC solutions on \(B_{r_0}(s_0)\),
...
Choose
    \[
    \psi(x,t):=\Lambda(\tau(t))^{-1}\phi\!\left(\frac{x-X(\tau(t))}{\Lambda(\tau(t))},\tau(t)\right)^2,\qquad
...
\item \textbf{Scale-rigid branch:}
  ...
  \cref{paper3:prop:local-scale-rigidity-exclusion}, so \cref{paper3:prop:local-scale-rigidity-exclusion} excludes it;
...
\cref{paper3:prop:local-single-core-exclusion}, the scale-rigidity exclusion
\cref{paper3:prop:local-scale-rigidity-exclusion}, and the terminal admissibility in
...
scale-rigidity alternative in \cref{paper3:prop:local-scale-rigidity-exclusion}; if the degeneracy
...
scale-rigid alternative is excluded by \cref{paper3:prop:local-scale-rigidity-exclusion}, which includes
...
\begin{proposition}[Selected-window suitable estimates]\label{paper5:prop:selected-window-suitable-estimates}
...
\[
  ...
  s\in(0,T).
\]
Assume that the loss-of-retained-active-core row does not occur on these
selected windows.  Then, after passing to a subsequence, for every bounded Lipschitz domain \(K\Subset\mathbb R^3\), there is a constant
...
\begin{proof}
The localized lower bound is given by the retained-window branch of
\cref{paper5:thm:core-retention} below, after excluding the
loss-of-retained-active-core row on the chosen windows.  With that lower bound in hand,
...
bound stated in Proposition~\ref{paper5:prop:selected-window-suitable-estimates}.  Then
Proposition~\ref{paper5:prop:selected-window-suitable-estimates} holds.
\end{proposition}
...
Proposition~\ref{paper5:prop:selected-window-suitable-estimates}.  The retained selected-core lower bound is the
last condition in that proposition.  Hence all items in
Proposition~\ref{paper5:prop:selected-window-suitable-estimates} follow from the representation,
...
\[
  ...
  \|\chi^{1/2}(V_n+U)\|_{L^2(0,T;L^2)}.
\]
Since \(K_\chi\) is compact, Proposition~\ref{paper5:prop:selected-window-suitable-estimates} and
...
Proposition~\ref{paper5:prop:selected-window-suitable-estimates} gives for large \(n\),
...
\begin{theorem}[Windowwise retained-core alternative]\label{paper5:thm:core-retention}
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item there exist \(R>0\), a cutoff \(\chi\in C_c^\infty(B_R)\), \(\chi\ge0\), and
  \(\eta>0\) such that
  \[
    \int_{\mathbb R^3}|V_n(y,s)|^2\chi(y)\,dy\ge\eta
    \quad\text{for a.e. }s\in(0,T)
...
selected windows, the translated sequence satisfies the localized nontriviality
condition in Proposition~\ref{paper5:prop:selected-window-suitable-estimates}.
\end{theorem}

\begin{proof}
The selected-core construction marks, on each admissible selected window, an inner compact
renormalized core carrying positive localized kinetic mass on the time set used in the
window analysis.  If no subsequence admitted a uniform lower floor of the displayed form
with one fixed inner cutoff, then after shrinking once inside the candidate core every
further selected-window subsequence would lose that retained active mass; this is exactly
the loss-of-retained-active-core row in the local first-failure ledger.  Therefore, once
that row is excluded, a subsequence exists on which one compact inner cutoff and one
positive threshold work uniformly on all remaining windows, yielding alternative
\textup{(ii)}.  The final assertion is just a restatement of \textup{(ii)}.
\end{proof}
...
On thick windows, either the local estimate package holds and the retained-window
branch of the selected-core alternative is available, in which case
Proposition~\ref{paper5:prop:selected-window-derived} together with the retained-window branch of
Theorem~\ref{paper5:thm:core-retention} give
Proposition~\ref{paper5:prop:selected-window-suitable-estimates}; if the local estimate package or the
retained-core alternative fails on the selected windows, the branch lies in the compactness
alternative.  If
...
\]
\item the retained selected core is active on an inner cylinder: for some
\(0<r<R\), a nonnegative \(\chi\in C_c^\infty(B_r)\), and
\(\eta_*>0\),
\[
  \int_{B_r}|V_n(y,s)|^2\chi(y)\,dy\ge\eta_*
...
nonzero bounded selected-window limit, which exits the retained local Type II
row by \cref{paper3:lem:scale-rigid-bounded-limit-exit}, or it reduces
...
claimed finiteness is immediate.  The final assertion is exactly the
first-failure convention in \cref{paper7:lem:no-unrecorded-cost-escape}: a
missing compact-cylinder local energy bound is not kept inside the retained
compact-cost branch and is not repaired by a whole-space critical estimate.
\end{proof}
...
\]
\end{enumerate}
In the pressure-carrier case the branch is a pressure/exterior local row unless
\cref{paper8:lem:exterior-pressure-only-not-terminal} converts it, after
shrinking the cylinder and passing to a subsequence, into a velocity carrier on
an exterior or separated-core cylinder.
\end{lemma}
...
\(n\), and a subsequence gives one of the two alternatives.  The final sentence
is only the ordered local reading of the pressure alternative: a pressure lower
bound is not used as a hidden compact velocity lower bound, but is passed to
the pressure/exterior routing lemma
...
slightly larger annulus, hence
\cref{paper8:lem:exterior-ckn-carrier-or-pressure-route} routes it to a
velocity carrier or to the pressure-only exterior row;
...
\(\varepsilon_{\rm ext}\), then the velocity-pressure split gives item
\textup{(i)}.  If no such cylinder exists, every admissible subcylinder is
...
\textup{(i)} and routes back to the velocity-carrier cases.
\par
...
\emph{Case B: escape to infinity in the renormalized coordinate.}  For every
\(R<\infty\),
\[
...
producing a sequence of \(L^3\)-concentration centers
...
\textup{(A2)} hold on the recentered windows.  The moving \(L^2\) core floor is
the retained-window branch of \cref{paper5:thm:core-retention} read on the
same canonical terminal windows; if the loss branch of that theorem occurs,
the lower active-core row has failed and the branch is in \textup{(A1)}.  This
...
\item on a retained core cylinder carrying the positive local concentration, the
...
\begin{proof}
Fix a retained cylinder \(Q_\rho\) on which the positive core concentration is
recorded.  If
\[
...
modulation control on the selected windows, and positive core concentration are
...
positive CKN mass, compactness failure inside the local concentration selection,
or scale cascade is the multibubble/cascade side.  On a represented retained
core, rough-core loss is
...
bounded selected-window limit, which exits the retained local Type II row, or
it reduces to the compact single-core branch, which is excluded by
\cref{paper6:thm:single-core}.  Hence it cannot be a retained local Type II
branch.
\end{proof}
...
\cref{def:c4-cost-comparison}.  Therefore the retained cost-divergence
...
retain a Type II branch.  Instead one reruns the local validation with the
canonical identity cost supplied by the suitable local energy inequality.  If
the canonical channel passes, the retained stratum is closed by
...
Thus an incomparable noncanonical cost is a bookkeeping failure of that
diagnostic, not an additional analytic exit and not a hidden input.
...
\begin{corollary}[All three remaining named exit rows are closed]
\label{paper6:cor:remaining-named-exits-closed}
The three rows listed in \cref{paper6:def:remaining-named-exits} are all
closed adjacent rows: the autonomous-modulation row by
...
\cref{paper6:thm:active-core-loss-closed}.
\end{corollary}

\begin{proof}
Direct application of the three closure theorems cited in the statement.
\end{proof}
...
identity-cost channel, including noncanonical cost-comparison failures not
covered by an explicit local comparison theorem;
...
closed structural rows listed below.
\end{enumerate}
...
\medskip
The three rows \textup{(i)}, \textup{(ii)}, \textup{(iii)} listed above are
closed adjacent rows, by the closure theorems of
\cref{paper6:subsec:remaining-exit-closures}: the autonomous-modulation row
...
detection, by \cref{paper6:thm:active-core-loss-closed}.  This is
...
states, while \cref{paper6:prop:state-elimination} eliminates the retained
states.  The ambient and terminal wrappers verify the retained inputs and route
pre-retention first failures.  Those first failures are either closed by their
local closure theorem or listed in
\cref{paper6:def:remaining-named-exits}.  Thus no retained Type II branch
survives.  The closure corollary above closes the remaining named exits, and
the next theorem records the resulting full local Type II exclusion.
\end{proof}
...
Finally, a nonzero bounded selected-window limit is incompatible with the local
Type II definition by \cref{paper3:lem:scale-rigid-bounded-limit-exit}.  Hence
all hypotheses used in \cref{thm:c3-local-retained-mass-cost-divergence} are
local consequences of the retained compact data and validation rows.
\end{proof}
