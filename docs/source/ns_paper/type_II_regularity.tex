...
classification theorem is used implicitly.
\end{abstract}
...
terminal profile decoupling, cost comparison, and local state-space closure are
assumed or proved explicitly where they are used.  No additional stationary,
ancient, or self-similar classification theorem is invoked implicitly.
...
assumption.  The terminal profile and corrected monotonicity sections then turn
these local reductions into a branchwise classification, and the final assembly
eliminates each covered Type II alternative.
...
one of the explicitly named local state-space exits.
...
stationary omega-limit give a rigidity-covered shortcut when such a stationary
profile theorem is available;
\item thin or oscillatory drift is a named local drift alternative;
\item a thick autonomous limit which remains in the retained compact
scale-collapse state is routed by
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged} into the
scale-rigid local discharge, rather than into a new global Liouville problem.
\end{enumerate}
The remaining analytic alternatives are thin drift, compactness failure,
failure of modulation convergence, and failure to remain in the retained
compact scale-collapse state.
...
\item The retained scale-collapse drift stratum is excluded by the local
compact scale-collapse routing theorem
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged}: finite-cost
scale collapse is removed by the localized cost exclusions, while a thick
autonomous branch that keeps the retained compact-state inputs enters the
scale-rigid local stratum.  If one of those retained inputs fails, the branch is
already assigned to a named local state-space exit.
\end{enumerate}
...
If scale-collapse drift occurs, the localized cost exclusions first remove the
finite scale-collapsing-cost cases.  In the remaining thick autonomous case,
failure of any retained compact-state input is an ordered local exit, not an
unlabelled residual branch.  If all retained inputs hold, the branch is a
retained compact scale-collapse state and
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged} routes it into the
scale-rigid discharge, giving the required contradiction.  The scale-collapse
drift alternative is therefore excluded without a separate stationary
...
regularity, retained compact scale-collapse routing, or scale-rigid discharge,
not an additional singularity class.
...
\begin{definition}[Local active-cascade mass budget]
\label{def:s5-local-active-cascade-budget}
An admissible same-point critical cascade decomposition has local active mass
budget \(M\) if, for every finite set \(J\subset\mathcal J\),
\[
  \sum_{j\in J}\|\phi^j\|_{L^3(\mathbb R^3)}^3\le M^3 .
\]
This is a ledger condition on the extracted active profiles in the selected
local state-space branch.  It is not a global \(L^3(\mathbb R^3)\) bound for
...
with local active mass budget \(M\).  If every active profile satisfies
\[
  \|\phi^j\|_3\ge\eta>0,
\]
...
\begin{proof}
Let \(J\subset\mathcal J\) be finite.  The local active mass budget gives
\[
  M^3\ge
  \sum_{j\in J}\|\phi^j\|_3^3
  \ge
...
\item local compactness, autonomous modulation, and recognition of the retained
compact scale-collapse state are stable under adding \(E_n\to0\);
\item the perturbation creates no unrecorded terminal state: after passing to
the scale-collapse limit, every failure of a retained compact-state input is a
named local alternative, and otherwise the branch is eligible for
...
Let \(u\) be a suitable Navier--Stokes solution on a terminal neighbourhood of
\((x_*,T^*)\).
...
decomposition with local active mass budget \(M\);
...
\item the innermost scale-collapse branch is either already routed to a named
local nonretained alternative or satisfies the retained compact scale-collapse
state of \cref{paper5:def:retained-compact-scale-collapse-state}.
\end{enumerate}
...
topology required by the retained compact scale-collapse passage.
...
\(\lambda_1(t)\to0\) gives the scale-collapse hypothesis.  If any retained
compact-state input fails,
\cref{paper5:lem:retained-scale-collapse-first-failure} places the branch in a
named local nonretained alternative, which is incompatible with being an
unresolved regular strict retained cascade.  Otherwise the branch is a retained
compact scale-collapse state.  Applying
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged} excludes that
state by routing it into the already discharged scale-rigid branch.  This
contradicts the active innermost mass floor
\(\|\phi^1\|_3\ge\eta>0\).  Hence the regular strict same-point cascade cannot
occur.
\end{proof}
...
positive finite critical mass; perturbative scale-collapse robustness; and
retained compact scale-collapse routing for the innermost branch.  Bounded
critical mass alone proves only uniformly bounded cascade length.  Full
exclusion of the regular same-point cascade requires the inner-decoupling and
local retained-state routing hypotheses.
...
vanishes in the retained compact scale-collapse passage.
\end{enumerate}
\end{definition}
...
needed to enter the repaired-gauge Type II class.  Assume same-point nonlinear
decoupling, a finite local active-cascade mass budget \(M\), the innermost
repaired-gauge representation, and the local retained compact scale-collapse
routing statement for the innermost branch: either a retained compact-state
input fails and the branch is assigned to a named local exit, or the retained
compact scale-collapse theorem
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged} applies.  Then no
...
least \(\varepsilon_{\rm sd}\) critical mass.  The selected active-profile
ledger has a finite local budget \(M\), so
...
The scale \(\lambda_1^n\to0\) gives the scale-collapse hypothesis.  If a
retained compact-state input fails,
\cref{paper5:lem:retained-scale-collapse-first-failure} places the branch in a
named local exit rather than in an unresolved same-point cascade.  Otherwise
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged} applies to the
retained innermost branch and excludes it by the scale-rigid discharge.  The
nonzero branch input is the active bubble mass floor:
\[
  \|\phi^1\|_3\ge\varepsilon_{\rm sd}>0.
\]
Thus the routed scale-collapse branch contradicts the existence of the active
innermost bubble.
\end{proof}
...
the retained compact scale-collapse routing statement for the innermost branch.
These are analytic profile, representation, interaction, or local state-space
routing failures; a below-threshold profile is regular and is not a singular
cascade bubble.
...
\cref{paper3:ass:scale-rigidity} by an explicit local compactness
...
\cref{paper3:ass:scale-rigidity}, proved here as a theorem in the stated
class.
\end{proposition}
...
the sense of \cref{paper8:ass:terminal-windowed-profile-completeness}, local
...
the finite local critical norm and the retained local-state bounds.
...
\cref{paper5:ass:selected-window-estimates};
...
\item no unresolved thin-drift, multicore, same-point cascade,
separated/exterior, rough-core, autonomous-modulation-failure, or
compactness-failure label after the ordered local state-space tests have been
applied.
\end{enumerate}
\end{definition}
...
failure, autonomous-modulation failure, no retained active core,
...
\cref{paper5:thm:scale-collapse-stratification}.  Failure of
...
\cref{paper5:ass:selected-window-estimates} on the same window sequence.  Then
...
scale-collapse branch remains.  In particular, the compact scale-collapse
stationary-rigidity input is not an independent assumption on the retained
local state-space branch.
\end{theorem}
...
retained compact scale-collapse states.  They are exactly named local
alternatives in the ordered stratification and are excluded from the present
hypothesis by
\cref{paper5:def:retained-compact-scale-collapse-state}\textup{(vii)}.
...
nonzero bounded selected-window limit, which is incompatible with the local Type
II definition by \cref{paper3:lem:scale-rigid-bounded-limit-exit}, or it reduces
...
The stationary omega-limit and stationary-rigidity theorem remain valid
shortcuts when available, by \cref{paper5:thm:stationary-rigidity-blocker}, but
they are not needed for the retained local state-space branch: a nonstationary
autonomous obstruction is already visible as a scale-rigid local branch and is
removed by the scale-rigidity discharge.
\end{proof}
...
compactness alternatives are items \textup{(iv)}--\textup{(vi)}.  It remains
...
compact cost-divergence regime, to be further stratified by carrier
...
\item the compact cost-divergence regime is evaluated as the retained compact
...
\item every carrier-validation adjacent stratum arising from this regime is
...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item the compact cost-divergence regime, with its carrier-validation
subalternatives;
...
only remaining alternative is the compact cost-divergence regime.  By
hypothesis this regime has already been passed through the ordered
...
verified either by the fixed-cutoff criterion of
\cref{paper7:lem:fixed-monotonicity-gives-cost-to-exclusion-data} or by the
moving-cutoff criterion of
...
and is treated as an adjacent compactness stratum.
\end{definition}
...
Fix \(R_0<\infty\) and a unit terminal window \(I_n\).  Suppose either
\[
  \operatorname*{ess\,sup}_{\tau\in I_n}
  \|V(\tau)\|_{L^2(B_{R_0})}<\infty,
\]
as supplied by the local suitable-energy class on compact repaired-gauge
cylinders, or the positive finite critical annulus of
\cref{def:c3-critical-mass} holds on the terminal tail:
\[
...
identity cost is available on every retained compact cylinder either from the
local energy class or, as a sufficient bookkeeping route, from the global
critical-annulus test.  Failure of both local \(L^2\) control on the selected
compact cylinder and the finite critical-annulus route is recorded by the
ordered local validation as a local-energy, rough-core, or critical-mass exit.
\end{lemma}

\begin{proof}
If the displayed local \(L^\infty_\tau L^2_y(B_{R_0})\) bound is part of the
retained local state data, there is nothing to prove.  This is the case used in
the physical local no-Type-II route: the repaired-gauge pullback of a suitable
weak solution belongs to the local energy class on compact cylinders, and the
ordered validation records absence of that bound as a local-energy or
rough-core failure rather than repairing it by a global estimate.
\par
In the optional critical-annulus route, for a.e. \(\tau\in I_n\), Holder's
inequality on the finite-measure ball gives
\[
  \|V(\tau)\|_{L^2(B_{R_0})}
  \le
  ...
  |B_{R_0}|^{1/6}M .
\]
Taking the essential supremum over \(I_n\) proves finiteness in this second
case.  The final assertion is exactly the first-failure convention in
\cref{paper7:lem:no-unrecorded-cost-escape}: a missing compact-cylinder local
energy bound is not kept inside the retained compact-cost branch.
\end{proof}
...
\[
  ...
  \qquad n\in\mathbb N,
\]
the branch has the local compact-cylinder \(L^\infty_\tau L^2_y(B_{R_0})\)
input of \cref{paper7:lem:critical-annulus-local-l2} and local windowed
\(H^1(B_{R_0})\) control.  Suppose also that the local
...
\par
The same conclusion holds by the older sufficient route when the positive
finite critical annulus supplies the compact-cylinder \(L^2\) input through
\cref{paper7:lem:critical-annulus-local-l2}.  Failure of the conclusion is
exactly one of the local failures in the hypotheses: local compact-cylinder
\(L^2\) control, windowed \(H^1\) control, modulation-integrability, or
...
\item retained finite critical-mass window, read either as the compact
positive finite critical \(L^3\) annulus or, in the physical local route, as
the compact-window finite critical-mass input of
...
For the compact-cost bookkeeping criterion this is the ordered critical
\(L^3\) annulus test of \cref{def:c3-critical-mass-alternatives}.  For the
physical local route it is the compact-window version of
\cref{paper6a:lem:terminal-local-critical-mass-routing}: lower active mass
failure means non-retention, and failure of a finite local upper bound produces
...
directly or by placing the branch in a previously listed non-compact-cost
...
\item the retained finite critical-mass window: in the compact-cost
bookkeeping route this may be the positive finite critical \(L^3\) annulus of
\cref{def:c3-critical-mass}, while in the physical local route it is the
compact-window finite critical-mass input supplied by
\cref{paper6a:lem:terminal-local-critical-mass-routing};
...
failure of the whole-space critical annulus is not repaired by assuming a global
\(L^3\) bound; it is routed by the local terminal critical-mass and
exterior/state-space alternatives.
\end{definition}
...
\begin{definition}[Fixed annular PDE estimates]
...
\]
A branch in the pre-carrier validated compact-cost regime satisfies the fixed
annular PDE estimates if, on a terminal tail,
\begin{enumerate}[label=\textup{(\roman*)}]
\item
\[
...
\begin{lemma}[Fixed PDE estimates gives fixed finite error]
\label{paper7:lem:fixed-pde-package-gives-components}
If a pre-carrier branch satisfies the fixed annular PDE estimates of
...
criterion, \(I_5=a_-M_\phi\) is integrable by item \textup{(v)}, and \(I_6\) is
...
\begin{definition}[Moving annular PDE estimates]
\label{paper7:def:moving-pde-error-package}
A branch in the pre-carrier validated compact-cost regime satisfies the
moving annular PDE estimates if there is a finite-valued nondecreasing
...
\begin{remark}[Pressure item in the moving PDE estimates]
...
Thus the genuinely new moving-cutoff burden is concentrated in the annular
velocity packing, the translation term, the positive scale-shell term, and the
negative-drift alternative.
\end{remark}

\begin{lemma}[Moving PDE estimates gives moving finite error or scale-drift alternative]
\label{paper7:lem:moving-pde-package-gives-components}
If a pre-carrier branch satisfies the moving annular PDE estimates of
...
\]
Hence items \textup{(i)} and \textup{(iii)} of the moving criterion make
...
\begin{theorem}[Carrier closure reduced to explicit PDE estimates and scale-drift alternative]
...
\(\omega\) satisfies either the fixed annular PDE estimates of
\cref{paper7:def:fixed-pde-error-package} or the moving annular PDE estimates of \cref{paper7:def:moving-pde-error-package}.  Then exactly one of the
following occurs:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
If the fixed annular PDE estimates hold, then
...
comparison are automatic on the pre-carrier regime by
...
Thus the fixed-cutoff localized-energy closure criterion holds, and
\cref{paper7:thm:fixed-carrier-closure} excludes \(\omega\).
\par
Suppose instead that the moving annular PDE estimates hold.  By
...
automatic on the pre-carrier regime by
\cref{paper7:lem:precarrier-finite-local-energy,paper7:lem:nested-moving-carrier-comparison}.
Therefore the moving-cutoff localized-energy closure criterion holds, and
\cref{paper7:thm:moving-carrier-closure} excludes \(\omega\).  The three
alternatives are disjoint by construction.
\end{proof}

\begin{corollary}[Explicit PDE estimates leave no unresolved compact-cost branch]
...
in the pre-carrier validated compact-cost regime whose validated compact cost
is energy-controlled by the fixed identity cost satisfies either the fixed
annular PDE estimates of
\cref{paper7:def:fixed-pde-error-package} or the moving annular PDE estimates of \cref{paper7:def:moving-pde-error-package}, then every branch with
...
carrier closure, or assigned to a non-carrier or scale-drift branch that is
already closed in the ambient class.  In particular, the compact-cost regime
...
compact-cost regime.  Then the explicit PDE-estimate hypothesis and
...
carrier closure, or assigned to the scale-collapse drift obstruction, which is
...
\begin{remark}[The explicit PDE estimates are not the remaining target]
\label{paper7:rem:pde-packages-optional}
The explicit PDE estimates of
...
remaining analytic target.  The proof does not require one global
...
into a neighboring compactness stratum.  The remainder of this section
therefore isolates only the small alternative statements needed for that
local compactness reduction.
\end{remark}
...
is not used here as a hidden global estimate.  The finite initial localized
energy required by either localized-energy criterion is supplied locally, after
a harmless terminal restriction, by
...
\(\eta\).
\par
Localize each annulus by a finite smooth partition after rescaling it to a
fixed reference annulus.  The Calderon--Zygmund part generated by the velocity
stress inside the recorded annulus is controlled by the local annular
\(L^3\)-mass.  Since the retained branch has not failed critical tightness or
exterior compactness, those annular \(L^3\)-masses vanish along \(S_m\to\infty\)
on the selected compact interval.  Hence the local Calderon--Zygmund pressure
part cannot carry the lower bound \(\eta\).
\par
The remaining pressure contribution is harmonic on the retained compact
subannulus after subtracting functions of time.  The quantitative nested-ball
harmonic-tail lemma
\cref{paper8:lem:nested-ball-harmonic-tail}, applied after the same annular
rescaling, says that any non-vanishing retained harmonic pressure must be
generated by a non-vanishing recorded exterior annular source.  If that source
is present, the ordered pressure/exterior routing
\cref{paper8:lem:harmonic-cross-pressure-routing,paper8:thm:exterior-stratification}
assigns the branch to a pressure-reconstruction, exterior-leakage, or
separated-core label.  If no such source is present, the exterior pressure
vanishes modulo functions of time by the retained pressure lemma used in
\cref{paper8:lem:harmonic-cross-pressure-routing}.  Both possibilities
contradict the hypotheses that the branch is retained on \(I\) and that the
pressure flux carries mass \(\eta\).
\par
...
\]
Since the branch is retained in the pre-carrier regime, pressure
reconstruction and the exterior/critical-tightness rows have not failed on
\(I_n\).  Applying
\cref{paper7:lem:retained-pressure-tail-windowwise} on \(I_n\) with tolerance
\(2^{-n}\), choose \(P_n<\infty\) such that every finite-valued moving radius
\(R(\tau)\ge P_n\) on \(I_n\) has pressure flux at most \(2^{-n}\) on that
window.
...
finite critical-mass row itself, not inferred from the \(J_7\)-density.  In the
physical local route this is the compact-window active mass supplied by
\cref{paper6a:lem:terminal-local-critical-mass-routing}; in the older
compact-cost bookkeeping route it may be represented by the positive finite
critical \(L^3\)-annulus test.  Since the present occupation state has no
exterior/leakage label, the corresponding critical mass remains inside the
retained compact single-core region.  Cover a
...
statements of
\cref{paper7:def:windowwise-transition-routing,paper7:def:windowwise-positive-scale-routing}
hold, together with the negative-drift alternative statement used in
...
stratified moving-cutoff alternative.  Then
...
branch is either class-incompatible or is assigned to a named local
state-space alternative which is already eliminated by the local packages of
the paper.
\end{lemma}
...
multibubble/cascade, gauge-degenerate, scale-degenerate, or scale-rigid
alternative, all eliminated by
\cref{paper6:prop:state-elimination}.
\par
...
\cref{paper6a:lem:terminal-local-critical-mass-routing}.  The whole-space
critical \(L^3(\mathbb R^3)\) alternatives in
\cref{thm:c3-l3-annulus-classification} are therefore pre-compact-data exits
for the compact-cost criterion, not global assumptions in the local no-Type-II
proof.  Once the retained local finite critical-mass window is reached, the
...
\cref{paper8:thm:exterior-stratification}.  Local windowed \(H^1\)-loss is the
rough-core row and is eliminated by
\cref{paper6:thm:rough-core,paper6:prop:state-elimination}.  Multibubble,
multicore, cascade, and gauge-degenerate rows are alternatives in
\cref{paper6:thm:state-decomposition} and are eliminated by
\cref{paper6:prop:state-elimination}.  Finite-cost scale collapse is eliminated
by \cref{paper6:thm:scale-cost}.  Scale-rigid residual and retained compact
...
The thin-drift, autonomous-modulation, and compactness failures which can arise
inside the scale-collapse stratification are the named first failures of
\cref{paper5:lem:retained-scale-collapse-first-failure}; they are non-retained
scale-collapse exits, while the retained compact scale-collapse state is
excluded by
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged}.
Thus every non-carrier first failure is already class-incompatible or assigned
to a closed named local branch.  No step uses a global \(L^3\) bound, a global
profile theorem, or vanishing localized dissipation outside the selected local
branch.
\end{proof}
...
alternatives are excluded by \cref{paper6:thm:scale-cost}.  Thin-drift,
autonomous-modulation failure, compactness failure, exterior/separated-core
loss, rough-core loss, and multicore or same-point cascade are exactly the
non-retained first failures listed in
\cref{paper5:lem:retained-scale-collapse-first-failure}; by
\cref{paper7:lem:ambient-noncarrier-exits-close} those labels are already
routed to closed local state-space branches.  The only retained scale-collapse
state left after those exits is the retained compact scale-collapse state, and
it is excluded by
...
routed local scale-collapse branch and not a hidden assumption.
\end{proof}
...
\begin{proof}
Non-carrier adjacent exits are closed by
\cref{paper7:lem:ambient-noncarrier-exits-close}.  The exterior,
...
and positive-scale alternative statements are no longer assumptions:
\cref{paper7:cor:windowwise-transition-routing-holds,paper7:cor:windowwise-positive-scale-routing-holds}
prove them on the canonical windowwise moving family.  The negative-drift
...
the unconditional no-Type-II theorem is discharged by local state-space
stratification and the canonical windowwise carrier package.
\end{corollary}
...
\begin{definition}[Transition/leakage alternative statement]
\label{paper7:def:transition-leakage-routing}
The pre-carrier validated compact-cost regime has transition/leakage alternative
if, for every branch \(\omega\) in that regime and every moving test family in
...
\begin{definition}[Pressure alternative statement]
\label{paper7:def:pressure-routing}
The pre-carrier validated compact-cost regime has pressure alternative if, for
every branch \(\omega\) in that regime and every moving test family in the
...
\begin{definition}[Translation alternative statement]
\label{paper7:def:translation-routing}
The pre-carrier validated compact-cost regime has the translation alternative if, for
every branch \(\omega\) in that regime and every moving test family in the
sense of \cref{paper7:def:moving-test-family}, failure of
\[
...
\]
on every later tail \([\tau_1,\infty)\) forces \(\omega\) into the
translation alternative of
\cref{paper7:def:stratified-moving-carrier-routing}.
\end{definition}

\begin{definition}[Positive-scale alternative statement]
\label{paper7:def:positive-scale-routing}
The pre-carrier validated compact-cost regime has positive-scale alternative if,
for every branch \(\omega\) in that regime and every moving test family in the
sense of \cref{paper7:def:moving-test-family}, failure of
\[
...
\begin{theorem}[Small alternative statements imply stratified moving-cutoff alternative]
...
subidentity in the sense of
\cref{paper7:lem:suitable-energy-gives-carrier-subidentities}.  Assume also
that the regime has the transition/leakage, pressure, translation, and
positive-scale alternative statements of
...
\cref{paper6a:def:negative-drift}.  Then the regime has stratified
moving-cutoff alternative in the sense of
\cref{paper7:def:stratified-moving-carrier-routing}.
\end{theorem}
...
localized-energy closure criterion.  Hence \(\omega\) satisfies alternative \textup{(ii)}
...
the choice of \(k\).  If \(k\in\{1,2,3\}\), the transition/leakage alternative
statement places \(\omega\) in alternative \textup{(iii)} of
\cref{paper7:def:stratified-moving-carrier-routing}.  If \(k=4\), the pressure
alternative statement gives the same conclusion.  If \(k=5\), the translation
alternative statement gives alternative \textup{(iv)}.  If \(k=6\), the negative
scale-drift hypothesis gives alternative \textup{(vi)}.  If \(k=7\), the
positive-scale alternative statement gives alternative \textup{(v)}.  Therefore
one of the alternatives in
\cref{paper7:def:stratified-moving-carrier-routing} always holds.
\end{proof}

\begin{theorem}[Stratified moving-cutoff alternative leaves only named branches]
\label{paper7:thm:stratified-moving-carrier-routing}
Assume every branch in the pre-carrier validated compact-cost regime uses a
validated compact cost that is energy-controlled by the fixed identity cost.
Assume also that the regime has stratified moving-cutoff alternative in the sense
of \cref{paper7:def:stratified-moving-carrier-routing}.  Then every branch in
the pre-carrier validated compact-cost regime, after restriction to a later
...
stratified alternative itself.  This yields the displayed list.
\end{proof}

\begin{corollary}[Adjacent closure reduced to stratified moving-cutoff alternative]
...
compact-cost regime has stratified moving-cutoff alternative, then the
compact-cost regime has adjacent-stratum closure in the sense of
\cref{paper7:def:compact-cost-adjacent-closure}.
\end{corollary}
...
\cref{paper7:cor:retained-stratum-cost-exclusion}.  If \(\omega\) enters a
non-carrier adjacent stratum before the pre-carrier regime, then the first
...
branch is either excluded on the retained stratum or assigned to a previously
closed adjacent branch.  This is exactly
\cref{paper7:def:compact-cost-adjacent-closure}.
\end{proof}

\begin{corollary}[Adjacent closure reduced to the small alternative statements]
...
negative scale-drift obstruction are closed by their assigned local arguments
in the ambient class.  Assume every branch in the pre-carrier validated
compact-cost regime uses a validated compact cost that is energy-controlled by
...
translation, and positive-scale alternative statements of
\cref{paper7:def:transition-leakage-routing,paper7:def:pressure-routing,paper7:def:translation-routing,paper7:def:positive-scale-routing}
hold, together with the negative-drift alternative statement used in
...
conditional on the analytic inputs explicitly stated below: terminal
...
\[
  ...
  \right\|_{L^3(B_R)}.
\]
Terminal windowed state-space completeness means that if
...
\begin{proof}
This is the compact-window form of
\cref{paper6a:thm:critical-ns-profile-decomposition-discharged}.  If
...
compactness lemma
\cref{paper6a:lem:l3-profile-extraction-cited} gives an occupation state, and
\cref{paper6a:lem:heat-flow-kato-smallness} assigns that state to the first
closed label in the local state-space decomposition.  An adjacent label is
handled by its local closure theorem and is not part of the retained terminal
branch.  A retained comparable label must already have been grouped into the
selected terminal class by the terminal frame selection rule.  Thus positive
nonretained residual mass cannot persist on a retained terminal branch.
\end{proof}
...
\begin{proof}
The local part tends to zero by Calderon--Zygmund estimates:
\[
...
Calderon--Zygmund piece and the recorded annular pieces have been removed, the
...
\]
be the local Calderon--Zygmund pressure generated by the cross stress.  Suppose
...
follows from the retained positive local critical annulus and local
...
the local pressure part by Calderon--Zygmund estimates.  Any non-vanishing
...
component has a compact terminal cylinder \(Q_{R,I}\) with
\[
  \limsup_{n\to\infty}
...
the asserted \(L^\infty_\tau L^3_y\) lower bound with
...
\cref{paper7:cor:windowwise-positive-scale-routing-holds,paper7:cor:windowwise-transition-routing-holds}.
We make this implication explicit here so that the assumption can be removed
...
same negative-drift alternative hypothesis already standing in that theorem,
...
\cref{paper7:cor:windowwise-positive-scale-routing-holds,paper7:cor:windowwise-transition-routing-holds},
with the standing hypothesis set of
\cref{paper7:thm:windowwise-routing-gives-stratified-routing} (negative-drift
alternative for \(J_6\)) plus the adjacent-stratum closure of
...
\begin{proof}
The concentration extraction supplies a nonzero local occupation state on a
compact terminal cylinder.  The repaired-gauge representation transports that
state to the renormalized trajectory \(V(\tau)\) by Navier--Stokes critical
symmetries and admissible chart maps.  These symmetries preserve the local
scale-critical \(L^3\) quantity on corresponding compact cylinders.

Because the extracted state is nonzero in \(L^3_{\rm loc}\) after choosing
...
nonretained residual \(S_n\).  The old critical-profile label is replaced by
the following local notions.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\[
  ...
  \Big)<\infty ,
\]
and the same local velocity bounds hold for \(U_n\).  The time derivatives are
bounded in \(L^1(I;H^{-m}(B_{2R}))\) for some \(m\) large enough for the
Aubin--Lions compactness argument.
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)} on these
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)} on
\(Q_{2R,I}\).  Then every sequence of selected subwindows and subcylinders has
a subsequential local occupation state.  More precisely, after passing to a
subsequence, \(V_n\), \(U_n\), and \(S_n\) converge weakly in
\(L^2(I;H^1(B_R))\), strongly in \(L^2(I;L^q(B_R))\) for every \(q<6\), and
their pressure gradients converge distributionally modulo functions of time.
\end{lemma}

\begin{proof}
The velocity bounds and the time-derivative bound give compactness by the
Aubin--Lions--Simon theorem applied to
\[
  H^1(B_R)\Subset L^2(B_R)\hookrightarrow H^{-m}(B_R).
\]
Interpolation with the uniform \(L^2(I;H^1(B_R))\) bound gives strong
convergence in \(L^2(I;L^q(B_R))\) for every \(q<6\).  The pressure
normalization bound gives weak compactness in \(L^{3/2}(Q_{R,I})\); on overlaps
the limits differ only by functions of time, so the gradients assemble
distributionally.
\end{proof}
...
\begin{proof}
The topology records the velocity variables weakly in \(L^2H^1\), strongly in
\(L^2L^q_{\rm loc}\) for every \(q<6\), the selected-time residual measures
\(|S_n(\tau_n)|^3\,dy\) weakly-\(*\) as finite Radon measures on compact balls,
the pressures weakly in \(L^{3/2}_{\rm loc}\) modulo functions of time, and the
modulation, scale, center, cutoff, and cost variables in the compactified
repaired-gauge coordinates used in the ordered validation procedure.  The
compactification adds boundary faces exactly for loss of representation, scale
admissibility, bounded modulation, pressure reconstruction, local \(H^1\)
control, critical tightness, and cost compatibility.
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)}.  Suppose
...
\[
  ...
  \mu_S(B_\rho)>0 .
\]
Then, after shrinking \(B_\rho\) if necessary, either the full represented
branch \(V_n\) carries a positive compact local concentration label, or the
retained component \(U_n\) carries a positive comparable retained label in the
same terminal frame, or a separated-center/separated-scale adjacent label has
already occurred.  In particular, residual \(L^3\) concentration cannot be
invisible in the augmented local state space by cancellation between \(V_n\)
and \(U_n\).
\end{lemma}
...
\[
  |S_n|^3=|V_n-U_n|^3\le4\bigl(|V_n|^3+|U_n|^3\bigr).
\]
Passing to weak-\(*\) limits of the finite measures on a slightly smaller ball
gives
\[
  \mu_S\le4(\mu_V+\mu_U)
\]
for subsequential limits of \(|V_n(\tau_n)|^3dy\) and
\(|U_n(\tau_n)|^3dy\).  Hence \(\mu_V\) or \(\mu_U\) has positive mass on some
subball.  If \(\mu_V\) has positive mass, the full represented branch has a
positive compact local concentration diagnostic and the ordered local
state-space decomposition assigns it to the corresponding first label.  If
\(\mu_U\) has positive mass, the mass belongs to the retained component already
represented in the selected terminal frame.  If it were not comparable to that
frame, separated-center or separated-scale decoupling would place it in an
adjacent row.  If it is comparable but not already absorbed, then it is a
retained class missed by the terminal grouping, contradicting
\cref{paper6a:lem:retained-frame-maximality} below.

This visibility argument does not assert a Br\'ezis--Lieb identity for the
moving pair \(V_n=U_n+S_n\).  Such an identity would require additional
orthogonality hypotheses and is false for arbitrary moving decompositions.  The
local state-space proof uses only the domination inequality above and the
maximality of the retained grouping: cancellation of \(V_n\) against \(U_n\)
can make the represented measure small only by making the retained measure
large, and a positive retained measure is itself a recorded comparable or
adjacent local label.  Hence positive residual \(L^3\) mass cannot disappear
from the augmented state-space diagnostics.
\end{proof}
...
\item the retained terminal component has positive finite local critical
  \(L^3\) mass on the cylinder;
\item the lower active mass floor fails, so the candidate is not a retained
  terminal component;
\item the local \(L^3\) upper bound fails on the compact cylinder, in which
  case the ordered local state space records an additional compact active
  label: a critical-mass adjacent label at the same retained frame, a
  comparable finite-mass retained class which must be grouped into the retained
  component, a multibubble/multicore label, a cascade label, a rough-core label,
  or an exterior/separated label.
\end{enumerate}
In the retained terminal branch only \textup{(i)} remains.
\end{lemma}

\begin{proof}
The lower bound in \textup{(i)} is the active local mass floor of
...
compact cylinder \(B_R\times I\).  Then there are times \(\tau_n\in I\) and
levels \(M_n\to\infty\) with
\[
  \int_{B_R}|V_n(y,\tau_n)|^3\,dy\ge M_n .
\]
Cover \(B_R\) by finitely many smaller balls.  On one ball, after passing to a
subsequence and renormalizing at the corresponding local scale if necessary,
the branch carries a further positive compact \(L^3\) diagnostic.  The ordered
local state-space compactification records the first label of that diagnostic.
If the diagnostic stays at the same comparable retained frame and the upper
local \(L^3\) bound still fails, the first label is the critical-mass adjacent
face itself; by the retained-state definition this is not a retained terminal
...
one of the already proved local alternatives.  The adjacent labels are closed
...
are assigned to their named local alternatives and do not remain on the retained
terminal branch.
\end{theorem}
...
rows are routed by
\cref{paper6a:lem:terminal-local-critical-mass-routing}: a lower-mass failure
...
\cref{paper8:thm:exterior-stratification}.  Local \(H^1\)-loss is the rough-core
row in \cref{paper6:prop:state-elimination}.  Multibubble, multicore, cascade,
and gauge-degenerate rows are the alternatives of
\cref{paper6:thm:state-decomposition} and are eliminated or assigned by
\cref{paper6:prop:state-elimination}.  Finite-cost scale-collapse is routed by
\cref{paper6:prop:state-elimination} through the scale-cost theorem, and
scale-rigid residual labels are routed by
\cref{paper3:prop:scale-rigidity-discharged}.  Carrier and compact-cost
compatibility rows are closed by the branchwise ambient adjacent wrapper
\cref{paper7:cor:ambient-adjacent-wrapper-discharged} and the
cost-divergence discharge
\cref{paper6a:thm:cost-divergence-exclusion-discharged}.  Thus every adjacent
first label is already a named local alternative.  The only first label not
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)}.  If
\[
  \limsup_{n\to\infty}\|S_n\|_{L^\infty(I;L^3(B_R))}>0,
\]
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)} and
...
diagnostic is visible either as a positive compact local label of the full
represented branch, as a positive comparable retained label, or as an already
named separated-center/separated-scale adjacent label.  Hence it cannot be
hidden by cancellation with \(U_n\).
...
\cref{paper6:thm:state-decomposition} to the full represented branch
\((V_n,P_n)\) on the selected compact window.  If the full branch produces a
new compact concentration, splitting, cascade, scale-collapse, or scale-rigid
state, this is again a named adjacent local alternative.  The remaining
possibility, using
\cref{paper6a:lem:residual-concentration-visible}, is that the residual
...
adjacent rows.  Hence the first nonretained residual row is a retained
comparable class.
\end{proof}
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(ii)}.
\end{lemma}
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)} satisfies the
local terminal completeness conclusion of
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(iii)}, after the
...
needed to pass to occupation states.  \Cref{paper6a:lem:critical-mass-decoupling}
identifies the closed local labels, and
\cref{paper6a:thm:terminal-adjacent-label-routing} routes every adjacent label
to a named local alternative.  \Cref{paper6a:lem:heat-flow-kato-smallness}
assigns any positive nonretained residual mass to one of those labels.
\Cref{paper6a:lem:nonlinear-profile-evolution-cited} rules out persistence of
such a label inside the retained branch once comparable retained classes have
been grouped, and \cref{paper6a:lem:retained-frame-maximality} certifies
that the terminal selection rule actually performs that grouping
maximally.  Hence
\cref{paper6a:lem:profile-exhaustion-by-diagonal-extraction} gives
\(S_n\to0\) on compact terminal cylinders.  This is the local terminal
completeness conclusion named in
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(iii)}.
\end{proof}
...
\cref{paper6a:ass:critical-ns-profile-decomposition}\textup{(i)}.  Equivalently,
...
Calderon--Zygmund decomposition, then their gradients vanish in
\[
  L^1(I;H^{-1}(B_R)).
\]
\end{theorem}
...
\(L^2(I;L^6(B_{2R}))\) bounds.  Holder's inequality gives
\[
  \|U_n\otimes S_n\|_{L^1(I;L^2(B_R))}
  \le
  \|S_n\|_{L^\infty(I;L^3(B_R))}
...
\begin{lemma}[Critical-tightness failure routes to an exterior alternative]
\label{paper6a:lem:retained-tightness-routing}
Assume the retained compact single-core stratum: the branch \(V\) has
passed the ordered local tests of
\cref{paper7:lem:no-unrecorded-cost-escape} up to and including the local
state-space decomposition \cref{paper6:thm:state-decomposition}, with the
single-core alternative \textup{(i)} retained.  If the critical-tightness
condition of \cref{def:c5-compact-tests} fails, i.e.\ there exist
\(\varepsilon_0>0\) and sequences \(R_n\uparrow\infty\) and
\(\tau_n\ge\tau_0\) such that
\[
  \int_{|y|>R_n}|V(y,\tau_n)|^3\,dy\ge\varepsilon_0,
\]
then, after passing to a subsequence, the branch admits an exterior
concentration sequence on every annulus \(A_{r,R}\) with \(r\) sufficiently
large.  By \cref{paper8:thm:exterior-stratification} the branch then
belongs to one of the exterior-profile, separated-core, multicore,
noncompact-exterior, or scale-collapse alternatives.  All of these are
non-retained alternatives in
\cref{paper6:thm:state-decomposition}\textup{(ii),(iv)} and are excluded
from the retained single-core stratum.  Equivalently, on the retained
stratum critical tightness holds automatically.
\end{lemma}

\begin{proof}
Failure of critical tightness gives \(\varepsilon_0>0\), \(R_n\uparrow\infty\),
and \(\tau_n\ge\tau_0\) with
\[
  \int_{|y|>R_n}|V(y,\tau_n)|^3\,dy\ge\varepsilon_0
  \qquad\text{for all }n.
\]
The integrand \(|V|^3\,dy\) is scale-invariant under the parabolic
rescaling defining \(V\) from \(u\), so this lower bound is a positive
critical-mass-floor that, in the original physical coordinates centred
at \((x_*,T^*)\), records positive \(L^3\) mass on physical annuli
\(\{|x-x_*|>R_n e^{-\tau_n}\}\) at terminal times
\(t_n:=T^*-e^{-2\tau_n}\to T^*\).  Two cases occur after passing to a
subsequence.

\emph{Case A: bounded distance from the core.} Some \(R^*<\infty\) and
some \(r_0>0\) admit infinitely many \(n\) with
\[
  \int_{r_0<|y|<R^*}|V(y,\tau_n)|^3\,dy
  \ge\varepsilon_0/2.
\]
Suppose, towards contradiction, that there is no exterior concentration
on the slightly larger annulus
\(A_{r_0/2,2R^*}\) in the physical-coordinate sense of
\cref{paper8:def:exterior-concentration-branch}.  By
\cref{paper8:thm:exterior-regularity-no-concentration} (\(k=0\)), there
is then a uniform \(C^0\) bound for \(u\) on
\(A_{r_0,R^*}\) on a terminal time interval, hence by integration the
critical \(L^3\) mass on the corresponding renormalized annulus is
\(O(e^{-3\tau})\), contradicting the positive lower bound
\(\varepsilon_0/2\) for \(\tau_n\to\infty\).  Therefore
\cref{paper8:lem:exterior-test-dichotomy} forces the exterior
concentration alternative on \(A_{r_0/2,2R^*}\).

\emph{Case B: escape to infinity in the renormalized coordinate.}  For
every \(R<\infty\), \(\int_{r_0<|y|<R}|V(y,\tau_n)|^3\,dy<\varepsilon_0/2\)
infinitely often.  Then a subsequence puts at least \(\varepsilon_0/2\)
of its mass on shells \(\{|y|>R_k\}\) with \(R_k\uparrow\infty\),
producing a sequence of \(L^3\)-concentration centres
\(x_n\) with \(|x_n|/1\to\infty\) in the renormalized frame.  This is
the noncompact-exterior alternative of
\cref{paper8:thm:exterior-stratification}, which is one of the listed
exterior concentration alternatives.

In either case the dichotomy of
\cref{paper8:thm:exterior-stratification} selects the exterior
concentration branch.  By the second clause of that theorem the branch
is exterior profile, separated core, multicore, noncompact exterior, or
scale collapse.  Each is one of the alternatives
\textup{(ii)}--\textup{(iv)} of \cref{paper6:thm:state-decomposition}
and disqualifies the branch from the retained single-core alternative
\textup{(i)}.  This contradicts the retained-stratum hypothesis, so on
the retained stratum critical tightness holds.
\end{proof}

\begin{lemma}[Local windowed \(H^1\)-loss routes to the rough-core alternative]
\label{paper6a:lem:retained-h1-routing}
Assume the retained compact single-core stratum, with the standing
hypotheses of
\cref{paper7:prop:uniform-windowed-gradient}: a smooth repaired-gauge
representation, the local compact-cylinder critical bound on the outer
cylinder \(B_{2R}\), bounded modulation \(M_{ab}<\infty\), and the
pressure normalization of
\cref{paper7:lem:uniform-pressure-from-c3}.  These standing hypotheses
are part of the retained-stratum input package supplied by the ordered
tests in \cref{paper7:lem:no-unrecorded-cost-escape}\textup{(i),(ii),(iv),(vii)}.
...
\begin{proof}
The estimate is the conclusion of
\cref{paper7:prop:uniform-windowed-gradient}, applied with the standing
inputs of the retained stratum: the local critical bound \(M_{3,R}\) is
the outcome of the positive finite critical \(L^3\) annulus test
\cref{paper7:lem:no-unrecorded-cost-escape}\textup{(iv)}; the bounded
modulation \(M_{ab}\) is supplied by
\cref{paper7:lem:local-modulation-integrability}; the pressure
normalization is
\cref{paper7:lem:uniform-pressure-from-c3}; and the smooth repaired-gauge
representation is the output of
\cref{paper7:lem:no-unrecorded-cost-escape}\textup{(ii)}.  The
smoothness hypothesis of
\cref{paper7:prop:uniform-windowed-gradient} is the standard one used
throughout the local validation: the estimate is first proved for
spatial-temporal mollifications \(V^\varepsilon,P^\varepsilon\) and
passes to the suitable weak limit by lower semicontinuity of the
displayed gradient norm.  All four standing inputs are present on the
retained stratum by hypothesis.  Hence the windowed
\(H^1\) bound holds and the corresponding test of
\cref{def:c5-compact-tests} passes.

If, contrapositively, the windowed \(H^1\) bound fails for some \(R>0\)
along a terminal sequence, then by
\cref{paper7:prop:uniform-windowed-gradient} at least one of the
standing inputs must fail along that sequence (since the conjunction of
the standing inputs implies the bound).  By the ordered tests
...
representation, repaired-gauge, critical-mass, or modulation
alternative.  Independently, in the local state-space terminology of
...
\begin{proof}
Critical tightness holds by
\cref{paper6a:lem:retained-tightness-routing}: failure routes the branch
to the exterior-stratification alternatives, all non-retained.  Local
...
\begin{corollary}[Discharge of the retained compact-test closure assumption]
\label{paper6a:cor:compact-test-closure-discharged}
The retained compact-test closure obligation listed as item~3 of the
proof-obligation tracker is discharged on every retained compact
single-core branch produced by the ordered local validation procedure of
...
\subsection{Discharge of the Type II analytic-data exhaustiveness assumption}
\label{paper6a:subsec:discharge-typeII-analytic-data}

The exhaustion hypothesis of \cref{thm:c1-typeII-branch-exhaustion} is the
entry condition for every admissible local Type II branch.  It is the item
formerly listed as item~1 of the proof-obligation tracker.  We discharge it by
an ordered local extraction package: each component of
...
\cref{paper1:def:bounded-window-limit} and exits Type II by
...
compact cylinder; combined with the standing absence of nonzero bounded
selected-window limits, it is a local Type II sequence in the sense of
...
profile data via the local scale-translation selection and
\cref{paper1:prop:representation-input}, or routes the branch to the
...
\begin{corollary}[Discharge of the Type II analytic-data exhaustiveness assumption]
\label{paper6a:cor:typeII-analytic-data-exhaustiveness-discharged}
The ordered-routing exhaustion hypothesis used by
\cref{thm:c1-typeII-branch-exhaustion} is now a theorem on
\(\mathcal U_{\mathrm{II}}^{NS}\): item~1 of the proof-obligation tracker is
...
\textup{(i)} of
\cref{paper6a:ass:critical-ns-profile-decomposition} (the admissibility
...
\begin{corollary}[Local compactness from orbit sampling]
\label{paper6a:cor:boundedness-from-orbit-sampling}
In the presence of terminal orbit sampling, local repaired-gauge estimates, and
a positive finite local terminal \(L^3\) annulus, the local compact
terminal-window hypothesis in
\cref{paper6a:def:bounded-critical-terminal-sequences} follows from
\cref{paper6a:thm:bounded-terminal-sequences-from-l3}.
\end{corollary}

\begin{proof}
The theorem gives the required compact-cylinder bounds for every terminal
active window.
\end{proof}
...
construction, a positive finite local terminal \(L^3\) annulus for \(V\), local
Caccioppoli and pressure reconstruction estimates on compact terminal
cylinders, bounded modulation, and local perturbative residual stability.
Then the terminal local completeness statement in the final argument holds.
\end{corollary}

\begin{proof}
\Cref{paper6a:thm:representation-gives-terminal-orbit-sampling} gives terminal
orbit sampling.  Together with the positive finite local \(L^3\) annulus and
the local PDE estimates,
\cref{paper6a:thm:bounded-terminal-sequences-from-l3} gives local compact
terminal windows.  Local perturbative residual stability is supplied by
\cref{paper6a:cor:kato-gives-small-data-stability}.  The local adjacent labels
are routed by \cref{paper6a:thm:terminal-adjacent-label-routing}, and
\cref{paper6a:thm:critical-ns-profile-decomposition-discharged} gives the
terminal local completeness conclusion.
Therefore the terminal local compactness package of
...
\cref{paper6a:cor:absolute-scale-cost-criterion} and then excluded by the
...
is one of the costs covered by the cost-divergence theorem
\cref{paper6a:thm:cost-divergence-exclusion-discharged}.  Items \textup{(v)} and
...
completeness.  Exterior regular components are removed by hypothesis.
...
decomposition and the compact, multibubble, scale-collapse, and scale-rigidity
exclusions.  The cost-divergence criterion is supplied separately by the
cost-divergence theorem
...
\begin{theorem}[Expanded terminal hypotheses hold branchwise]
\label{paper6a:thm:expanded-terminal-hypotheses-branchwise}
Let \(\omega\) be a physical local Type II branch after
\cref{paper6:cor:covered-class-entry-discharged}.  Then either \(\omega\)
exits through a named local alternative already eliminated by the local
state-space packages, or the nine items of
...
earlier named exit.  In the retained case
\cref{paper6:thm:state-decomposition} gives the local alternative
decomposition.  The compact single-core, multibubble/cascade,
finite-cost scale-collapse, retained compact scale-collapse, and
scale-rigid alternatives are eliminated by
\cref{paper6:thm:single-core,paper6:thm:multibubble,paper6:thm:scale-cost,paper5:thm:compact-scale-collapse-rigidity-discharged,paper3:prop:scale-rigidity-discharged}.
\par
...
\cref{paper8:thm:exterior-stratification,paper6:thm:rough-core,paper6:prop:state-elimination}.
...
\cref{paper6a:thm:terminal-critical-local-compactness,paper6a:cor:terminal-compactness-from-explicit-hypotheses};
their adjacent labels are routed by
\cref{paper6a:thm:terminal-adjacent-label-routing}, whose carrier and cost rows
now use the ambient adjacent wrapper
\cref{paper7:cor:ambient-adjacent-wrapper-discharged}.
\par
\emph{Residual stability, representation, exterior removal, and local
regularity.}
Terminal state-space completeness is
\cref{paper8:ass:terminal-windowed-profile-completeness}, proved locally from
...
on the retained terminal tail or its first failure is one of the named local
alternatives already closed above.  The verification uses only local compact
windows, ordered state-space labels, suitable local energy, and compact
terminal estimates; it does not import a global profile theorem, global Kato
smallness, or a whole-space \(L^3\) bound.
\end{proof}

\begin{corollary}[Expanded terminal wrapper discharged]
\label{paper6a:cor:expanded-terminal-wrapper-discharged}
The expanded terminal hypotheses are no longer an active assumption for the
physical no-Type-II theorem: every physical local Type II branch either
satisfies them on a retained terminal tail or exits through a named local
alternative closed by the local packages.
\end{corollary}

\begin{proof}
This is \cref{paper6a:thm:expanded-terminal-hypotheses-branchwise}.
\end{proof}
...
\cref{paper3:ass:terminal-admissibility} and scale-rigidity is discharged by
...
the retained compact scale-collapse branch, and the remaining retained
scale-collapse state is removed by
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged}.
\end{theorem}
...
state is passed to the local scale-rigidity and compact scale-collapse routing
theorems.
\end{theorem}
...
scale collapse or one of the retained compact scale-collapse/scale-rigid
terminal states.  These cases exhaust the local alternatives.
\end{proof}
...
terminal state.  If this terminal state is velocity-virial dissipative,
...
The remaining retained compact scale-collapse states are excluded by
\cref{paper5:thm:compact-scale-collapse-rigidity-discharged}.  The remaining
scale-rigid terminal states are excluded by
\cref{paper3:prop:scale-rigidity-discharged}: the state either has a nonzero
bounded selected-window limit, which is incompatible with the local Type II
condition, or it reduces to the compact single-core branch, which is excluded
by \cref{paper6:thm:single-core}.  Hence it cannot be a local Type II branch.
\end{proof}
...
\cref{paper6:thm:state-decomposition}, unless it has already exited through one
...
a positive local Type II concentration sequence.  The decomposition theorem
\cref{paper6:thm:state-decomposition} is formulated exactly for such sequences:
after passing to subsequences and selected windows, the sequence enters one of
the five alternatives listed there.  This is covered entry in the sense of
...
The covered-class entry obligation for the no-Type-II theorem is discharged:
...
adjacent-stratum closure hypotheses or the expanded terminal hypotheses used in
...
closure of those named exits is supplied either by the retired local packages
cited in the tracker or by the ambient/terminal wrappers
...
\begin{theorem}[Unconditional local no-Type-II theorem]
\label{paper6:thm:unconditional-no-typeII}
Let \((u,p)\) be a suitable weak solution on
\(\mathbb R^3\times(T-\delta,T)\).  Then there is no physical local Type II
singular germ \((u,p,x_0,T)\) in the sense of
\cref{paper6:def:physical-local-typeII-germ}.  Equivalently, no singular point
at time \(T\) can lie in the local Type II alternative after the local
Type I/Type II separation.
\end{theorem}

\begin{proof}
Assume for contradiction that \((u,p,x_0,T)\) is a physical local Type II
singular germ.  By
...
\cref{paper6:lem:physical-typeII-germ-admissible}.  A named non-retained exit
is closed by the ordered local packages and, where carrier or cost rows are
involved, by the ambient adjacent wrapper
\cref{paper7:cor:ambient-adjacent-wrapper-discharged}.  If the branch reaches
the retained terminal path, the expanded terminal wrapper
\cref{paper6a:cor:expanded-terminal-wrapper-discharged} supplies the expanded
terminal hypotheses of \cref{paper6a:def:expanded-terminal-data}, and
\cref{paper6a:thm:expanded-terminal-typeII} excludes the branch.
\par
Equivalently, once the branch has entered the state-decomposition path,
\cref{paper6:thm:state-decomposition} lists the five possible local Type II
states and \cref{paper6:prop:state-elimination} eliminates all five.  The
previous paragraph records why no physical branch can avoid that list by an
untracked terminal or adjacent-row failure.  Hence every possible outcome of
the assumed physical Type II germ is impossible, a contradiction.
\end{proof}
...
\begin{proof}
\Cref{paper6:thm:unconditional-no-typeII} excludes physical local Type II
singular germs.  If a point \(x_0\in\Sigma(T)\) lay in the local Type II
alternative, then it would satisfy the three clauses of
...
\(\mathbb R^3\times(T-\delta,T)\).  This contradicts
\cref{paper6:thm:unconditional-no-typeII}.
\end{proof}
...
the local Type II class of alternatives contains no remaining singular branch.
\end{corollary}
...
Assume the Type II analytic data are exhaustive by ordered local routing.  Then
each admissible Type II branch enters the subsequent analysis in exactly one of
the following ways:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item it has already exited through a routed C1 concentration, scale, profile,
or outside-class failure.
\end{enumerate}
\end{theorem}

\begin{proof}
By \cref{thm:c1-typeII-branch-exhaustion}, an admissible Type II branch either
has the concentration, scale, and profile data or has already exited through a
routed C1 failure.  In the latter case item \textup{(iii)} holds.  In the
former case the representation step either gives a repaired-gauge
representation or one of the ordered representation failures in
\cref{def:c2-representation-failures}.  If a repaired-gauge orbit exists, the
critical \(L^3\) test gives either the positive finite annulus or one of the
ordered critical-mass alternatives in \cref{def:c3-critical-mass-alternatives}.
If the positive finite annulus holds, the remaining compact cost-divergence
inputs are critical tightness and local windowed \(H^1\) control.  Their
failures are exactly the two local failures in \cref{def:c5-compact-tests}.  If
both hold, the compact Type II cost-divergence criterion applies.
\end{proof}
...
\cref{def:c2r-minimal-failures}, one of the critical-mass alternatives
\cref{def:c3-critical-mass-alternatives}\,\textup{(i)}--\textup{(iii)}, or a
...
On that repaired-gauge branch, apply the critical \(L^3\)-annulus
classification \cref{thm:c3-l3-annulus-classification}.  Alternatives
\textup{(i)}--\textup{(iii)} of
\cref{def:c3-critical-mass-alternatives} are routed pre-compact-data exits.  In
the remaining case the positive finite critical annulus
\cref{def:c3-critical-mass} holds on a terminal tail.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item a routed pre-compact-data exit;
...
\begin{proof}
Fix an admissible Type II branch.  By
\cref{lem:c5-precompact-data-routing}, either the branch has a routed
pre-compact-data exit or it has compact Type II data.  In the first case item
\textup{(i)} holds.  In the second case only the two compact cost-divergence
tests remain.

Evaluate critical tightness.  If tightness fails, the failure-of-tightness
alternative holds.  If tightness holds, evaluate local windowed \(H^1\) control.
If the windowed \(H^1\) condition fails, the windowed-control alternative holds.
If it holds, \cref{paper7:thm:classwise-retained-compact-coverage} places the
branch either in the retained compact cost-divergence stratum or in a named
adjacent exit stratum.  In the retained case,
...
finite-cost admissible Type II branch that is not in a routed
pre-compact-data exit and is not excluded by the retained compact
cost-divergence argument or by the treated adjacent-stratum alternatives is
either non-tight or fails local windowed \(H^1\) control.
\end{corollary}

\begin{proof}
Apply \cref{thm:c5-exclusion-or-two-alternative}.  Since the branch is not in a
routed pre-compact-data exit, item \textup{(i)} is unavailable.  Since it is not
excluded by retained compact cost-divergence or by the treated adjacent-stratum
alternatives, item \textup{(ii)} is unavailable.  Therefore the ordered
alternative is failure of critical tightness or failure of local windowed
\(H^1\) control.  The finite-cost assumption records that the resulting branch
remains finite-cost.
\end{proof}
...
named thin-drift, modulation, compactness, exterior, rough-core, or multicore
local exits.

Thus, under the analytic inputs and local routing hypotheses made explicit in
the text, every physical local Type II concentration branch is eliminated.
The final local dichotomy is that a singular point must belong to the local
Type I alternative once the Type II branch has been removed.  The contribution
is an unconditional no-Type-II theorem within the local state-space framework
developed here; it is not an unconditional resolution of the Navier--Stokes
regularity problem, since the Type I side is a separate problem.
