...
nonzero constant gives a bounded selected-window limit, which exits the
retained local Type II row.  Pressure-only harmonic concentration is routed to
...
\(\int |Y|^{-p-1}|V|^3<\infty\), while the cutoff term is supported on a compact
annulus away from the origin and is controlled by local \(L^3\). The translation
components are supported in \(B_{2R}\)
