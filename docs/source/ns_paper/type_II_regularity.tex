...
This is a branch condition inside the selected-window state-space analysis.  It
is not the physical definition of a local Type II singular germ.  Physical local
Type II germs are defined later in
\cref{paper6:def:physical-local-typeII-germ}.  If a selected branch has a
nonzero bounded selected-window limit, it leaves the strict selected-window
branch and is handled by the bounded-window physical routing theorem rather
than being declared incompatible with physical Type II by definition.
\end{definition}
...
\begin{lemma}[Weighted translation differentiability]
\label{paper2:lem:weighted-translation-differentiability}
Let \(0<p<3\), let \(\chi_R\in C_c^\infty(B_{2R})\), and let
\(V\in\mathcal A_{p,R}\). Then the map
\[
  c\mapsto \int \chi_R(Y)|Y|^{-p}|V(c+Y)|^3\,dY
\]
is differentiable at \(c=0\), and
\[
  \partial_{c_k}
  \int \chi_R|Y|^{-p}|V(c+Y)|^3\,dY\bigg|_{c=0}
  =
  p\int \chi_RY_k|Y|^{-p-2}|V|^3\,dY
  -\int(\partial_k\chi_R)|Y|^{-p}|V|^3\,dY .
\]
\end{lemma}

\begin{proof}
First assume \(V\) is smooth and compactly supported in \(B_{4R}\).  Then
differentiation under the integral sign is legitimate and
\[
  \partial_{c_k}\int \chi_R|Y|^{-p}|V(c+Y)|^3\,dY\bigg|_{c=0}
  =3\int \chi_R|Y|^{-p}|V|\,V\cdot\partial_kV\,dY.
\]
Integrating by parts gives the displayed identity.

For general \(V\in\mathcal A_{p,R}\), mollify \(V\) inside \(B_{4R}\) and
apply the smooth identity to \(V^\varepsilon\).  The defining weighted bounds
\[
  \int |Y|^{-p}|V|^3\,dY<\infty,\qquad
  \int |Y|^{-p-1}|V|^3\,dY<\infty
\]
control the singular part, while the cutoff derivative term is supported on a
compact annulus away from the origin and is controlled by local \(L^3\).
Hence both right-hand side integrals converge as \(\varepsilon\downarrow0\).
The translated integrals converge locally uniformly for \(c\) near \(0\) under
the same weighted bounds, so the derivatives converge to the claimed formula.
\end{proof}
...
the scale component is supplied by
\cref{paper2:lem:weighted-translation-differentiability}. The translation
components are supported in \(B_{2R}\)
and are controlled by \(V\in L^2_{\rm loc}\) and \(\nabla V\in L^3_{\rm loc}\).
These bounds give an integrable majorant for the remaining nonsingular
difference quotients, so dominated convergence applies there.
\[
\mathcal F_0(\lambda,c;V)
...
\begin{lemma}[Bounded selected-window limits exit the strict selected-window branch]
...
\[
  ...
  \qquad\text{strongly in }L^3(Q_r).
\]
Then the branch is not a strict selected-window Type II branch in the sense of
...
\cref{paper1:def:bounded-window-limit}.  By
\cref{paper1:def:typeII}, such a branch is outside the strict selected-window
Type II branch.  This conclusion is only a selected-window branch statement.
For a physical local Type II germ, the bounded-window branch is handled by
\cref{paper6:thm:bounded-selected-window-physical-routing}.
\end{proof}
...
is item \textup{(i)}.  If the resulting carrier is still pressure-supported,
the same ordered local pressure test is rerun on the smaller nested cylinder;
no part of the pressure-only closure is used here.
\par
...
pressure-supported subcase is sent back to the same nested local pressure test.
A regular annular source is removable and cannot be an independent
pressure-only exterior exit.  An escaping dyadic stress occupation state is tested by
...
It remains to control the pressure oscillation.  Use the local pressure
decomposition on \(B_{2\rho}(y_0)\):
\[
  P_n=P^{\rm loc}_n+H_{n,\rho}+c_n(s).
\]
The local part \(P^{\rm loc}_n\) is generated by the localized tensor
\(V_n\otimes V_n\); since \(V_n\to V_*\) strongly in \(L^3\), the localized
pressures converge strongly in \(L^{3/2}\), and their scaled oscillation is
small after choosing \(\rho\) sufficiently small.  The harmonic part satisfies
the standard interior harmonic decay estimate after subtracting spatial means:
for \(0<\theta<1\),
\[
  D_{H_{n,\rho}}(\theta\rho;z_0)
  \le C\theta^\alpha D_{H_{n,\rho}}(\rho;z_0),
\]
with constants independent of \(n\), using the compact-cylinder pressure
bound.  Choose \(\theta\) small and then \(n\) large so that
\[
  C_{V_n}(\theta\rho;z_0)+D_{P_n}(\theta\rho;z_0)
  <\varepsilon_{\rm rg}.
\]
The represented-gauge CKN regularity lemma then gives uniform boundedness of
\(V_n\) on a smaller cylinder around \(z_0\).
...
state-space tests are restarted on the first remaining positive CKN carrier.
By construction of the ordered tests, that carrier is one of the already named
rows: smaller active core, exterior core, pressure-only carrier,
multibubble/cascade, gauge-degenerate branch, rough-core loss, or a later
terminal retained branch.  Therefore the bounded-window case is a routing row
into the existing state-space ledger and cannot survive as an independent
...
\cref{paper6:thm:bounded-selected-window-physical-routing}; the autonomous-modulation row by
...
\cref{paper6:thm:bounded-selected-window-physical-routing}; the autonomous-modulation row
