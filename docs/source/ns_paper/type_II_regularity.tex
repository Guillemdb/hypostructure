...
\begin{proof}
First assume \(V\) is smooth.  The displayed identities follow from the chain
rule, rewriting the resulting \(x\)-derivatives as \(Y\)-derivatives, and then
integrating by parts in the \(Y\)-variable.  The
...
\ge \eta_*.
\]
For part \textup{(b)}, choose a nonnegative cutoff \(\phi\in C_c^\infty(Q')\)
that equals \(1\) on a slightly smaller compact subcylinder \(Q''\Subset Q'\).
Then
\[
\int \phi|V_n|^3\,dy\,ds\ge \int_{Q''}|V_n|^3\,dy\,ds,
\]
so any occupation-state or Young-state limit carries a positive local mass
coordinate after shrinking once if necessary.  Part \textup{(c)} is exactly how
the later compactness and routing arguments use the retained floor: they need a
nonvanishing spacetime activity signal on some compact window, not an a.e.-time
\(L^2\) persistence statement.
\end{proof}
...
If a support profile has nonzero local Calder\'on--Zygmund source, then
some compact test in the pressure-defect occupation chart detects a positive
velocity-stress level.  By the dyadic activity ladder, this is recorded at a
positive local level \(\eta_m>0\).  The fixed-annular stress dichotomy and the
local CKN test then produce either a regular annular source or a velocity
carrier at that positive level.  The regular source is absorbed by the local
pressure atlas; the carrier case contradicts pressure-only terminality.

If the support profile is harmonic and remains non-affine after quotienting by
time-dependent constants and every admissible local accelerating rebasement,
then the far-field harmonic-tail row occurs.

If the realized harmonic profile is spatially constant, it is removed by the
ordinary pressure gauge.  If it is spatially affine with nonzero linear part,
then \cref{paper8:lem:affine-harmonic-pressure-absorption} either absorbs that
affine forcing into the repaired accelerating-center modulation on a smaller
local cylinder, reducing the harmonic remainder to a time-gauge term, or else
the branch enters the affine-pressure/modulation failure row.  Thus no terminal
pressure-only branch can be supported by an untreated affine harmonic profile.

If the pressure gauge, pressure atlas, pressure reconstruction, compact upper
estimates, local energy structure, or rough-core control needed to make one of
these conclusions fails, the branch is exactly the corresponding named failure
row.  Thus no diffuse pressure-only defect is terminal.
\end{proof}
...
compactness used throughout the bounded-window analysis once one records
failure faces for loss of represented structure, compact-upper control, local
energy, pressure atlas, gauge chart, and rough-core breakdown.  Hence the
normalized tree measures are tight and possess weak-* limits.
...
of descendants produces a fixed compact local carrier in the original
reselection branch.
...
\item a compact local carrier, contradicting the no-fixed-carrier case;
...
If the realized descendant centers and scales leave every compact relative
scale-center chart, then the ordered relative scale-center partition gives
scale-collapse or noncompact exterior behavior.  If they remain at the same
physical point but form a strict infinite nested chain, the same-point cascade
row occurs.  This is alternative \textup{(ii)}.

If some realized support point carries positive compact local activity on a
fixed model cylinder, then the support-realization lemma pulls this activity
back to a fixed local carrier in the original reselection branch.  This
contradicts the diffuse/no-fixed-carrier case.  This is alternative
\textup{(iii)}.

It remains to consider the genuinely diffuse case: the weak-* limit is nonzero,
but no support point gives a fixed compact carrier and no failure face or
scale-center boundary row has occurred.  The realized descendant states are
then genuine local suitable states with the pressure/gauge/local-energy
interfaces intact.  Their positive normalized defect is visible in the
augmented local occupation-state coordinates used in
\Cref{paper6a:thm:local-residual-exhaustion}.  Applying that theorem gives a
named adjacent local row, unless the defect is a retained comparable class.
The retained comparable class is impossible here, because comparable retained
classes are grouped before the bounded-window row is declared and before the
reselection tree is closed.  Hence only named adjacent rows remain.
...
every realized support profile is diffuse.  If every realized support profile
is diffuse and no fixed compact carrier is produced, then
\Cref{paper10:lem:diffuse-reselection-tree-row-closed} applies.  It routes the
diffuse reselection tree either to a named failure face, to a scale-center
boundary row, or to a visible residual defect in the augmented local
occupation-state coordinates.  Hence the infinite diffuse reselection tree is
not terminal.
\end{proof}
