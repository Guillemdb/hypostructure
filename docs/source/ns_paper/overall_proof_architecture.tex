...
This should be read in the hybrid sense used by the papers themselves.  The architecture is assertive about the intended proof flow, but it is explicit about the inputs on which each branch depends.  The Type I residual closure is now discharged by \path|paperIV_residual_branch.tex|, including the setup residual hypothesis.  In \path|type_II_regularity.tex|, several items that used to be treated as strategic assumptions have also been converted into theorem or corollary packages: \path|paper3:prop:local-scale-rigidity-exclusion|, \path|paper7:cor:windowwise-transition-routing-holds|, \path|paper7:cor:windowwise-positive-scale-routing-holds|, \path|paper6a:thm:cost-divergence-exclusion-discharged|, and \path|paper6a:thm:critical-ns-profile-decomposition-discharged|.  The current status is that the Type II exit ledger is closed: entry into the local alternative class, repaired-gauge representation, compactness and local energy propagation, retained compact-test closure, cost/carrier routing, and terminal profile exhaustion are all supplied by named theorem packages in \path|type_II_regularity.tex|.
...
\begin{itemize}
  ...
  \item Retained obstruction:
  \item I. The retained compact-cost token before carrier-component routing.
  \item Failure routes:
...
\begin{itemize}
  ...
  \item Retained obstruction:
  \item I. The retained compact-cost token on the canonical moving schedule.
  \item Failure routes:
  \item F1. If the corrected-monotonicity hypotheses are not available, reroute to K1 rather than invoking the dichotomy.
  \item F2. If cost divergence has not yet been validated on the active branch, remain in the upstream compact-cost validation layer rather than entering K2.
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
...
\begin{itemize}
  ...
  \item Retained obstruction:
  \item I. The carrier cost token that persists until all named exits are closed.
  \item Failure routes:
  \item F1. If a named carrier exit is not yet closed, reroute to the corresponding TII8c* capsule instead of invoking K\_final.
  \item F2. If adjacent-stratum closure is unavailable, remain in the ambient Type II closure package rather than claiming final cost-divergence exclusion.
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
...
\begin{itemize}
  ...
  \item Every implementation label cited above has been verified to occur
      ...
      \path|type_II_regularity.tex|) at the time of writing.  One obsolete
      terminal nonactive-exclusion placeholder from earlier diagram drafts
      has been corrected to the actual label \texttt{thm:terminal-inactive-exclusion}; this
