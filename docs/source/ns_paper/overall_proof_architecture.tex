...
\title{Overall Proof Architecture for the Navier--Stokes State-Space Stratification}
\author{}
\date{}
...
This document explains how the three papers in this folder fit together as a single proof architecture for routing and excluding finite-time singularity scenarios for the three-dimensional incompressible Navier--Stokes equations. The organizing principle is not to guess the shape of a singularity.  The organizing principle is to force every possible singular branch into an explicit state-space stratum, then eliminate that stratum by the tool that is strongest for exactly that geometry.
...
This should be read in the hybrid sense used by the papers themselves.  The architecture is assertive about the intended proof flow, but it is explicit about the inputs on which each branch depends.  The Type I residual closure is now discharged by \path|paperIV_residual_branch.tex|, including the setup residual hypothesis.  In \path|type_II_regularity.tex|, several items that used to be treated as strategic assumptions have also been converted into theorem packages: \path|paper3:prop:local-scale-rigidity-exclusion|, \path|paper7:def:windowwise-transition-routing|, \path|paper7:def:windowwise-positive-scale-routing|, \path|paper6a:thm:cost-divergence-exclusion-discharged|, and \path|paper6a:thm:critical-ns-profile-decomposition-discharged|.  The current status is that the Type II exit ledger is closed: entry into the local alternative class, repaired-gauge representation, compactness and local energy propagation, retained compact-test closure, cost/carrier routing, and terminal profile exhaustion are all supplied by named theorem packages in \path|type_II_regularity.tex|.

\section{Current Closure Status}

All exits in the three-paper state space are now closed.

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Branch or interface} & \textbf{Current status} & \textbf{Closing theorem package} \\
\midrule
\endhead
...
Repaired-gauge representation and pressure/local-energy compatibility & Closed. & \path|paper6:thm:representation|, \path|thm:c2-representation-implication|, \path|thm:c2r-minimal-representation|, \path|paper6a:thm:ac-repaired-gauge-consequences|. \\
...
Terminal hidden critical profile and residual mass exits & Closed. & \path|paper6a:thm:critical-ns-profile-decomposition-discharged|, \path|paper6a:thm:retained-compact-tests-closed|, \path|paper6a:thm:terminal-critical-local-compactness|, \path|paper6a:thm:expanded-terminal-typeII|. \\
Final Type II assembly & Closed. & \path|paper6:thm:state-decomposition|, \path|paper6:thm:unconditional-no-typeII|, \path|paper6:thm:local-typeII-exclusion|. \\
\bottomrule
\end{longtable}

The final proof is therefore no longer organized around a list of outstanding strategic assumptions.  It is organized around a finite routing graph whose terminal nodes are all contradictions: either local regularity contradicts the choice of singular point, a Liouville theorem contradicts retained Type I mass, the residual state space is empty, or a Type II exit contradicts retained core mass, finite cost, scale-rigidity closure, or terminal profile exhaustion.

\section{Complete Exit and Rerouting Ledger}
\label{sec:complete-exit-ledger}

The tables in this section are the referee-facing ledger for the whole proof. Each row records an exit, the routing test that sends a branch there, whether the exit terminates directly or reroutes to another exit, and the closure mechanism.  The entries are intentionally more explicit than the diagrams so that a reader can check the proof linearly.
...
Direct Type I class exhaustion failure & A profile avoids all direct classes. & This is not a terminal gap; it is exactly the residual class. & Routed to the residual ledger by \path|p1:thm:remaining-criterion|. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Imported residual object mismatch & The residual paper would not be working with the same Seregin object produced by setup. & The imported setup package fixes the object, retained mass, pressure gauge, and ancestry. & \path|thm:imported-setup-results| and \path|hyp:base-seregin-hypotheses| align the interface. \\
...
Rotational relative equilibrium & The residual profile is stationary modulo rotation and carries local Coriolis transport structure. & Routed through local Coriolis flux and terminal concentration; lower compact profiles are sent to already closed strata. & \path|thm:terminal-stratification| and the rotational routing package close the exit. \\
...
Repaired-gauge representation failure & A retained core cannot be put into a stable scale-center chart. & Routed to minimal representation, multibubble, cascade, or scale-rigid alternatives. & \path|paper6:thm:representation|, \path|thm:c2-representation-implication|, \path|thm:c2r-minimal-representation|. \\
Pressure or local-energy incompatibility in repaired variables & The transformed branch might fail to remain suitable or pressure-stable. & Routed through pressure decomposition, AC repaired-gauge consequences, and local Caccioppoli estimates. & \path|paper2:thm:pressure-decomp|, \path|paper2:thm:A|, \path|paper2:thm:B|, \path|paper2:thm:C|, \path|paper6a:thm:ac-repaired-gauge-consequences|. \\
...
Collapse core-floor failure & A collapsing branch would fail to retain a nonzero localized core. & Routed to vanishing critical \(L^3\) or terminal compactness closure. & \path|paper5:thm:collapse-core-floor|, \path|paper6a:thm:collapse-core-floor|, \path|paper6a:lem:typeII-concentration-extraction-routes|. \\
...
Autonomous-modulation alternative & Thick windows exist but modulation fails to converge cleanly. & Routed to autonomous reduced-limit extraction or compactness failure. & \path|paper5:thm:autonomous-modulation-selection|, \path|paper5:thm:autonomous-limit|, \path|paper5:lem:autonomous-scale-collapse-is-scale-rigid|. \\
...
Scale-rigid stratum decomposition & Scale-collapse reaches a scale-rigid terminal state but its subtype is not yet identified. & Routed into the finite scale-rigid sub-strata and then to virial, compact-core, or multibubble closures. & \path|paper3:thm:scale-rigid-stratum-decomposition| and \path|paper3:prop:scale-rigidity-discharged|. \\
Critical tightness failure & Critical mass tries to escape outside every compact window. & Routed through non-tight exclusion and tail windowed control; surviving mass re-enters terminal or exterior routing. & \path|paper7:thm:critical-tightness-criteria|. \\
...
Final unassigned Type II state & A branch has passed through all earlier tests without terminal contradiction. & The state decomposition has no remaining open target; the final assembly closes the branch. & \path|paper6:thm:state-decomposition|, \path|paper6a:thm:abstract-final-typeII|, \path|paper6a:thm:expanded-terminal-typeII|, \path|paper6:thm:unconditional-no-typeII|, \path|paper6:thm:local-typeII-exclusion|. \\
\bottomrule
\end{longtable}

\section{Referee Navigation Guide}

This document is meant to be a map, not a substitute for checking the proofs. A referee can use it to track three questions through the papers:
...
\item Which theorem closes that alternative, and what retained quantity gives the
\end{enumerate}
...
5 & \path|type_II_regularity.tex|, cost/carrier and terminal profile sections. & Scale-rigidity, retained compact cost coverage, windowwise carrier routing, cost-divergence exclusion, and terminal critical-profile decomposition are proved under the hypotheses under which the assembly invokes them. \\
...
The proof has three interface contracts.  These are the main places where a reader should check that no hypothesis is lost between papers.
...
For quick tracking, the proof should be read as a chain of retained obstructions:
...
Residual ancestry & Imported setup package in \path|paperIV_residual_branch.tex|. & Recentered limits, tail compactifications, active descendants, and terminal profiles. & A closed auxiliary descendant is meaningful only because it is realized by the original residual branch. \\
...
Canonical carrier cost & Retained compact-cost validation in \path|type_II_regularity.tex|. & Corrected monotonicity and windowwise \(J_1,J_6,J_7\) routing. & Divergent compatible cost cannot remain a free terminal branch; it enters transition/exterior, negative-drift, or positive-scale/scale-rigid closures. \\
\bottomrule
\end{longtable}

The most useful cross-check is this: every edge in the decision diagrams below should correspond either to a named theorem in the register at the end of this document or to a cited external theorem explicitly imported in one of the three papers.  Every terminal node should say what contradiction is obtained and which retained obstruction supplies it.

\section{Latest Strategy Refinement}

The current strategy is sharper than the original "prove one large missing local theorem" picture.  The proof now proceeds by reducing each hard interface to a small, named theorem package.

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.64\linewidth}@{}}
\toprule
\textbf{Former strategic bottleneck} & \textbf{Current closed treatment} \\
\midrule
\endhead
...
The main discipline for checking the closed proof is therefore not to invoke these packages as black boxes outside their hypotheses.  Cost-divergence is tied to the canonical windowwise moving schedule; carrier routing is tied to normalized carrier states rather than pointwise lower bounds; and the terminal profile theorem is tied to the \(L^3_\sigma\) critical profile framework and its Kato perturbative stability.

\section{State-Space Decision Trees}
\label{sec:state-space-trees}

The diagrams below give the state-space stratification as a theorem-labelled decision procedure. They are rendered natively in LaTeX/TikZ.  They are intentionally redundant with the tables: the tables say what each layer means, while the diagrams show how a candidate branch moves through the proof.
...
\begin{tikzpicture}[archnode/.style={draw, rounded corners, align=center, text width=0.27\textwidth, minimum height=7mm}, decision/.style={draw, diamond, aspect=2.1, align=center, text width=0.22\textwidth}, arr/.style={-{Latex[length=2mm]}, thick}, node distance=8mm and 10mm]
\node[archnode] (A) {Finite-energy suitable weak solution};
\node[archnode, below=of A] (B) {Assume terminal singular point $z_*$};
\node[decision, below=of B] (C) {Critical CKN or velocity concentration?};
\node[archnode, left=of C, xshift=-18mm] (C0) {Regularity: \path|p0:thm:ckn|, \path|p0:thm:velocity-ckn|, \path|p0:thm:no-concentration|};
\node[archnode, below=of C] (C1) {Positive concentration: \path|p0:thm:singular-positive|, \path|p0:cor:velocity-concentration|};
\node[decision, below=of C1] (D) {Local pointwise Type I envelope?};
\node[archnode, left=of D, xshift=-18mm] (I0) {Type I concentration sequence: \path|p0:thm:local-weak-serrin-typeI|};
\node[archnode, right=of D, xshift=18mm] (II0) {Local Type II branch: \path|p0:thm:typeI-typeII-dichotomy|, \path|paper6:thm:paper0|};
\node[archnode, below=of I0] (I1) {Seregin extraction: \path|p1:thm:extraction|};
\node[decision, below=of I1] (I2) {Direct Liouville class?};
\node[archnode, below left=of I2, xshift=-11mm] (I3) {Small or stationary: \path|p1:thm:classical-classes|, \path|p2:thm:NRS|};
\node[archnode, below=of I2] (I4) {Tight: \path|p1:thm:tight-liouville|, \path|p2:thm:tight-liouville|};
\node[archnode, below right=of I2, xshift=11mm] (I5) {Structured or decay: \path|p1:thm:known-structure-decay-liouville|, \path|p3:thm:typeI-zero-hypotheses|};
\node[archnode, right=of I2, xshift=24mm] (R0) {Paper IV residual tree};
\node[archnode, below=of R0] (RX) {Residual empty: \path|thm:paperIV-residual-closure|};
\node[archnode, below=26mm of I4] (IX) {Contradiction with retained nonzero Seregin mass};
\node[archnode, below=of II0] (T0) {Type II state-space tree};
\node[archnode, below=of T0] (TX) {No Type II branch remains: \path|paper6:thm:local-typeII-exclusion|};
\node[archnode, below=of IX, text width=0.24\textwidth] (END) {No singular point};
\draw[arr] (A)--(B); \draw[arr] (B)--(C); \draw[arr] (C)--node[above]{No}(C0); \draw[arr] (C)--node[right]{Yes}(C1); \draw[arr] (C1)--(D);
\draw[arr] (D)--node[above]{Yes}(I0); \draw[arr] (D)--node[above]{No}(II0); \draw[arr] (I0)--(I1); \draw[arr] (I1)--(I2);
\draw[arr] (I2)--node[left]{Small or stationary}(I3); \draw[arr] (I2)--node[right]{Tight}(I4); \draw[arr] (I2)--node[right]{Structured or decay}(I5); \draw[arr] (I2)--node[above]{Residual}(R0);
\draw[arr] (I3)--(IX); \draw[arr] (I4)--(IX); \draw[arr] (I5)--(IX); \draw[arr] (R0)--(RX); \draw[arr] (RX)--(IX); \draw[arr] (II0)--(T0); \draw[arr] (T0)--(TX); \draw[arr] (C0)|-(END); \draw[arr] (IX)--(END); \draw[arr] (TX)|-(END);
\end{tikzpicture}
\end{adjustbox}
...
\scriptsize
\begin{adjustbox}{max width=\linewidth,max totalheight=.82\textheight,center}
\begin{tikzpicture}[archnode/.style={draw, rounded corners, align=center, text width=0.30\textwidth, minimum height=7mm}, decision/.style={draw, diamond, aspect=2, align=center, text width=0.24\textwidth}, arr/.style={-{Latex[length=2mm]}, thick}, node distance=7mm and 9mm]
\node[archnode] (R) {Residual normalized Seregin profile};
\node[decision, below=of R] (R1) {Axisymmetric bounded circulation?};
\node[archnode, left=of R1, xshift=-17mm] (R1x) {\path|thm:R1-exclusion|};
\node[decision, below=of R1] (R2) {Rotational relative equilibrium?};
...
\node[decision, below=of R2] (R3) {Degenerate stationary hull?};
...
\node[decision, below=of R3] (R4) {Affine or parasitic quotient?};
\node[archnode, left=of R4, xshift=-17mm] (R4x) {\path|thm:oscillatory-entry-normalization|};
\node[decision, below=of R4] (R5) {Log-diffuse or Young critical tail?};
...
\node[decision, below=of R5x] (R6) {Generic terminal residual?};
...
\foreach \a/\b in {R/R1,R1/R2,R2/R3,R3/R4,R4/R5,R5c/R5x,R5x/R6,R6/R6a,R6a/R6b,R6b/R6c,R6c/R6d,R6d/DONE,DONE/FINAL}{\draw[arr] (\a)--(\b);}
...
This is the expanded residual branch, matching the internal organization of \path|paperIV_residual_branch.tex|.  The point is that the residual class is not closed by one theorem in isolation: it is routed through lower strata, tail-defect compactification, terminal concentration, descendant heredity, and endpoint \(L^3\) closure.
...
\node[archnode, left=of D, xshift=-35mm] (LOW) {Lower residual closures: R1 axisymmetric \(\to\) \path|thm:R1-exclusion|; R2 rotational \(\to\) \path|thm:terminal-stratification|; R3 stationary hull \(\to\) \path|thm:R3-no-attainable-degenerate-family|, \path|thm:R3S-terminal-theorem|, \path|thm:R3S-no-active-stationary-carrier|; affine/parasitic quotient \(\to\) \path|thm:affine-normalization-dichotomy|, \path|thm:oscillatory-entry-normalization|.};
...
\foreach \a/\b in {A/B,B/C,C/D,D/TAIL,TAIL/CT,CT/CT2,CT2/GEN,GEN/G0,G0/TC,TC/TC1,TC1/AB,AB/CLOSE,CLOSE/FINALR}{\draw[arr] (\a)--(\b);}
\draw[arr] (D)--node[above]{Yes}(LOW); \draw[arr] (LOW)|-(CLOSE); \draw[arr] (TAIL)--node[above]{Log-diffuse}(L1); \draw[arr] (TAIL)--node[above]{Young}(Y1); \draw[arr] (L1)|-(CT); \draw[arr] (Y1)|-(CT); \draw[arr] (CT2)--node[above]{Yes}(CT3); \draw[arr] (CT3)|-(CLOSE); \draw[arr] (TC1)--(NON); \draw[arr] (NON)|-(CLOSE); \draw[arr] (TC1)--(ACT); \draw[arr] (ACT)|-(AB);
\end{tikzpicture}
\end{adjustbox}
...
\node[archnode, below=of T11] (CLOSED) {State eliminated};
...
\foreach \a/\b in {T/T1,T1/T2,T2/T3,T3/T4,T4/T5,T5/T6,T6/T7,T7/T8,T8/T10,T10/T11,T11/CLOSED,CLOSED/FINALII}{\draw[arr] (\a)--(\b);}
...
\draw[arr] (K)--(K1); \draw[arr] (K1)--node[above]{Yes}(K2); \draw[arr] (K1)--node[above]{No}(K3); \draw[arr] (K2)--(K4); \draw[arr] (K4)-|node[near start,above]{Yes}(K3); \draw[arr] (K3)--(K5);
\draw[arr] (K5)--node[left]{$J_1$}(K6); \draw[arr] (K5)--node[right]{$J_6$}(K7); \draw[arr] (K5)--node[right]{$J_7$}(K8); \draw[arr] (K6)--(K12); \draw[arr] (K7)--(K12); \draw[arr] (K8)--(K12);
\end{tikzpicture}
\end{adjustbox}
...
The diagrams above should be read as theorem interfaces, not just pictures. Every edge has a payload: a property that must be proved before the target node can be used.  The tables below list those properties explicitly.

\subsection{Global Routing Edges}

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Edge} & \textbf{What must be verified} & \textbf{Why the edge is valid} \\
\midrule
\endhead
Finite-energy suitable weak solution to terminal singular point & The object is a suitable weak solution on a terminal time interval, with the local energy inequality and pressure normalization available. & This is the counterexample setup for \path|p0:thm:ckn|, \path|p0:thm:velocity-ckn|, and the local concentration package. \\
...
Concentration test to positive concentration & There is a sequence of shrinking cylinders carrying a fixed positive critical amount of velocity-pressure or velocity mass. & \path|p0:thm:singular-positive| and \path|p0:cor:velocity-concentration| supply the retained mass token used later. \\
...
Rotational residual to terminal package & The relative-equilibrium structure must produce the local Coriolis flux identity and a terminal concentration recentering. & \path|thm:terminal-stratification| routes the branch to lower closed terminal alternatives. \\
...
Single terminal profile to endpoint \(L^3\) closure & Parasitic-free mildness and sequence-\(L^3\) completeness must be manufactured from the previous exclusions. & \path|thm:mildness-inheritance-main| and \path|thm:AB-main| force zero, contradicting retained mass. \\
\bottomrule
\end{longtable}
...
\section{What Is Novel or Interesting to a PDE Expert}

The proof architecture has several features that are not just organizational. They change where the hard PDE estimates are used.
...
For a PDE reader, the most important structural point is that the proof tries to replace a single impossible classification theorem by many smaller rigidity statements whose hypotheses are produced by the preceding routing step.  The interesting work is therefore not only in the terminal contradictions; it is in showing that the routing is exhaustive and that descendants remain tied to the original singularity.

\section{Strategy-Level Portability Beyond This Particular Proof}

The architecture can be evaluated at two levels.  At the proof level, every estimate, compactness passage, pressure reconstruction, and Liouville invocation must be correct for the three-dimensional Navier--Stokes equations. At the strategy level, many of the ideas are more general: they describe how to organize a hard PDE regularity problem even if a particular implementation in 3D Navier--Stokes later has to be repaired or replaced.

The portable lesson is not "copy these strata verbatim."  The portable lesson is "find the invariant observables, route every possible failure mode, and use only the rigidity theorem justified by the stratum that has actually been reached."

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Strategy-level item} & \textbf{Generality as a PDE technique} & \textbf{What changes in another PDE} \\
\midrule
\endhead
...
This strategy-level viewpoint remains useful even if one later finds that a specific estimate in the 3D Navier--Stokes implementation is wrong.  A failed estimate would not invalidate the architectural lesson automatically; it would identify one of three things:

\begin{enumerate}
\item the proposed stratum is not actually closed by the stated tool;
\item the routing into that stratum is not exhaustive;
\item the observable used to define the stratum is not stable enough under the
\end{enumerate}

relevant compactness or gauge operation.

Those are useful failure modes.  They tell a PDE analyst where the proof must be repaired: strengthen the observable, split the stratum further, prove a missing rigidity theorem, or accept that the branch is a genuine remaining obstruction.  In this sense, the architecture is valuable not only as a claimed proof route but also as a diagnostic framework for regularity programs.

\section{The Central Idea: Stratify the State Space Before Trying to Prove Rigidity}

The main efficiency gain is that the proof does not seek one universal Liouville theorem for every bounded ancient solution or one universal description of every possible blow-up.  Instead it uses a decision procedure. At every stage the proof asks a scale-invariant question whose answer places the candidate singular branch into a smaller state space.
...
\item Does the branch satisfy a local Type I envelope, or does every local Type I
\end{enumerate}

envelope fail?

\begin{enumerate}
\item If Type I, what kind of centered ancient Seregin profile is produced?
\item If the Seregin profile is not in a known Liouville class, which residual
\end{enumerate}

terminal geometry does it have?

\begin{enumerate}
\item If Type II, does the concentration have one retained core, several cores, a
\end{enumerate}

cascade, a rough core, scale collapse, or a scale-rigid terminal state?

\begin{enumerate}
\item If the branch escapes compactness, is the escape radiative, diffuse,
\end{enumerate}

critical-tail, affine/parasitic, or active and recurrent?

The point of this ordered classification is that each negative result only has to be proved on the stratum where its hypotheses are naturally true.  For example, endpoint \(L^3\) Liouville theory is used only after the argument has converted a profile into a sequence-\(L^3\) or uniformly tight state.  The axisymmetric circulation equation is used only after the profile has been placed in the axisymmetric bounded-circulation stratum.  Repaired gauges are used only for Type II branches where the selected core has moving scale and center.  Cost identities are used only after genuine scale collapse has been selected.
...
After this gate, every remaining singular point has positive critical mass at arbitrarily small scales.  That positive mass is the conserved token that the rest of the proof carries through blow-up, compactness, recentering, and terminal-profile extraction.  Whenever a branch claims to disappear, diffuse away, or become perturbative, the argument checks whether the retained mass has actually been accounted for.  If the mass cannot be accounted for, the branch is not a singular branch.
...
This dichotomy is efficient because the two branches have different natural compactness theories.
...
The proof deliberately does not merge these branches.  The Type I argument is an ancient-solution classification problem.  The Type II argument is a local concentration-geometry problem.  Treating them separately keeps each tool in the regime where it has a clean scale-invariant meaning.
...
\[
  ...
  \qquad \nabla\cdot V=0 .
\]
The crucial output is not merely that a limit exists.  The output is a normalized object with positive retained local mass.  In schematic terms, there are compact sets and times for which
...
\subsection{Why Seregin Extraction Is Used}

Seregin extraction is the right compactness mechanism for Type I because the Type I envelope produces uniform local boundedness away from the terminal time after rescaling.  It converts a dynamic singularity into an ancient object on the whole time interval \((-\infty,0)\), then centered variables turn the self-similar scaling into time translation in \(\tau\).  This makes the problem a classification problem for bounded ancient trajectories, where Liouville and compact-dynamical tools are effective.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Type I class} & \textbf{Selection criterion} & \textbf{Why it is removable} \\
\midrule
\endhead
...
The first four classes are not intended to be disjoint.  The residual class is the complement of their union after those classes have been removed.  This is important: residual means "not already removed by the available direct Liouville mechanisms," not "mysterious singularity with no structure."
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Tool} & \textbf{Where it enters} & \textbf{Why it is chosen} \\
\midrule
\endhead
...
\section{The Residual Type I Branch: Why a Second Paper Is Needed}
...
The key idea of the residual paper is that this remaining class is still not a single amorphous object.  It has terminal geometry.  The proof refines the residual state space by asking ordered questions about symmetry, rotation, stationary hulls, affine/parasitic modes, tail compactness, active loci, diffuse defects, and terminal recurrence.
...
\subsection{Why the Residual Tools Are Different}

The residual branch is difficult because a direct global \(L^3\) bound or global Liouville theorem is not available at the start.  The residual paper therefore builds such a conclusion only after eliminating the ways it could fail.

This is a PDE point worth emphasizing.  A residual profile may have good local boundedness and retained compact mass while still failing every direct global critical-space hypothesis.  The proof does not pretend otherwise.  It first removes affine/parasitic modes, diffuse tails, coherent critical tails, and active descendant chains.  Only after these exits are closed does the generic residual branch inherit the mildness and sequence-\(L^3\) structure needed for the endpoint ancient Liouville theorem.
...
This is why the residual proof is efficient.  It does not prove a universal Liouville theorem for every bounded centered ancient residual profile.  It forces any residual profile that avoids lower strata to become endpoint Liouville-admissible.
...
\section{The Type II Branch: What Has to Be Classified}
...
The repaired chart is also designed to preserve the parabolic character of the equation: the time reparametrization is tied to the spatial scale so that the viscosity coefficient remains fixed in the renormalized equation.  This is why the Type II analysis can still use local energy, Caccioppoli, pressure reconstruction, and compactness estimates in the transformed variables.

\subsection{Why Repaired Gauges Are Used}

Without a repaired gauge, a Type II concentration branch can look noncompact for trivial reasons: the core center moves, the selected scale drifts, or two nearby possible cores compete for the normalization.  Repaired gauges separate these effects:
...
This is the Type II branch closest to a simple blow-up profile.  A single core is retained, the repaired-gauge representation is valid, and the local velocity-pressure quantities are uniformly controlled on compact cylinders.

If localized dissipation vanishes, compactness forces the limit to be too rigid.  Either the limit is zero, which contradicts the positive CKN mass used to select the core, or the limit is a nonzero bounded selected-window profile, which contradicts the premise that no local Type I scale-invariant bound exists.

This branch is efficient because it turns a Type II question into a local compactness contradiction, not a global profile classification.

\subsection{Multibubble and Cascade Branches}

The multibubble/cascade analysis handles the ways a Type II branch can avoid being a single compact core.

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.35\linewidth}>{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Subtype} & \textbf{Description} & \textbf{Why it is a separate stratum} \\
\midrule
\endhead
...
The proof uses critical small-data stability to remove perturbative profile classes.  It uses lower mass thresholds to ensure that every active profile consumes a definite amount of critical mass.  The current profile package makes this step explicit: bounded terminal \(L^3_\sigma\) sequences admit an orthogonal profile decomposition, cubic \(L^3\)-mass decoupling, Kato-small heat remainders, nonlinear profile perturbation, and an exhaustion clause saying that any residual terminal frame with nonzero critical mass generates another profile.  This makes infinite or uncontrolled active splitting impossible in the finite critical-mass regimes reached by the terminal profile theorem.
...
Therefore, if the inner \(H^1\) control fails, the branch cannot be a new singularity type.  It means that one of the enclosing compact-cylinder controls has failed, and that failure is already part of the compactness-failure, multibubble, or cascade side of the state-space partition.
...
The cost method is efficient because it avoids classifying every possible scale-collapse trajectory.  If collapse carries retained mass, the cost must diverge.  If it does not diverge, the retained core cannot survive.  The latest refinement adds the converse local exclusion interface: once a validated nonnegative localized cost diverges on the canonical windowwise moving schedule, corrected monotonicity rules out the fully summable-error case, so the branch must enter one of the final carrier exits \(J_1,J_6,J_7\).  Those exits are now routed to transition/exterior leakage, negative scale drift, or positive scale-work/scale-rigidity.

For a PDE expert, the useful analogy is that the cost plays the role of a localized monotonicity defect adapted to a moving scale-center frame.  It is not introduced as a new global norm.  It measures exactly the dissipation and scale drift that would be required for a compact retained core to collapse through renormalized scales, while the corrected-energy argument prevents divergent cost from being treated as a separate final assumption.

\subsection{Retained Compact Cost and Carrier Routing}

The retained compact-cost layer is the part of the Type II proof that was most refined in the latest pass.  The older architecture treated adjacent compact cost exits and cost-divergence exclusion as broad interfaces.  The current paper splits them into explicit local tests.
...
This matters because the proof no longer needs to turn nonintegrability into a pointwise or unit-window lower bound.  Diffuse nonintegrable tails are converted into normalized local carrier states, and those states carry the geometric label needed by the ordered partition.

\subsection{Terminal Type II States}

The Type II paper also records terminal profile and critical-annulus classifications.  These distinguish finite critical mass, infinite tail, vanishing critical norm, exterior regularity, and active terminal profiles. The role of these classifications is to prevent terminal escape from becoming a hidden sixth alternative.  A branch that tries to avoid the compact single-core mechanism must either regenerate active concentration, enter a scale-rigid state, or lose one of the explicit analytic inputs.  It is not left unclassified.

The terminal critical-profile input is also now explicit.  The paper proves the critical Navier--Stokes profile package from the standard \(L^3\) profile decomposition and nonlinear profile perturbation theory, plus internal lemmas for \(L^3\)-mass decoupling, heat-flow Kato smallness, and diagonal extraction of hidden terminal mass.  Thus "no hidden terminal profile" is not a separate terminal assumption in the current architecture; it is the exhaustion clause of the critical profile theorem.

\section{Tool Map Across the Whole Proof}

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.28\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}@{}}
\toprule
\textbf{Tool} & \textbf{Used in} & \textbf{What it proves} & \textbf{Why it is efficient} \\
\midrule
\endhead
CKN epsilon-regularity & First gate in \path|proof_setup.tex| and Type II entry. & Vanishing critical mass implies regularity. & It removes the easiest branch before blow-up analysis begins. \\
...
\section{How the State-Space Partition Is Actually Done}
...
Each "yes" branch has a closure theorem.  The "none of the above" branch is not ignored.  It is the generic residual class, and it is closed last using the terminal concentration theorem, descendant heredity, recurrence exclusion, and endpoint Liouville.
...
This partition is also ordered.  Failure of one representation condition is not a gap; it is routed to an explicit alternative, usually multibubble, cascade, rough-core, compactness failure, or scale-rigidity.

\section{Why the Architecture Is Efficient}
...
Second, it separates Type I and Type II before doing expensive analysis.  Type I has a strong compactness theory leading to bounded ancient solutions.  Type II does not.  Treating them with the same argument would either lose the Type I structure or over-assume compactness in Type II.
...
Fourth, it turns failures into strata.  If pressure compactness fails, if a gauge degenerates, if a core becomes rough, if a tail is diffuse, if a critical defect lives between scales, or if a moving carrier component is nonintegrable, the proof does not treat that as a vague technical obstruction. It gives the failure a name and an ordered route in the state space.
...
Seventh, the proof distinguishes classification from closure.  A decomposition theorem says where a branch must go; a closure theorem says that the destination is empty or contradictory.  Keeping those roles separate is what makes the dependency graph legible: when a Type II rigidity or compactness input is needed, it appears as the closure of a named stratum, not as an unstated premise hidden inside the decomposition.

\section{What Is Not Being Assumed Silently}

Several tempting shortcuts are explicitly avoided by the architecture.

\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.64\linewidth}@{}}
\toprule
\textbf{Shortcut not used} & \textbf{Replacement in the proof architecture} \\
\midrule
\endhead
...
This section instantiates the proof-capsule template of the strategy paper
(\emph{Local Proof Architectures for Critical PDE Regularity}, Appendix
``Proof-Capsule Template'') for each node of the decision diagrams above.
...
Section~\ref{sec:state-space-trees}, and are intentionally compact: the prose in the
preceding sections is the explanation, the templates here are the contract.
...
\begin{itemize}
  \item Capsule name: Capsule G1: Vanishing CKN concentration $\to$ regularity.
  \item Implementation labels:
  \item L1. \texttt{p0:thm:ckn}.
  \item L2. \texttt{p0:thm:no-concentration}.
  \item Source: proof\_setup.tex (Part I, local CKN).
  \item Input contract:
  \item H1. Suitable weak solution on a backward parabolic cylinder $Q_R(z_*)$.
  \item H2. The CKN scale-invariant quantity $C(z_*,r)+D(z_*,r)$ tends to 0 as r $\to$ 0.
  ...
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item A3. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (terminal).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule G2: Vanishing velocity concentration $\to$ regularity.
  \item Implementation labels:
  \item L1. \texttt{p0:thm:velocity-ckn}.
  \item Source: proof\_setup.tex (velocity-only epsilon-regularity).
  \item Input contract:
  ...
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A3. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (terminal).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule G3: Positive critical concentration token.
  \item Implementation labels:
  ...
  \item Retained obstruction:
  \item I. The fixed lower bound $\eta_0$ (the retained CKN/velocity mass token).
  \item Failure routes:
  \item F1. None inside this capsule; failure here would re-enter G1/G2.
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (transport; produces the retained token).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule G4: Type I / Type II dichotomy at $z_*$.
  \item Implementation labels:
  \item L1. \texttt{p0:thm:typeI-typeII-dichotomy}.
  \item L2. \texttt{p0:thm:local-weak-serrin-typeI}.
  \item Source: proof\_setup.tex.
  \item Input contract:
  \item H1. Capsule G3 token is available at $z_*$.
  \item Output contract:
  \item O1. The branch enters either the local pointwise Type I envelope state or the local Type II state (no such envelope).
  \item Retained obstruction:
  \item I. The G3 token is preserved on both outgoing branches.
  \item Failure routes:
...
\begin{itemize}
  \item Capsule name: Capsule G5: Terminal velocity no-escape.
  \item Implementation labels:
  \item L1. \texttt{p0:thm:auto-velocity-no-escape}.
  \item L2. \texttt{p0:thm:local-weak-serrin-typeI}.
  \item Source: proof\_setup.tex (no-escape preparation for extraction).
  \item Input contract:
  ...
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item A5. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item Closure status:
...
\begin{itemize}
  ...
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item A3. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A4. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A5. Nonlinear passage: each nonlinear term passes to the limit in the declared topology, with no implicit Aubin--Lions or whole-space estimate.
  \item A6. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (transport into Type I class ledger).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule TI2: Small-amplitude Type I closure.
  \item Implementation labels:
  ...
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A3. Nonlinear passage: each nonlinear term passes to the limit in the declared topology, with no implicit Aubin--Lions or whole-space estimate.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
...
\begin{itemize}
  \item Capsule name: Capsule TI3: Stationary $L^3$ Type I closure.
  \item Implementation labels:
  \item L1. \texttt{p1:thm:classical-classes}.
...
\begin{itemize}
  \item Capsule name: Capsule TI4: Uniformly $L^3$-tight Type I closure.
  \item Implementation labels:
  ...
  \item Audit checklist:
  \item A1. Scaling: every term in the local identity transforms with its declared critical dimension.
  \item A2. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (terminal).
...
\begin{itemize}
  \item Capsule name: Capsule TI5: Structured / decay-covered Type I closure.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TI6: Direct-class exhaustion $\to$ residual hand-off.
  \item Implementation labels:
  \item L1. \texttt{p1:thm:remaining-criterion}.
  \item Input contract:
  \item H1. V fails the small, stationary, tight, and structured/decay tests.
  \item Output contract:
  \item O1. V is a normalized residual Seregin profile, handed to paperIV via \texttt{hyp:base-seregin-hypotheses}.
  \item Retained obstruction:
  \item I. Compact $L^3$ mass plus residual ancestry.
  \item Failure routes:
  \item F1. None: this is a definitional handoff into the Type I residual graph.
  \item Audit checklist:
  \item A1. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A2. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
...
\begin{itemize}
  \item Capsule name: Capsule R0: Imported setup package.
  \item Implementation labels:
  ...
  \item Input contract:
  \item H1. Output of TI6 (residual normalized Seregin profile from setup).
  \item Output contract:
  \item O1. The same object is studied throughout paperIV: pressure gauge, retained mass, ancestry, and terminal suitability are fixed.
  \item Retained obstruction:
  ...
  \item Audit checklist:
  \item A1. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item A2. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A3. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
...
\begin{itemize}
  \item Capsule name: Capsule R0b: Ancestor-realization and descendant heredity.
  \item Implementation labels:
  \item L1. \texttt{thm:ancestor-realization-inheritance}.
  \item L2. \texttt{thm:descendant-heredity}.
  \item Input contract:
  \item H1. Output of R0.
...
\begin{itemize}
  \item Capsule name: Capsule R1: Axisymmetric bounded-circulation residual.
  \item Implementation labels:
  ...
  \item A2. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item A3. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A4. Nonlinear passage: each nonlinear term passes to the limit in the declared topology, with no implicit Aubin--Lions or whole-space estimate.
  \item A5. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (terminal in axisymmetric stratum).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule R2: Rotational relative equilibrium.
  \item Implementation labels:
  \item L1. \texttt{thm:terminal-stratification}.
  \item L2. \texttt{thm:R3-no-attainable-degenerate-family} (rotational sub-route).
  \item Input contract:
...
\begin{itemize}
  \item Capsule name: Capsule R3: Degenerate stationary hull.
  \item Implementation labels:
  \item L1. \texttt{thm:R3-no-attainable-degenerate-family}.
  \item L2. \texttt{thm:R3S-terminal-theorem}.
  \item L3. \texttt{thm:R3S-no-active-stationary-carrier}.
  \item Input contract:
  ...
  \item Output contract:
  \item O1. No realized active stationary carrier; the hull element does not retain concentration in any descendant.
  \item Retained obstruction:
  ...
  \item A2. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A3. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item A4. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item Closure status:
  \item verified (terminal in stationary-hull stratum).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule R4: Affine / parasitic quotient.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R5a: Recurrent tail-core residual.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R5b: Log-diffuse critical tail.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R5c: Young critical-tail / variance defect.
  \item Implementation labels:
  ...
  \item Audit checklist:
  \item A1. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A2. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A3. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (transport).
...
\begin{itemize}
  \item Capsule name: Capsule R5d: Coherent critical-tail rigidity (homogeneous / log-periodic /.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R6a: Terminal concentration extraction.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R6b: Terminal inactive / nonconcentrating exclusion.
  \item Implementation labels:
  \item L1. \texttt{thm:terminal-inactive-exclusion}.
  \item Input contract:
  \item H1. Terminal extraction (R6a) yields no active compact concentration.
  \item Output contract:
  \item O1. Retained mass cannot vanish: this branch contradicts the residual mass token.
  \item Retained obstruction:
  \item I. Residual mass token (preserved through R0/R0b).
  \item Failure routes:
...
\begin{itemize}
  \item Capsule name: Capsule R6c: Diffuse and mixed compact-diffuse terminal exclusion.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R6d: Active terminal profile -- separated / descendant / recurrent.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule R7: Endpoint $L^3$ closure of the generic residual.
  \item Implementation labels:
  ...
  \item A4. Nonlinear passage: each nonlinear term passes to the limit in the declared topology, with no implicit Aubin--Lions or whole-space estimate.
  \item A5. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item A6. Pressure/gauge: the pressure is normalized only modulo functions of time or spatial means, compatibly with all limits.
  \item Closure status:
...
\begin{itemize}
  \item Capsule name: Capsule TII1: Type II analytic-data exhaustion (entry).
  \item Implementation labels:
  ...
  \item Source: type\_II\_regularity.tex (Part C1 / Paper 0 / Paper 6a entry).
  \item Input contract:
  \item H1. Capsule G3 token at $z_*$; capsule G4 routes to "no Type I envelope".
  \item Output contract:
  ...
  \item Retained obstruction:
  \item I. Positive local Type II concentration token.
  \item Failure routes:
  ...
  \item A3. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A4. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A5. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A6. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (transport).
\end{itemize}
...
\begin{itemize}
  \item Capsule name: Capsule TII2: Repaired-gauge representation.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII3: Pressure / local-energy compatibility in repaired variables.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII4a: Compact single-core vanishing-dissipation.
  \item Implementation labels:
  ...
  \item A3. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A4. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A5. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A6. Nonlinear passage: each nonlinear term passes to the limit in the declared topology, with no implicit Aubin--Lions or whole-space estimate.
  \item A7. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
  \item verified (terminal in single-core row).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule TII4b: Same-point comparable-scale multibubble.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII4c: Same-point strict scale cascade.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII4d: Separated-point multibubble.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII4e: Gauge-degenerate branch.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII5: Rough core / windowed $H^1$ failure.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII6a: Finite scale-collapse cost (exclusion).
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII6b: Collapse core-floor failure.
  \item Implementation labels:
  ...
  \item Retained obstruction:
  \item I. Retained core mass token.
  \item Failure routes:
  \item F1. Core preserved $\to$ TII6a.
  \item Audit checklist:
  \item A1. Locality: the capsule uses only the declared cylinder/window/state, with no hidden whole-space data.
  \item A2. Retention: the retained obstruction declared in $I$ is preserved through every operation in the capsule.
  \item A3. Compactness topology: the convergence used is strong enough for every nonlinear passage in the capsule.
  \item A4. Dependencies: every hypothesis is produced by an upstream capsule, with no circular use of a downstream closure.
  \item Closure status:
...
\begin{itemize}
  \item Capsule name: Capsule TII6c: Thin-drift scale-collapse alternative.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII6d: Autonomous-modulation alternative.
  \item Implementation labels:
  ...
  \item Input contract:
  \item H1. Thick windows exist but modulation fails to converge cleanly.
  \item Output contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII7: Scale-rigid stratum closure.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII8a: Critical tightness / compact cost validation.
  \item Implementation labels:
  ...
  \item Retained obstruction:
  \item I. Retained compact-cost token.
  \item Failure routes:
...
\begin{itemize}
  \item Capsule name: Capsule TII8b: Adjacent compact-cost / carrier completion.
  \item Implementation labels:
  ...
  \item Input contract:
  \item H1. Output of TII8a; fixed/moving cutoff carrier errors must be decomposed into finite-error identities.
  \item Output contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII8c1: $J_1$ transition / exterior carrier.
  \item Implementation labels:
  ...
  \item L3. \texttt{paper7:def:windowwise-transition-routing}.
  \item Input contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII8c6: $J_6$ negative scale-drift carrier.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII8c7: $J_7$ positive scale-shell work carrier.
  \item Implementation labels:
  ...
  \item L3. \texttt{paper7:def:windowwise-positive-scale-routing}.
  \item Input contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII8d: Cost-divergence exclusion (assembly).
  \item Implementation labels:
  ...
  \item Output contract:
  \item O1. Divergent compatible cost cannot remain a free terminal branch; the capsule routes to TII8c1/TII8c6/TII8c7 and closes.
  \item Retained obstruction:
  \item I. Carrier cost token.
  \item Failure routes:
...
\begin{itemize}
  \item Capsule name: Capsule TII9a: Vanishing terminal critical $L^3$.
  \item Implementation labels:
  ...
  \item Retained obstruction:
  \item I. Terminal critical mass token.
  \item Failure routes:
...
\begin{itemize}
  \item Capsule name: Capsule TII9b: Terminal adjacent label.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII9c: Residual terminal mass / hidden critical profile.
  \item Implementation labels:
  ...
  \item Input contract:
  \item H1. A terminal branch retains mass outside the recorded compact/core states; terminal frames sampled from the represented orbit are bounded in the critical $L^3$\_$\sigma$ framework.
  \item Output contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII10: Retained compact-test closure / terminal local compactness.
  \item Implementation labels:
  ...
  \item Input contract:
  \item H1. Retained compact tests survive but are not yet assigned to an existing Type II state; terminal frames are bounded, unbounded, or sampled in incompatible ways.
  \item Output contract:
...
\begin{itemize}
  \item Capsule name: Capsule TII11: Exterior terminal concentration.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule TII12: Final Type II assembly.
  \item Implementation labels:
...
\begin{itemize}
  \item Capsule name: Capsule K1: Corrected-monotonicity check on the validated compact cost.
  \item Implementation labels:
  \item L1. \texttt{paper6a:thm:fixed-corrected-monotonicity} \texttt{paper6a:thm:moving-corrected-monotonicity}.
  \item Input contract:
  \item H1. validated compact Type II cost; finite initial corrected energy.
  \item Output contract:
  \item O1. fixed or moving corrected monotonicity identity.
  \item Closure status:
  \item verified (transport).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule K2: Finite-vs-divergent total cost dichotomy.
  \item Implementation labels:
  \item L1. \texttt{paper6a:lem:corrected-monotonicity-versus-divergent-cost} \texttt{paper6a:thm:fixed-cost-criterion}, \texttt{paper6a:thm:compact-cost-criterion}.
  \item Input contract:
  \item H1. corrected monotonicity from K1.
  \item Output contract:
  \item O1. either total cost finite (compact criterion gives divergent cost, contradiction routes to K3) or some moving-error component nonintegrable (route to K3 directly).
  \item Closure status:
  \item verified (transport).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule K3: Canonical component selection ($J_1$, $J_6$, $J_7$).
  \item Implementation labels:
  \item L1. \texttt{paper7:thm:moving-carrier-closure} \texttt{paper7:thm:carrier-package-closes-precarrier}.
  \item Input contract:
  \item H1. nonintegrable moving-error component.
  \item Output contract:
  \item O1. J\_2-J\_5 are summable on the canonical moving family; the failing component is in {$J_1$, $J_6$, $J_7$}.
  \item Closure status:
  \item verified (routing).
\end{itemize}

\begin{itemize}
  \item Capsule name: Capsule K\_final: Cost-divergence exclusion.
  \item Implementation labels:
  \item L1. \texttt{paper6a:thm:cost-divergence-exclusion-discharged}.
  \item Input contract:
  \item H1. TII8c1 / TII8c6 / TII8c7 closure.
  \item Output contract:
  \item O1. cost-divergence exclusion is discharged.
  \item Closure status:
...
\begin{itemize}
  ...
  \item Every implementation label cited above has been verified to occur
        in one of the three source files (\path|proof_setup.tex|,
        \path|paperIV_residual_branch.tex|,
        \path|type_II_regularity.tex|) at the time of writing.  One label
        used in earlier drafts of the diagrams,
        \texttt{thm:terminal-nonactive-exclusion}, has been corrected to
        the actual label \texttt{thm:terminal-inactive-exclusion}; this
        correction does not change the proof strategy and does not
...
This register is regenerated against the current \texttt{theorem} environments in the three TeX sources.  It is grouped by file so the labels can be cross-checked against the decision trees and the source.  Some theorem environments carry non-\texttt{thm:} prefixes because the source uses hypothesis-style or lemma-style labels for theorem-environment statements.
...
The proof architecture is therefore a finite decision tree with terminal contradictions.  Its strength is that every possible singularity type is routed before it is eliminated.  Its discipline is that every route specifies the analytic input it uses.
