...
\]
Apply the mild formula \eqref{eq:mild} on \([\tau-T,\tau]\).  By
\cref{lem:stokes-estimates},
\[
   \|V(\tau)\|_{L^\infty}
   \le
   e^{-T/2}M
...
\[
   M\le C_*M^2.
\]
Choose \(\varepsilon_0=(2C_*)^{-1}\).  If \(M\le\varepsilon_0\), the last inequality forces
\(M=0\).  Hence \(V\equiv0\).
\end{proof}
...
\(W_0\in L^3(\mathbb R^3)\), and substituting into \eqref{eq:centered} gives the
stationary profile equation \eqref{eq:stationary-profile}.  \Cref{thm:NRS}
yields \(W_0\equiv0\).
\end{proof}
