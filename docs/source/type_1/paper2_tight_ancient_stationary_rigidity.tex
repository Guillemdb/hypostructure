...
the \(i\)-th Riesz transform on \(\mathbb R^3\).
...
\begin{proof}
For almost every \(\tau\), choose \(\chi\in C_c^\infty(B_{4R})\) with
\(\chi\equiv1\) on \(B_{3R}\), and write in \(B_{3R}\)
\[
   P=q+h,\qquad
   q=(-\Delta)^{-1}\partial_i\partial_j(\chi V_iV_j).
\]
Then \(h(\cdot,\tau)\) is harmonic in \(B_{3R}\).  Calderon--Zygmund estimates
and boundedness of \(V\) give the \(L^q(B_{2R})\) bound for \(q\).  Set
\(a(\tau)=(h)_{B_{2R}}(\tau)\).  Interior estimates for harmonic functions and
Poincare's inequality give
\[
   \|h-a(\tau)\|_{L^q(B_R)}
   \le C(R,q)\|h\|_{L^1(B_{2R})}.
\]
The asserted estimate follows after integrating in \(\tau\).  In the
whole-space Leray case, the far-field part is represented by the Riesz kernel;
subtracting its value at the center gives the usual dyadic estimate, while the
near field is controlled by Calderon--Zygmund boundedness on \(L^p\),
\(1<p<\infty\), on a bounded spatial set.  More explicitly, if \(K_{ij}\) is
the kernel of \(R_iR_j\), then for
\(x\in B_R\) and \(|z|>4R\),
\[
   |K_{ij}(x-z)-K_{ij}(-z)|\le CR|z|^{-4}.
\]
Hence, with \(M_3=\sup_\tau\|V(\tau)\|_{L^3}\),
\[
   \sum_{m=2}^\infty
   R(2^mR)^{-4}
...
the chosen constant, with a bound depending only on \(M_3\).
\end{proof}
...
\[
   ...
   \qquad t\in\{\sigma,\tau\}.
\]
The same argument applies to \(V_n\otimes V_n\) against \(L^1\) matrix-valued
test functions.  Now first test \eqref{eq:mild} against
\(\phi\in C_c^\infty(\mathbb R^3)\).  By duality from
...
\]
Hence we may pass to the limit in the Duhamel term by dominated convergence
in \(s\).  The singular factor
\((1-e^{-(\tau-s)})^{-1/2}\) is integrable at \(s=\tau\).  Thus
\eqref{eq:mild} holds for \(V\) against \(C_c^\infty\) test functions.  Since
both sides are bounded in \(L^\infty\), density of \(C_c^\infty(\mathbb R^3)\) in
\(L^1(\mathbb R^3)\) gives the weak-* \(L^\infty\) identity.
\end{proof}
