...
the setup paper, local compactness and formal reduction statements
proved here, and the first-bad-mesoscopic-scale routing theorem which
discharges the hidden critical-tail boundary defects.  The affine stratum and the coherent
...
peer-reviewed Navier--Stokes regularity and Liouville literature.  The
...
estimates rather than imported as global estimates.
...
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   ...
   status: proved/imported.
   \item Uniformly tight stratum: handled in the setup paper; status:
   proved/imported.
...
list are discharged by the case-closure theorems cited in the proof of
...
input is also provided there.  The present paper uses this as an imported
...
\textbf{Exclusion.}  Log-window descendant selection plus first-bad
...
the setup package in \texttt{proof\_setup.tex} and the local concentration input
...
the setup package is the fixed compact velocity \(L^3\) lower bound.
\end{definition}
...
the setup package have been removed.
\end{definition}
...
Seregin compactness supplied by the admissible setup package, velocities converge
strongly in
...
package, and hence remains inside the localized limiting equation; it carries no
independent CKN
...
alternatives used in the setup package, by the Escauriaza--Seregin--\v Sver\'ak endpoint
...
the setup package and cited below as \Cref{thm:AB-main}.  Hence \(W\equiv0\).  A
...
unified setup package in \texttt{proof\_setup.tex}: local positive concentration
and the Type~I Seregin compactness package.  It then uses paired terminal
...
and pressure-gauge bounds supplied by the setup package on every fixed compact
cylinder.
...
the setup package and all profiles obtained from them by the state-space operations used
...
is not a pressure-gauge assertion.  By the normalization in the setup package
...
terminal cylinders in the variables of the chosen extraction.  The setup package
gives
...
supplied by the setup package \texttt{proof\_setup.tex}.  Then the retained
...
point gives a positive local velocity concentration sequence.  The setup package
...
positive concentration input in that package would select a unit terminal
cylinder with mass
...
For terminal sequences generated by the Seregin setup in
\texttt{proof\_setup.tex}
...
concentration input and the local compactness package in
...
recorded in the setup package \texttt{proof\_setup.tex} have been removed.  Then
...
\end{equation}
in the bounded mild duality sense used in the setup package.  The same conclusion is
...
duality formulation used in the setup package then extends the identity from
...
The setup package constructs normalized Seregin limits as centered bounded mild
...
status as a terminal profile generated directly by the setup package extraction.
...
by the setup package in \texttt{proof\_setup.tex}.  It is not enough to say that
the diffuse or noncompact state is not
...
velocity mass from the setup package still forces the terminal active locus to be
...
\begin{enumerate}[label=\textup{(\arabic*)}]
   \item \emph{Entry certificate:} The setup package supplies a positive local
   velocity concentration sequence; the endpoint weak Serrin local Type~I package
...
structural decomposition, from the unified setup package in
\texttt{proof\_setup.tex}.  All remaining branch-closure steps are proved in this
...
\item hidden-scale compactness routing for critical tails;
\item bounded-origin realization and critical-tail branch exclusion;
\item and the compactness and branching exclusions culminating in
 \Cref{thm:R4-final-closure}.
\end{itemize}
\end{remark}
