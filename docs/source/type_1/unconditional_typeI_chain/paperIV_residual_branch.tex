...
Papers I--III, local compactness and bookkeeping statements proved here, and the
remaining realized critical-tail rigidity theorem needed to close the critical
boundary compactification.  No global Coriolis--Leray Liouville theorem for
bounded profiles or global tail estimate is used in the residual closure.
\end{abstract}
...
\Cref{sec:local-typeI-bridge}.  The paper does not use the phrase ``Type~I''
to mean an unrelated global \(L^\infty\), local CKN, weak-\(L^3\), or Serrin
scale condition unless that condition is explicitly stated.
\end{definition}
...
neighborhood of \(x_*\) also enters the same pipeline.  Purely CKN-type,
weak-\(L^3\), or other critical criteria are not silently identified with this
hypothesis; they require their own implication into the local pointwise
envelope before the present exclusion applies.
\end{remark}

\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   \item Local concentration entry: depends on CKN theory and Paper~0;
   ...
   \item Closed structured/decay strata: depend only on the unconditional part
   of Paper~III; status: proved/imported.
   ...
   \item Realized critical-tail rigidity for the boundary compactification:
   explicit local state-space input
   \Cref{hyp:realized-critical-tail-rigidity}.
...
The final local pointwise Type~I exclusion does not use the conditional
controlled-swirl, weighted-velocity, Burgers-vortex association, or shell-control
hypotheses from Paper~III.  Those conditional statements remain background
criteria for other structured classes and are not on the main logical path
closed here.
...
Assume the realized critical-tail rigidity input
\Cref{hyp:realized-critical-tail-rigidity}.
...
Assume \Cref{hyp:realized-critical-tail-rigidity}.
No Paper~0-admissible finite-energy local pointwise Type~I singularity exists
whose normalized Seregin collection is \(\calS\).
\end{theorem}
...
implicit in the proof architecture of Papers~0--IV.  A finite-energy singular
point satisfying the local Type~I bound of Paper~0 already supplies the local
compactness needed to enter the terminal concentration package.  Thus the
whole-space no-escape hypothesis used to formulate the Paper~I admissible class
is not an additional assumption for a locally Type~I terminal singularity: the
local Type~I envelope gives compactness on every fixed terminal cylinder which
remains inside the local Type~I window after rescaling around the singular
point, while concentration attempting to leave every such bounded local window
is one of the exterior terminal alternatives.  The Paper~0 concentration
theorem supplies the retained local \(L^3\)-mass.
...
\Cref{prop:local-typeI-terminal-compactness}, there exist \(\eta_v>0\) and a
positive local velocity concentration sequence \(r_n\downarrow0\) at \(z_*\)
such that
\begin{equation}\label{eq:local-typeI-bridge-velocity-concentration}
   \iint_{Q_1(0,0)} |u_n|^3\,dx\,dt\ge \eta_v
...
\(\eps_{\rm sd}\le\eta_v\).  Then the retained compact concentration generated
from the origin recentering has no loss of local compactness.  Any retained
local recentering which stays in the local Type~I window has the same
compactness property by \Cref{prop:local-typeI-terminal-compactness}; any
concentration leaving that window is routed to the exterior or diffuse
terminal remainder.  The paired terminal decomposition of
\Cref{thm:terminal-exhaustion-main}, together with the mixed-state and active
branch exclusions
...
\begin{proof}
By the local velocity concentration theorem of Paper~0, every singular point
admits \(\eta_v>0\) and \(r_n\downarrow0\) such that
\[
   r_n^{-2}\iint_{B_{r_n}(x_*)\times(T-r_n^2,T)}|u|^3\,dx\,dt\ge\eta_v .
\]
After the rescaling this is exactly
\eqref{eq:local-typeI-bridge-velocity-concentration}.  By
\Cref{prop:local-typeI-terminal-compactness}, the rescaled sequence has local
Seregin compactness on every compact terminal cylinder, and therefore the
origin concentration produces a bounded ancient Seregin profile with retained
local \(L^3\)-mass.

The local CKN measure \(\mu_N\) in \Cref{def:terminal-measures} dominates
\(|V_N|^3\,dy\,d\tau\).  Choosing \(\eps_{\rm sd}\le\eta_v\) therefore makes
the distinguished origin recentering \(\eps_{\rm sd}\)-concentrating.  The
origin recentering satisfies the local-window condition in
\Cref{prop:local-typeI-terminal-compactness}, so it is not in
the local-compactness-failure class.  Other retained local recenterings inside the
...
active profile locus rather than treating them as mutually exclusive
alternatives.  The sole-carrier radiative cases are excluded by
\Cref{thm:terminal-nonactive-discharge}, mixed compact--diffuse or
compact--noncompact coexistence is excluded by
\Cref{thm:no-mixed-compact-diffuse}, and infinite active branches are excluded
by \Cref{thm:compact-active-descendant}.  Hence
\Cref{cor:persistent-forces-active} leaves only a single retained concentrating
terminal profile or a finite separated family of retained concentrating
terminal profiles.
\end{proof}
...
finite-energy local Type~I singular point exists.
\end{theorem}

\begin{proof}
Apply \Cref{lem:local-typeI-into-terminal-dichotomy}.  The local Type~I
rescalings therefore produce at least one bounded terminal profile carrying
retained local concentration.  After passing to the centered variables, the
...
branches are excluded by \Cref{thm:compact-active-descendant}, and mixed
...
\begin{corollary}[Local Type I exclusion under realized critical-tail rigidity]
\label{cor:local-typeI-unconditional}
Assume the realized critical-tail rigidity input
\Cref{hyp:realized-critical-tail-rigidity}.
Let \((u,p)\) be a suitable Leray--Hopf solution on
\(\R^3\times(0,T)\).  If \(z_*=(x_*,T)\) is a singular point, then \(z_*\)
cannot satisfy the local Type~I bound \eqref{eq:local-typeI-bridge-bound}.
Equivalently, in the local Type~I/local Type~II dichotomy of Paper~0, every
finite-energy singular point belongs to the local Type~II alternative.
\end{corollary}

\begin{proof}
The first assertion is exactly
\Cref{thm:local-typeI-entry-or-discharge}.  The equivalence is the local
Type~I/local Type~II dichotomy of Paper~0: at a singular point, either some
local Type~I bound of the form \eqref{eq:local-typeI-bridge-bound} holds, or
every such bound fails.
\end{proof}
...
\begin{definition}[Threshold ledger]
\label{def:threshold-ledger}
...
\begin{enumerate}[label=(\roman*)]
   ...
   infinite active descendant branches, are impossible in the refined residual
   class.
...
\Cref{lem:atomic-sequence-L3}, and item (vii) is
   ...
   proved below from local compactness.  The only remaining state-space input
   named in the theorem statement is realized critical-tail rigidity.  No global
   bound is assumed.
\end{proof}
...
\end{equation}
After the covariant recentering \(\mathscr T_{(x_n,\tau_n)}\), either the local
compactness and pressure control required for a terminal profile fail, or a
subsequence converges in the terminal topology to a retained concentrating tail
limit.  The time-slice lower bound gives parabolic CKN activity by
\Cref{lem:timeslice-to-ckn-activity}.  Both alternatives contradict terminal
indecomposability.
...
branches, and mixed compact--diffuse or compact--noncompact remainders are
...
descendants, active-tree hulls, and recurrent-core limits below are taken in
this topology.
\end{definition}
...
\begin{definition}[Affine-constant lower stratum and residual normalization]
\label{def:affine-constant-lower-stratum}
The affine-constant stratum consists of centered profiles which become zero
after one affine homogeneous symmetry \(S_c\).  In the ordered residual
decomposition this stratum is part of the previously closed lower classes, not
part of \(\Cgen\).  Equivalently, every generic residual profile is considered
after the canonical affine normalization which removes its spatially homogeneous
mild mode.  Retained activity in the residual class is therefore always measured
after this normalization.
\end{definition}

\begin{remark}[How affine constants are used]
\label{rem:affine-constant-use}
When a compact recurrent tail core is shown below to consist only of affine
constants, the contradiction is not that an unnormalized nonzero constant has
zero local \(L^3\)-mass.  The contradiction is that affine constants have already
been assigned to the closed lower stratum in
...
Assume the covariant observer action of \(G\) is continuous on
\(\mathfrak D_M\).  Every nonzero diffuse-defect state
...
\]
Then, after passing to a subsequence, the residual diffuse branch falls into
...
\begin{enumerate}[label=\textup{D\arabic*.}]
   ...
   normalization; the branch belongs to the affine/parasitic lower stratum of
...
regeneration branch, the affine/parasitic branch, and the critical diffuse-tail
class \(\Ccritdiff\).
\end{theorem}
...
\Cref{def:affine-constant-lower-stratum} places the branch in the affine
...
\begin{assumption}[Realized critical-tail rigidity]
\label{hyp:realized-critical-tail-rigidity}
Every nonzero critical-tail defect \(\mathcal T\in\Tcrit\) generated by a
bounded normalized residual Navier--Stokes profile satisfies one of the
following alternatives:
\begin{enumerate}[label=\textup{(\roman*)}]
   \item \(\mathcal T\) regenerates affine-normalized active covariant observers;
   \item \(\mathcal T\) collapses to the affine/parasitic lower stratum;
   \item \(\mathcal T\) lies in one of the structured critical-tail lower strata
   already removed before entering \(\Cgen\):
   \[
      \Chomcrit,\qquad \Clogper,\qquad \Capcrit,\qquad
      \Cyoung,\qquad \Clogdiff .
   \]
\end{enumerate}
Equivalently, the realized residual class contains no nonzero critical-tail
defect in
\[
   \Tcoh\cup\Tyoung\cup\Tlogdiff
\]
after active regeneration, affine/parasitic collapse, and the listed structured
critical-tail strata have been removed.
\end{assumption}

\begin{theorem}[No mixed compact--diffuse residual state]
\label{thm:no-mixed-compact-diffuse}
Assume the realized critical-tail rigidity input
\Cref{hyp:realized-critical-tail-rigidity}.  Let \((U,\Pi)\) be a retained
terminal profile in the refined residual class, after the affine normalization
of \Cref{def:affine-constant-lower-stratum}.  Then \(U\) cannot have a diffuse
exterior oscillation remainder, nor an escaping retained noncompact
...
\(\mathcal T\in\Tcrit\).  Apply
\Cref{hyp:realized-critical-tail-rigidity}.  Active regeneration and
affine/parasitic collapse have already been excluded, and every structured
critical-tail lower stratum listed in the remaining alternative has been removed
before \(\Cgen\) is entered.  All alternatives are impossible.  Hence no mixed
compact--diffuse residual profile exists.
\end{proof}
...
successor, or an active descendant branch.  The mixed successors are excluded
by \Cref{thm:no-mixed-compact-diffuse}; infinite active branches and finite
...
\]
while every unit-scale exterior covariant recentering sequence remains below
the retained-concentration threshold, unless that recentering sequence has loss
of local compactness.  An escaping non-compact recentering sequence is a
...
\]
after lowering \(\eps_*\) by a harmless fixed factor if necessary.  By
\Cref{lem:timeslice-to-ckn-activity}, this time-slice velocity concentration
gives parabolic CKN activity on \(Q_1(0,0)\).  Thus \(W\) is a retained
concentrating covariant tail limit, again contradicting terminal
indecomposability.
...
Assume the Paper~I normalized Seregin extraction package together with the
Paper~II bounded mild centered normalization.  Let \((U,\Pi)\) be a retained
terminal profile, retained covariant tail descendant, active-tree limit, or
recurrent-core limit represented in the normalized residual state space.  Then
...
\end{enumerate}
The dichotomy and the normalized mildness property are part of this imported
normalization convention and are preserved by the local state-space operations:
Seregin extraction, centered time shifts, covariant translations, covariant tail
descendants, active-tree limits, and recurrent-core limits.
\end{theorem}
...
stratum.  The Paper~I normalization, interpreted through the bounded mild
centered formulation of Paper~II, is imported as part of the normalized
residual representative: the spatially homogeneous parasitic mode has already
been removed before the residual terminal analysis begins.  The local
state-space operations \(T_a\), \(\Theta_s\), \(\mathscr T_{(x,\sigma)}\), and
\(S_c\) act by exact centered symmetries and admissible gauge changes, with
the \(S_c\) bound tracked as in \Cref{lem:state-space-local-compactness}.  Hence
they carry normalized representatives to normalized representatives, unless
the limit is affine/parasitic, in which case the ordered decomposition assigns
it to the affine lower stratum.  This is bookkeeping for the imported
normalization; it is not a new global estimate.

Fix \(T<0\).  On \(\R^3\times(-\infty,T]\), the physical pullback is smooth and
bounded by \((-T)^{-1/2}\|U\|_{L^\infty}\), and it solves the physical
Navier--Stokes equations by the direct self-similar change of variables.  Since
the parasitic zero mode has been removed, the pressure has no unprojected
linear parasitic component on this finite terminal interval.  Hence the
bounded mild formulation of Paper~II applies: for
\(-\infty<s<t\le T\),
\[
   u(t)=e^{(t-s)\Delta}u(s)
...
\]
After translating the terminal time \(T\) to \(0\), this is the mild ancient
formulation for \(u^T\) on every finite forward interval
\((-\infty,0)\).  The bound
...
\Cref{def:threshold-ledger}, and it has the same parasitic-free normalization
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   so the active-tree and terminal-indivisibility arguments may be iterated
   from \(W\).
\end{enumerate}
\end{theorem}
...
for all through \Cref{def:threshold-ledger}.  The affine/parasitic
...
generic residual state space.  This is local state-space bookkeeping and does
...
\[
   \mathfrak A_\eta:=\{U\in\mathfrak X_M:\Phi(U)\ge\eta\}
\]
be the closed affine-normalized activity set of
\Cref{thm:closed-nonaffine-activity}.  We write
\[
   U\,\mathcal R_\eta\,V
\]
...
\Cref{thm:closed-nonaffine-activity}, and lower semicontinuity of the retained
...
\begin{proof}
The set \(\mathfrak A_\eta\) is compact in the local state topology inherited
from the bounded Seregin state space.  Hence
...
\begin{theorem}[No infinite active descendant branch]
\label{thm:compact-active-descendant}
No infinite retained active descendant branch can occur in the refined residual
class.
\end{theorem}

\begin{proof}
Suppose an infinite retained active branch exists.  If
...
\(\eta>0\) and an infinite ordered subbranch contained in \(\mathfrak A_\eta\).
By transitivity of retained tail descendants through
\Cref{lem:diagonal-lifting-descendants}, this ordered subbranch is an
\(\mathcal R_\eta\)-chain.

\Cref{thm:active-path-space-recurrence} gives a compact shift-invariant active
path space and a stationary path measure supported on \(\mathfrak A_\eta\).  By
Poincare recurrence for the shift, a recurrent path returns arbitrarily close to
itself.  Concatenating the retained tail-limit observers along the returning
blocks and using the diagonal lifting lemma produces a compact tail-minimal
recurrent core for \eqref{eq:centered-main} whose states lie in
\(\mathfrak A_\eta\).  The recurrent-core rigidity theorem
...
rules out the infinite active branch.
\end{proof}

\begin{lemma}[Finite separated families produce indecomposable profiles or active branches]
...
family generates an infinite active descendant branch.  A finite recurrent loop
is counted as an infinite branch by periodic repetition.
\end{lemma}
...
\]
Repeating the loop periodically gives an infinite active descendant branch.  If
...
descendant branch.  The nonactive successor alternatives at each stage are
...
Repeating the loop periodically gives an infinite active descendant branch.
...
descendant branch.  The first alternative contradicts
...
\Cref{thm:no-mixed-compact-diffuse}; infinite active descendant branches are
...
\begin{proof}
This is the assembly of the local results and explicit state-space inputs in
...
\Cref{hyp:realized-critical-tail-rigidity}.  Infinite active trees and finite
...
\Cref{thm:no-mixed-compact-diffuse}.  Infinite active branches are excluded by
...
Assuming the realized critical-tail rigidity input
\Cref{hyp:realized-critical-tail-rigidity}, the class
closures above form the following dependency chain.  The rotational
...
\(\Phi\), is the common contradiction target in all four classes.

Equivalently, the final exclusion is the composition of five certificates:
\begin{enumerate}[label=\textup{(\arabic*)}]
   \item \emph{Entry certificate:} Paper~0 and Paper~I turn a finite-energy
   local pointwise Type~I singular point into a normalized Seregin profile with
   retained compact velocity concentration.
   \item \emph{Stratification certificate:} the ordered residual decomposition
   places every remaining retained profile in one of the refined strata
   \(\Cax,\Crot,\Cstat,\Cgen\).
...
\Cref{thm:R3S-R3-empty}.  The generic terminal residual is empty, under the
   explicit realized critical-tail rigidity input, by the finite
   separated-family and indecomposability argument in
...
The realized critical-tail rigidity statement is the explicit local state-space
input for the critical boundary compactification, not a hidden consequence of
local CKN observables.  The rotational
