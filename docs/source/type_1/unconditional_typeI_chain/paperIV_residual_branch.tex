...
\newcommand{\BranchClosurePackage}{\Cref{thm:first-bad-mesoscopic-routing}}
...
Serrin local Type~I package, terminal no-escape, and the Seregin blow-up
...
endpoint Liouville theorem.  The paper distinguishes statements imported from
Papers I--III, local compactness and formal reduction statements proved here, and
the first-bad-mesoscopic-scale routing theorem which discharges the hidden
...
alternatives are both reduced to the same first-bad mesoscopic routing theorem, which is then
...
In this paper and in the final Papers~0--IV assembly, a terminal singular point
...
\]
This is the local pointwise Type~I hypothesis used in Paper~0 and in
...
the scale-invariant local energy package needed for terminal velocity
no-escape.  Thus the terminal no-escape condition used by Paper~I is automatic
for the local pointwise Type~I singularities considered here.
...
neighborhood of \(x_*\) enters the same pipeline, provided the solution is
suitable in the corresponding cylinder.  The endpoint weak Serrin estimate then
gives the local scale-invariant energy package and terminal no-escape.
...
local pointwise envelope, or directly into the local Type~I package, before the
present exclusion applies.
\end{remark}

\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.2em]
   \item Local concentration and terminal no-escape entry: positive local
   concentration depends on CKN theory and Paper~0; terminal no-escape follows
   from the endpoint weak Serrin local Type~I package in
   \Cref{thm:AB-weak-serrin-typeI-package,thm:local-typeI-terminal-energy-package}.
   \item Seregin extraction and parasitic-free normalization: depends on
   Paper~I and the Paper~II bounded mild formulation; status: proved/imported.
   \item Small and stationary \(L^3\) strata: depend on Paper~I; status:
   proved/imported.
   \item Uniformly tight stratum: depends on Paper~II and endpoint Liouville
   input; status: proved/imported.
   \item Closed structured/decay strata: depend on Paper~III; status:
   proved/imported.
...
\node[box, below=of typeI] (admissible) {Paper~I admissible Type~I package};
...
\node[box, below=of seregin] (split)
   {Paper~I--III lower classes\\or\\\(\calR(\calS;I,J)\)};
...
\draw[arrow] (typeI) -- node[lab, right]
   {endpoint weak Serrin package\\and terminal no-escape} (admissible);
...
the endpoint weak Serrin local Type~I package.  The Paper~I--III lower classes
are imported closed classes.  The residual alternatives in the boxed list are
discharged by the case-closure theorems cited in the proof of
\Cref{thm:refined-residual-closure-complete-decomposition}.}
\label{fig:typeI-state-stratification-tree}
\end{figure}

\begin{theorem}[Paper I admissible Type I Seregin extraction theorem]
...
compactness theorem.  Starting with the remaining coarse residual class, one
removes the following alternatives in order; after the first nine alternatives have
been removed, whatever remains is by definition the generic terminal residual
\(\Cgen\):
\[
\begin{gathered}
   \Cax,\qquad \Crot,\qquad \Cstat,\qquad \Caff,\qquad \Cgen,\\
...
is the generic terminal residual class.  Thus the decomposition inclusion below is a
formal consequence of the ordered definition; the non-tautological work is
the closure of each alternative, especially \(\Cgen\).  Equivalently,
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   the local pressure gauges supplied by the Seregin compactness package;
   ...
   renormalizing it;
   \item first-bad mesoscopic blow-downs selected from such retained or
...
\begin{theorem}[Ancestor-realization and discharge principle]
\label{thm:ancestor-realization-discharge}
Every descendant, tail limit, active successor, log-window normalization,
critical-tail state, and first-bad blow-down used in the proof of
...
\begin{enumerate}[label=\textup{(\alph*)}]
   ...
   window descendant is discharged, then the original defect is discharged.
...
\begin{proof}
The assertion is a formal consequence of the construction of the state spaces
and is used below as a logical reduction principle.  Exact centered
...
Raw terminal recenterings before centering are the recenterings in the Paper~I
Seregin extraction and are compact after the gauges in
...
such support descendants are routed into closed lower alternatives, then the
ancestor defect measure itself has no surviving mass outside those alternatives.

First-bad blow-downs are selected from retained or normalized defect windows;
...
\[
\begin{gathered}
   \Cax=\Crot=\Cstat=\Caff=\Cgen=\varnothing,\\
...
The affine/parasitic stratum is excluded by
...
\begin{proof}
Suppose that such a singular point exists.  Paper~0 supplies a positive local
velocity concentration sequence.  By
\Cref{thm:local-typeI-terminal-energy-package,cor:local-typeI-implies-admissible},
the local pointwise Type~I envelope gives the Paper~I terminal velocity
...
\subsection{Local Type I singularities enter the terminal concentration package}
\label{sec:local-typeI-bridge}

The purpose of this subsection is to make explicit the local reduction that is
implicit in the proof architecture of Papers~0--IV.  There are two logically
...
not by itself say that the positive \(L^3\)-mass in
\(B_1\times(-1,0)\) survives away from the terminal layer
\((- \sigma,0)\).  The latter is exactly the terminal velocity no-escape
condition used in Paper~I.

The bridge below first records the admissible form used by Paper~I.  It then
uses the endpoint weak Serrin local Type~I estimate of Albritton--Barker to
derive the required scale-invariant \(L^2\) bound on the terminal interval.
Interpolation with the Type~I envelope gives terminal velocity no-escape.
...
local-window condition are, by definition, routed to the exterior or diffuse
...
We next record the pressure estimate because it is the only place where a
local Type~I proof can hide a nonlocal issue.  Use the whole-space Leray
pressure representative.  Fix a cutoff
...
kernel difference estimate for the Riesz pressure kernel gives, for
\(x\in B_{2R}\),
\[
   |h_n(x,t)-a_{n,K}(t)|
   \le
...
\]
For annuli whose physical image remains in \(B_\rho(x_*)\), the local Type~I
bound gives
\[
   \int_{2^mR<|z|<2^{m+1}R}|u_n(z,t)|^2\,dz
   \le C M^2\sigma^{-1}(2^mR)^3 .
\]
The corresponding part of the series is bounded by a geometric sum.  For the
remaining outer annuli one uses only the finite Leray energy:
\[
...
\[
   ...
   \le C_{\rho,R,u}.
\]
Thus \(h_n-a_{n,K}\) is uniformly bounded on \(B_{2R}\times[-S,-\sigma]\), and
\[
...
\subsection{Pressure-projected reduction of the terminal no-escape gap}
\label{subsec:pressure-projected-terminal-gap}

The preceding compactness result is intentionally stated away from the terminal
slice.  We next isolate the exact additional estimate needed to cross the
terminal layer.  The local pressure projection removes the nonlocal pressure
flux from the local energy inequality; what remains is an interior shielding
problem, namely preventing the Caccioppoli estimate on a fixed interior ball
from cascading through larger and larger balls until it reaches the edge of the
local Type~I window.
...
\begin{proof}
We recall the standard local pressure-projection argument in the scale \(L\)
...
multiple of the target ball, uniformly as the edge
\(\partial B_{\rho/r}\) of the local Type~I window recedes to infinity.
\end{definition}

\begin{proposition}[Pressure projection reduces the entry gap to interior shielding]
...
\(z_*\).  If the interior shielding estimate
...
\begin{remark}[The narrowed final bridge]
\label{rem:narrowed-final-bridge}
\Cref{lem:local-pressure-projected-caccioppoli} shows, independently of the
Albritton--Barker shortcut used below, how the terminal no-escape issue reduces
to an interior shielding estimate: after local pressure projection, the local
energy inequality controls the projected velocity and the dissipation up to the
larger-scale quantity \(A_{4L}\).  A proof of
\eqref{eq:interior-shielding-estimate} by this route would prevent an edge-fed
terminal cascade directly.  In the present paper this shielding is supplied
instead by the endpoint weak Serrin local Type~I package in
\Cref{thm:AB-weak-serrin-typeI-package}.
\end{remark}

\begin{theorem}[Albritton--Barker weak Serrin local Type I package]
...
\[
   ...
   \frac3p+\frac2q=1 .
\]
Then for every \(0<\theta<1\), the scale-invariant local Type~I package is
...
package holds for every \(0<r<r_R\):
...
energy package just proved gives
\[
...
leaving that window is routed to the exterior or diffuse terminal remainder.
...
\begin{theorem}[Local Type I singularities enter the full exclusion pipeline]
...
\((u,p,z_*)\).  Consequently the local terminal concentration package of
...
\begin{proof}
By the local velocity concentration theorem of Paper~0, choose
...
\]
\Cref{thm:local-typeI-terminal-energy-package,cor:local-typeI-implies-admissible}
show that this concentration sequence satisfies the Paper~I terminal
velocity no-escape condition.
...
classes, then the contradiction is supplied by the earlier papers:
small-amplitude and stationary \(L^3\) profiles are excluded in Paper~I, the
uniformly tight class is excluded in Paper~II, and the closed structured/decay
class is excluded in Paper~III.  Otherwise choose a retained profile outside
those lower classes.  It is then a refined residual candidate, so
...
routes are impossible.  Hence the assumed finite-energy local Type~I singular
point does not exist.
\end{proof}

\begin{corollary}[Unconditional local pointwise Type I exclusion]
\label{cor:local-typeI-unconditional}
Let \((u,p)\) be a suitable Leray--Hopf solution on
\(\R^3\times(0,T)\).  If \(z_*=(x_*,T)\) is a singular point satisfying the
local Type~I bound \eqref{eq:local-typeI-bridge-bound}, then the completed
case closures give a contradiction.  Equivalently, in the local
Type~I/local Type~II dichotomy of Paper~0, every finite-energy singular point
belongs to the local Type~II alternative.
\end{corollary}

\begin{proof}
The first assertion is exactly \Cref{thm:local-typeI-entry-or-discharge}.  The
equivalence is the local Type~I/local Type~II dichotomy of Paper~0.
\end{proof}

\section{Rendered decision tree for the state-space partition}
\label{sec:mermaid-decision-tree}

The following rendered decision tree records the ordered partition used in the paper.
Each diamond is a stratification criterion.  Each terminal node is a possible
singularity or candidate blow-up profile type selected by the criteria.  The
local Type~II terminal alternative is the remaining regime after the local
pointwise Type~I case is excluded; every Type~I terminal alternative is
discharged either by Papers~I--III or by the case-closure theorems proved
below.
...
\node[terminalnode, right=1.55cm of QI] (TII)
   {Local Type~II singularity\\(not excluded here)};
\node[process, below=of QI] (WS)
   {Endpoint weak-Serrin package\\terminal no-escape};
\node[process, below=of WS] (SE)
   {Admissible Seregin profile\\retained local \(L^3\) mass};
\node[decision, below=of SE] (P13) {Earlier Paper~I--III class?};
...
\end{tikzpicture}}
\caption{Rendered entry tree.  Local pointwise Type~I enters the Paper~I
admissible Seregin package through endpoint weak Serrin estimates and terminal
no-escape.  If the bound fails, the point belongs to the local Type~II
alternative, which is not excluded here.}
...
removed, the profile is routed to that terminal alternative; after all lower
...
not actually been removed, it is routed to its corresponding terminal
...
\caption{Classification and closure table for the decision tree.  Each row gives the criterion
selecting the terminal alternative, the mechanism that discharges it when it is a Type~I alternative,
and the theorem or imported statement used at that point.  For the imported
Paper~I--III alternatives, the displayed condition is a representative form of the
imported definition; the precise definitions and closures are those of the
cited papers.}
\label{tab:decision-tree-classification}\\
\hline
\textbf{Terminal alternative} & \textbf{Criterion, discharge mechanism, and reference}\\
\hline
\endfirsthead
\hline
\textbf{Terminal alternative} & \textbf{Criterion, discharge mechanism, and reference}\\
...
\textbf{Status.}  Not excluded in this paper; this is the remaining alternative
after local pointwise Type~I is ruled out.
\textbf{Link.}  \Cref{cor:local-typeI-unconditional}.\\

Small Type~I profile &
\textbf{Criterion.}  Imported Paper~I small-amplitude class; schematically
\(\|V\|_{L^\infty(\R^3\times\R)}\le\eps_{\rm sm}\).
\textbf{Discharge.}  Small-data/regularity closure imported from Paper~I.
\textbf{Link.}  \Cref{hyp:previous-nonresidual}.\\

Stationary \(L^3\) Type~I profile &
\textbf{Criterion.}  Imported Paper~I stationary class; representative condition
\(\partial_\tau V=0\) and \(V\in L^3(\R^3)\).
\textbf{Discharge.}  Stationary \(L^3\) Liouville/regularity closure imported
from Paper~I.
\textbf{Link.}  \Cref{hyp:previous-nonresidual}.\\

Tight Type~I profile &
\textbf{Criterion.}  Imported Paper~II tight class; representative condition
\(\lim_{R\to\infty}\sup_\tau\int_{|y|>R}|V(y,\tau)|^3\,dy=0\).
\textbf{Discharge.}  Endpoint \(L^3\) rigidity for uniformly tight ancient
dynamics.
\textbf{Link.}  \Cref{hyp:previous-nonresidual}.\\

Structured Type~I profile &
\textbf{Criterion.}  Imported closed structured/decay class of Paper~III,
\(V\in\mathcal C_{\rm struct/decay}^{\rm PIII}\).
\textbf{Discharge.}  Closed structured/decay alternatives imported from
Paper~III.
\textbf{Link.}  \Cref{hyp:previous-nonresidual}.\\
...
\textbf{Discharge.}  Centered circulation equation plus axis-absorption
...
\textbf{Discharge.}  Localized Coriolis-flux routing into the terminal local
...
\textbf{Discharge.}  Scale reselection reduces to a stationary retained
...
\textbf{Discharge.}  Oscillatory entry normalization forces retained residual
...
\textbf{Discharge.}  Log-window descendant selection plus first-bad
mesoscopic routing and hidden-scale compactness.
...
\textbf{Discharge.}  No hidden-scale variance gives strong
...
\textbf{Discharge.}  Bounded-origin realization gives
...
\textbf{Discharge.}  Bounded-origin realization puts \(0\) in the compact
...
alternatives.
\textbf{Discharge.}  Paired active-locus decomposition, finite
...
The constants used in the terminal package are chosen in the following order.
The compact velocity lower bounds \(\eta_0\) and \(\eta_v\) are supplied by
Paper~I and Paper~0, respectively.  The local small-data threshold
...
It then uses paired terminal profile decomposition, sole-carrier exclusions,
critical-tail compactification with realized critical-tail rigidity, active
path-space recurrence, and the finite separated-profile theorem.  It does not
...
\begin{definition}[Affine-normalized observer oscillation]
\label{def:observer-oscillation}
Let \(Q\) be a bounded covariant terminal cylinder.  For a velocity field \(U\)
set
\[
...
\[
   \operatorname{osc}_3\bigl(U;\mathcal Q_1^{\rm cov}(z)\bigr)\ge\eta .
\]
...
\[
   ...
   :=
   \sum_{z_k\in E}
      \operatorname{osc}_3\bigl(U;\mathcal Q_1^{\rm cov}(z_k)\bigr) .
\]
...
``positive threshold,'' ``vanishing threshold,'' or ``diffuse defect.''
\end{definition}
...
and pressure-gauge bounds supplied by Paper~I on every fixed compact cylinder.
...
the local topology of \(\mathfrak X_M\), of all normalized Paper~I Seregin
profiles and all profiles obtained from them by the state-space operations used
...
homogeneous modes which are invisible to affine-normalized oscillation.  It has
...
\((C_n)\) has zero affine-normalized spatial oscillation on every compact
cylinder and belongs to \(\Caff\).  Conversely, after the canonical
...
weak limits and time recenterings.  Hence every such limit has zero
affine-normalized spatial oscillation on every compact cylinder and is, by
...
\[
   ...
   \inf_{c\in\mathbb R^3}
      \left(\iint_Q |U-c|^3\,dy\,d\tau\right)^{1/3}.
\]
Then
\[
   |d_Q(U)-d_Q(V)|
...
For every \((U,\Pi)\in\mathfrak X_M\) and every \(c\in\mathbb R^3\), exactly
one of the following alternatives holds:
\begin{enumerate}[label=\textup{(\roman*)}]
   \item \(\Phi(S_cU)>0\), so the affine-normalized representative has
   positive non-affine retained activity;
   \item \(\Phi(S_cU)=0\), so \(S_cU\) is globally constant and
   \((U,\Pi)\) belongs to the affine/parasitic lower stratum.
\end{enumerate}
...
\begin{proof}
By \Cref{thm:closed-nonaffine-activity}, \(\Phi(S_cU)=0\) if and only if
\(S_cU\) is globally constant.  That is exactly the affine/parasitic lower
stratum recorded in \Cref{def:affine-constant-lower-stratum}.  If this does
not happen, \(\Phi(S_cU)>0\).  The two alternatives are mutually exclusive and
exhaust all cases.  For the compact statement, \(\Phi\) is continuous and
...
\[
   \operatorname{osc}_3(U_n;\mathcal Q_1^{\rm cov}(z))\ge\eta ,
\]
...
positive affine-normalized oscillation-active unit observer remains outside
those loci.
\end{lemma}

\begin{proof}
By definition of the affine-normalized oscillation,
\[
   \operatorname{osc}_3(U_n;\mathcal Q_1^{\rm cov}(z))
   =
   \inf_{c\in\mathbb R^3}
...
Let \(\mu_n^{\rm osc}\) be the residual affine-normalized oscillation measures
...
\[
   ...
   \frac{C}{m_0}
   \sup_{\zeta\in E_n}
      \operatorname{osc}_3(U_n;\mathcal Q_1^{\rm cov}(\zeta))
   \longrightarrow0 .
\]
...
\begin{enumerate}[label=\textup{D\arabic*.}]
   ...
   \[
      \operatorname{osc}_3(U_n;\mathcal Q_1^{\rm cov}(z_n))\ge\eta .
   \]
...
\begin{proof}
If D1 occurs there is nothing to prove.  Assume D1 does not occur.  Then, for
every fixed \(\eta>0\), all sufficiently large \(n\) satisfy
\[
   \sup_{z\in E_n}
      \operatorname{osc}_3(U_n;\mathcal Q_1^{\rm cov}(z))<\eta .
\]
...
positive normalized diffuse oscillation mass, no unit-scale active observable,
and non-affine velocity content.  This is precisely the critical diffuse-tail
...
the covariant observer compactification, carry normalized affine oscillation
...
\[
   ...
   \nabla\cdot W=0
\]
in distributions, with nonzero affine-normalized diffuse oscillation and
...
\begin{lemma}[Recurrent virial equality case and routing]
...
normalization.  Assume that the log-annular virials in
\Cref{lem:log-annular-oscillation-virial} are legitimate compact observables on
the recurrent hull of \(\mathcal D\), in the sense of
\Cref{rem:virial-normalization-warning}, and let \(\mathbb P\) be an invariant
probability measure on that hull for the joint time-translation and
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   \[
      \mathcal D\in\Caff .
   \]
   \item \emph{Tight Dirac flux routing.}  Suppose
   ...
   \]
\end{enumerate}
The first-bad mesoscopic routing theorem proved below supplies the tight-Dirac
alternative for realized recurrent defects.  Combined with that theorem, the
preceding alternatives give the routing
\[
...
We now prove the tight Dirac flux routing.  Since the averaged flux does not
...
oscillation-active subcylinder.  Since the flux is not lost to logarithmic
...
\]
on every fixed annular cylinder.  The terminal compactness package gives a
...
has nonzero affine-normalized oscillation.  Thus \((W,Q)\in\Ccohcrit\).
\end{proof}

\subsection{Logarithmic escape as a hidden-scale obstruction}
\label{subsec:logarithmic-escape-hidden-scale}

The logarithmic escape alternative is not discharged below by a separate global
...
the Young alternative are both reduced to the same first-bad mesoscopic routing
theorem.
...
retains normalized affine oscillation mass.
\end{lemma}
...
affine-normalized oscillation.  No positive lower bound for the unnormalized
...
\(\mathcal F\).  In particular, if \(\mathcal F\) has already been discharged
by the lower-stratum closure theorems, then \(\mathcal D\) is impossible.
\end{lemma}

\begin{proof}
The only point needing clarification is that the selected logarithmic windows
may have unnormalized mass tending to zero.  This causes no loss of ancestry.
...
\(\mathcal F\) is closed under retained descendants and is already discharged,
the ancestor defect has no admissible surviving normalized support state.  The
fact that \(\lambda_n(I_n)\) may tend to zero is irrelevant: the normalized
restriction records a support state of the ancestor probability measure, and a
positive ancestor defect measure cannot be supported entirely on discharged
closed alternatives.
\end{proof}
...
its affine-normalized oscillation vanishes, then
\Cref{thm:affine-normalization-dichotomy} places it in \(\Caff\).  We may
therefore assume that it is neither active nor affine.
...
In all cases the log-window descendant lies in the closed discharged set
\[
...
\begin{corollary}[Log-diffuse alternative discharge from first bad scale routing]
...
escape is discharged by the same hidden-scale compactness theorem that removes
...
active-locus removal implies that \(\sup_z\mathcal A_n(z,1)\) is below the
retained oscillation threshold on the residual exterior window, after the fixed
cover constants are absorbed into the threshold.
\end{definition}

\begin{definition}[First bad mesoscopic scale]
\label{def:first-bad-mesoscopic-scale}
Let \(R_n\to\infty\) be the macroscopic scale of a tight critical-tail
blow-down.  A first bad mesoscopic scale is a sequence of covariant centers
...
the segment joining \((x,s)\in K\) to \((x+h_n,s+\theta_n)\) has
affine-normalized oscillation \(o(1)\).  Then there are
...
corresponding descendant has zero affine-normalized spatial oscillation on
every compact cylinder but a nonzero homogeneous time-dependent component.
That is the affine/parasitic lower stratum in the ordered state-space
decomposition.
\end{proof}

\begin{lemma}[Translation defect produces activity, affine collapse, or a bad radius]
...
positive affine-normalized oscillation lower bound, alternative (i) holds.
Otherwise the affine oscillation on every cylinder in the chain tends to zero.
...
cylinders had affine-normalized oscillation below a sufficiently small
...
\begin{lemma}[Failure of hidden-scale compactness produces a first bad scale]
...
stratum \(\Caff\) is reached by a descendant, or there is a first bad
mesoscopic scale in the sense of
...
slightly lowering \(\varepsilon_0\).  This is a first bad mesoscopic scale.
\end{proof}

\begin{definition}[First-bad spatial blow-down]
\label{def:first-bad-spatial-blowdown}
Let \(z_n\) and \(r_n\) be a first bad mesoscopic scale.  Write
\[
...
affine oscillation on each compact time interval.  The first-bad spatial
blow-down is
\[
...
\begin{lemma}[First-bad blow-down equation]
\label{lem:first-bad-blowdown-equation}
Let \(V_n,W_n,Q_n,c_n\) be the first-bad blow-downs of
...
\begin{lemma}[First-bad quotient formulation]
\label{lem:first-bad-quotient-formulation}
Let \(V_n,W_n,c_n,Q_n\) be as in
\Cref{lem:first-bad-blowdown-equation}.  The first-bad analysis uses the
...
under the first-bad spatial blow-down for \(V_n\), because \(V_n\) is obtained
...
does not change any affine-normalized oscillation seminorm in which the same
class of homogeneous gauges is minimized over.  If the sequence \(c_n\) is not
...
Every normalized Seregin, terminal, covariant descendant, first-bad blow-down,
or active descendant profile used in the residual state space is represented by a
...
\(\beta(s)\cdot y\) is exactly the affine pressure paired with the
spatially homogeneous parasitic velocity mode.  Outside the affine/parasitic
stratum \(\Caff\), the representative is taken with \(\beta=0\).
\end{proposition}

\begin{proof}
At the finite-energy Leray level, choose the standard whole-space pressure
representative.  For the approximating suitable Leray--Hopf solutions one has,
modulo a function of time,
\[
   p_n=\mathcal R_i\mathcal R_j(u_{n,i}u_{n,j})
\]
in \(\mathcal D'(\mathbb R^3)\).  This formula is stable under every operation
used before the affine quotient:
physical Type~I scaling, the change to centered variables, centered time
translations, spatial translations, covariant translations, and local smooth
limits after the admissible time gauges.  Indeed each operation either
conjugates the Riesz representative by a Euclidean/scaling change of variables
or adds only a time-dependent gauge.  Therefore no harmonic pressure component
with nonzero Hessian can be generated from the finite-energy approximants.

The only operation that changes the velocity by a spatially homogeneous vector
is the affine/parasitic normalization.  For a smooth homogeneous mode \(b(s)\),
substitution into the centered equation gives the affine pressure gradient
\[
   \nabla_y\bigl[(-\partial_s b-\tfrac12 b)\cdot y\bigr].
\]
For merely bounded measurable homogeneous gauges this expression is interpreted
only as the quotient distribution described in
\Cref{def:affine-constant-lower-stratum,lem:first-bad-quotient-formulation}.
Thus the only non-Riesz pressure direction admitted in the normalized state
space is the affine term \(\beta(s)\cdot y+\alpha(s)\).

Conversely, suppose a normalized limit contained a residual harmonic pressure
component \(h(\cdot,s)\) which was not affine in \(y\).  Then
\(\nabla^2h\not\equiv0\) on some compact ball and time interval.  Testing the
approximating pressure identities against compactly supported functions with
zero spatial mean and passing to the local pressure-gauge limit would force this
nonzero Hessian component to arise as a limit of zero Hessian components of the
allowed gauges, which is impossible.  Equivalently, the difference between the
limit pressure and the Riesz representative is harmonic and is produced by the
allowed gauges; the only such gauges in the normalized state space are
time-dependent constants and affine parasitic terms.  Hence
\eqref{eq:normalized-pressure-representation} is the full pressure
representation.

Finally, when \(\beta\ne0\) has a retained effect, the associated velocity
component is spatially homogeneous and belongs to the affine/parasitic lower
stratum \(\Caff\).  Outside that stratum the canonical parasitic-free
representative has \(\beta=0\).
\end{proof}
...
\end{equation}
The same statement holds for the first-bad pressures \(Q_n\) in the quotient
...
\eqref{eq:scaled-pressure-compactness-affine}.  The first-bad pressure is used
only up to the affine quotient described in
\Cref{lem:first-bad-quotient-formulation}; in that quotient the same estimate
applies.
\end{proof}

\begin{lemma}[Pressure compactness at a first bad scale]
...
first-bad scaling.  If the affine gauges retain nonzero descendant activity,
...
product convergence in distributions.
\end{proof}
...
affine-normalized spatial oscillation on every compact cylinder, while the
displayed lower bound gives a nonzero homogeneous time-dependent descendant.
...
Let \(V_n,W_n,Q_n,c_n\) be a first-bad blow-down.  Then either the sequence has
...
\(\Caff\), in which case we are done, or the pressure gradient limit in the
...
an affine/parasitic quotient direction.  If it has retained effect, the profile
...
\begin{proof}
Mollify \(F\) in space-time and apply the standard renormalized
characteristic argument; then let the mollification parameter tend to zero.
Equivalently, test the distributional inequality against the adjoint transport
kernel.  Along a backward characteristic
\[
   x(\sigma)=e^{(\sigma-s)/2}x,\qquad \sigma\le s,
\]
the inequality gives, for every \(T>0\),
\[
   F(x,s)\le e^{-T}F(e^{-T/2}x,s-T)
\]
at Lebesgue points of \(F\).  Since \(F\le C\), we obtain
\[
   F(x,s)\le Ce^{-T}.
\]
Letting \(T\to\infty\) gives \(F(x,s)=0\) at every Lebesgue point, hence
almost everywhere.  The distributional inequality then gives \(F\equiv0\).
\end{proof}

\begin{lemma}[First-bad blow-downs are strongly compact]
\label{lem:first-bad-strong-compactness}
Every first-bad blow-down \(W_n\) either has a retained affine/parasitic
...
\begin{theorem}[First bad mesoscopic scale routing]
\label{thm:first-bad-mesoscopic-routing}
Every first bad mesoscopic scale generated by a realized recurrent
...
\begin{proof}
Let \(z_n,r_n\) be a first bad mesoscopic scale.  If, before passing to the
first-bad blow-down, some unit covariant observer in the selected residual
window carries a positive affine-normalized oscillation threshold, then the
active alternative is regenerated and the descendant lies in \(\Cactive\).  We may
therefore assume unit activity has been removed at all fixed positive
thresholds.

Form the first-bad blow-down \(W_n\) of
...
\[
   ...
   \qquad \nabla\cdot W=0 .
\]
The first-bad lower bound and strong convergence give
\[
   \inf_{c\in L^\infty((-1,1);\mathbb R^3)}
   \int_{-1}^{1}\int_{B_1}|W-c(s)|^3\,dx\,ds
   \ge \varepsilon_0 .
\]
...
\(\Ccohcrit\).  Therefore every first bad mesoscopic scale routes to
\(\Cactive\), \(\Caff\), or \(\Ccohcrit\), as claimed.
\end{proof}

\begin{remark}[Critical-tail routing theorem]
\label{rem:remaining-hidden-scale-input}
\Cref{thm:first-bad-mesoscopic-routing} is the critical-tail alternative-routing
...
\begin{theorem}[No hidden-scale variance from first bad scale routing]
...
regenerated, the affine stratum is reached, or a first bad mesoscopic scale
...
\begin{proof}
Let \(\mu_n^{\rm osc}\) be the affine-normalized oscillation defect measure
after active-locus removal.  By the mixed-diffuse hypothesis there are escaping
...
\[
   ...
   \frac{\mu_n^{\rm osc}\lfloor E_n}{\mu_n^{\rm osc}(E_n)} .
\]
By \Cref{thm:diffuse-defect-compactness}, after passing to a subsequence these
probability measures converge to a boundary diffuse-defect state.  Because all
positive-threshold active observers have been removed, the limit gives no mass
to ordinary unit active observers in \(Z\).
...
\begin{proof}
If the covariant observer orbit closure of \(\mathcal T\) intersects an
affine-normalized active set \(\{U:\operatorname{osc}_3(U;Q_1^{\rm cov})\ge
\eta\}\) for some \(\eta>0\), then alternative \emph{(a)} holds.  We remove this
case first.

If every affine-normalized oscillation observable vanishes on the state, then
...
\emph{(b)}.  We therefore assume from now on that the state has nonzero
...
The critical-tail classification above is a classification theorem, not by
itself an emptiness theorem.  The complete residual decomposition is discharged by
local lower-stratum closures together with the first-bad mesoscopic routing
theorem \BranchClosurePackage.  That theorem first gives hidden-scale
...
\Cref{lem:hidden-variance-young-obstruction}.  The critical-tail routing theorem
used in both reductions is \BranchClosurePackage.  These are local state-space
...
Let \((U,\Pi)\) be a retained residual state after the earlier Paper I--III
lower classes have been removed.  Then exactly one of the following holds:
\begin{enumerate}[label=\textup{(\roman*)}]
   \item after an admissible affine symmetry \(S_c\), the profile retains
   positive non-affine oscillatory activity:
   \[
      \Phi(S_cU)>0;
   \]
   \item the profile belongs to the affine/parasitic stratum \(\Caff\).
\end{enumerate}
Consequently the retained residual decomposition has
\[
   \Caff=\varnothing .
\]
\end{theorem}
...
\begin{proof}
The dichotomy is exactly \Cref{thm:affine-normalization-dichotomy}.  If
\(\Phi(S_cU)=0\), then \(S_cU\) is globally constant and the profile is in the
affine/parasitic lower stratum \(\Caff\).  If the residual state is retained
after lower-stratum removal, this alternative is not available.  Hence a
positive affine-normalized oscillatory activity threshold survives.  A state in
\(\Caff\) therefore cannot carry retained residual concentration, so
\(\Caff=\varnothing\) inside the retained residual decomposition.
\end{proof}

\begin{corollary}[Young alternative discharge from first bad scale routing]
...
\begin{theorem}[Critical-tail rigidity from first bad scale routing]
...
contradicting the nonzero affine-normalized oscillation of a realized coherent
...
\begin{corollary}[Critical-tail discharge, complete form]
...
neighborhoods of the active locus at every positive affine-normalized activity
...
\end{equation}
in the bounded mild duality sense of Paper~II.  The same conclusion is
...
\begin{proof}
For each \(m\), the Paper~I/Paper~II normalized representative satisfies the
...
duality formulation of Paper~II then extends the identity from compactly
supported tests to the standard bounded mild class, yielding
...
\begin{theorem}[Parasitic-free mildness inheritance from the normalized package]
...
normalized residual state space generated by the Paper~I Seregin extraction.
...
Paper~I constructs normalized Seregin limits as centered bounded mild ancient
representatives in the Paper~II sense, after removal of the spatially
homogeneous parasitic mode.  The stability statement
...
status as a terminal profile generated directly by the Paper~I extraction.
...
\begin{enumerate}[label=\textup{(\roman*)}]
   ...
   in which case that descendant is already discharged;
...
by Paper I.  It is not enough to say that the diffuse or noncompact state is not
the sole carrier of the original compact concentration: the compact core
\((U_j,\Pi_j)\) may coexist with that tail state.  The paired exclusion
...
\]
be the compact affine-normalized activity set of
...
\Cref{thm:terminal-exhaustion-main}.  The old pure radiative alternatives are
only sole-carrier exclusions, and are used only in that form through
...
velocity mass from Paper~I still forces the terminal active locus to be
nonempty by \Cref{cor:persistent-forces-active}; after affine normalization the
surviving residual activity is measured by \(\Phi\).  Pure radiative
...
\Cstat
   ...
   \varnothing,\\
\Caff
   &\xrightarrow{\;\hbox{oscillatory entry}\;}
   \varnothing,\\
\Clogdiff
   &\xrightarrow{\;\hbox{log-window descendants + hidden-scale closure}\;}
   \varnothing,\\
\Cyoung
   &\xrightarrow{\;\hbox{first bad mesoscopic routing}\;}
   \varnothing,\\
...
Equivalently, the final exclusion is the composition of six certificates:
\begin{enumerate}[label=\textup{(\arabic*)}]
   \item \emph{Entry certificate:} Paper~0 supplies a positive local velocity
   concentration sequence; the endpoint weak Serrin local Type~I package
   ...
   Paper~I turns it into a normalized Seregin profile with retained compact
   velocity concentration.
   \item \emph{Stratification certificate:} the ordered residual decomposition
   ...
   \[
      ...
      \cup\Cgen .
   \]
   \item \emph{Closed-strata certificate:} the small, stationary \(L^3\),
   tight, closed structured/decay, axisymmetric, rotational, and
   stationary-hull strata are discharged by the results cited above.
   \item \emph{Lower-stratum certificate:} \(\Caff\) is discharged by
   oscillatory entry normalization.  The alternative \(\Clogdiff\) is discharged by
   ...
   discharged by the same hidden-scale compactness; see
   \Cref{cor:young-branch-discharge}.  The alternatives
   \(\Chomcrit,\Clogper,\Capcrit\) are discharged by bounded-origin
   realization of coherent critical tails.
   \item \emph{Residual terminal certificate:} the generic residual is reduced
   ...
   \item \emph{Endpoint Liouville certificate:} a terminally indecomposable
   retained profile is either affine/parasitic lower-stratum or has a
   parasitic-free bounded mild physical pullback with a backward
   \(L^3\)-bounded sequence, and is therefore zero by Albritton--Barker.
\end{enumerate}
...
\Cref{thm:R3S-R3-empty}.  The affine stratum is empty by
\Cref{thm:oscillatory-entry-normalization}.  The log-diffuse and Young
...
structured alternatives.  The terminal active-locus extraction, diffuse-defect
compactness, active path-space recurrence, and finite separated-family exclusion
are not imported: they are implemented above by local concentration analysis.
The realized critical-tail rigidity statement is proved above as an explicit
local state-space stratification of the critical boundary compactification, not
a hidden consequence of local CKN observables.  The affine lower stratum and the
coherent critical-tail alternatives are closed above by oscillatory entry
normalization and bounded-origin realization.  The log-diffuse and Young lower
alternatives are both reduced above to hidden-scale compactness.  The complete
discharge of hidden recurrent critical-tail defects is isolated in
\BranchClosurePackage; it is proved here by the first-bad blow-down,
local pressure compactness, and bounded ancient variance Liouville argument.
The rotational
