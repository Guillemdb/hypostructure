...
The proof uses two inputs stated below in this paper.  The uniformly
\(L^3\)-tight class is excluded by the tight-class closure theorem, and the
remaining class is excluded by the residual closure theorem.  The
structure/decay class is restricted to hypotheses covered by cited Liouville
theorems.
...
\item Uniformly \(L^3\)-tight: assumed in \Cref{hyp:tight-liouville} as the
endpoint \(L^3\) closure input.
\item Known structure/decay: excluded by the cited theorems in
\Cref{thm:known-structure-decay-liouville}.
\item Remaining class: assumed in \Cref{hyp:no-remainder}, the residual
closure input imported from the proof setup \cite{ProofSetupTypeI}.
\end{itemize}
...
exhaustion: \cref{hyp:tight-liouville} is stated as an endpoint \(L^3\)
closure assumption, and \cref{hyp:no-remainder} is the residual closure
assumption imported from \cite{ProofSetupTypeI}.  The structure/decay
...
the stated admissibility condition, the extraction proved here, and the
stated closure inputs.
...
\begin{proof}
This is the velocity-only concentration corollary of the local
\(\varepsilon\)-regularity criterion.  Indeed, if
\[
...
Every \(V\in\calS\) is bounded.  In the endpoint \(L^3\) closure input
\Cref{hyp:tight-liouville}, the tail condition \eqref{eq:L3-tightness} is
assumed to give the endpoint sequence-\(L^3\) control needed after physical
pullback; no separate \(L^\infty_\tau L^3_y\) hypothesis is imposed in the
definition of the tight class.
...
Gallay--Wayne weighted-vorticity hypotheses stated in the Liouville theorem
used below;
\end{enumerate}
\end{definition}
...
\begin{proof}
For bounded centered ancient profiles, the tail-tightness condition gives the
endpoint sequence-\(L^3\) control after physical pullback.  Applying the
Albritton--Barker endpoint ancient Liouville theorem then forces the associated
physical ancient solution to vanish.  Pulling this conclusion back to centered
variables contradicts the compact local velocity normalization in
\eqref{eq:nonvanishing}.
\end{proof}
...
the axial-periodic bounded-circulation case is excluded by Lei--Ren--Zhang
  ...
  are excluded by the weighted-vorticity Liouville theorem for the stated
  perturbative class.  Each
...
\begin{assumption}[Residual closure input from the proof setup]\label{hyp:no-remainder}
...
\]
This is the only input imported from the conditional proof setup
\cite{ProofSetupTypeI}.  The needed content is exactly the emptiness of the
remaining class after the ordered removal of the small, stationary \(L^3\),
uniformly \(L^3\)-tight, and Liouville-covered structure/decay classes
defined in the present paper.  No other result from the proof setup is used in
