...
\title[Type I blow-up limits and residual closure]
{Type I Blow-up Limits and Residual Closure for the
...
\keywords{Navier--Stokes equations, Type I singularities, ancient solutions,
suitable weak solutions, Seregin limits, Liouville theorems}
...
Caffarelli--Kohn--Nirenberg theory.  It yields a smooth bounded ancient profile
...
the companion tight-dynamics paper, and the remaining class is closed in the
companion residual paper by local concentration analysis.  Consequently the
Type I exclusion obtained from this extraction is unconditional within the
stated admissible finite-energy class.
\end{abstract}
...
\cref{def:admissible-type-I}.  It replaces a stronger all-scale compactness
...
of \eqref{eq:centered}.  We therefore work with a finite decomposition based
on explicit Liouville inputs.  A normalized Seregin limit is compared with the
...
\(\R^3\times(-\infty,0)\), and a centered profile \((V,\Pi)\) on
...
satisfies the local \(L^3\) nonvanishing condition \eqref{eq:nonvanishing}.
\end{theorem}
...
\label{thm:remaining-criterion}
No admissible finite-energy Type I singularity exists.
\end{theorem}

The proof uses two companion results.  The uniformly \(L^3\)-tight class is
excluded in Paper II \cite{DuranPaperII}.  The remaining class is excluded in
the residual closure paper \cite{DuranResidual2026}.  The structure/decay
...
\Downarrow
\\
\text{contradiction by the closed classes and residual closure.}
\end{array}
\]

\begin{itemize}[leftmargin=*,itemsep=2pt]
\item Small amplitude: excluded here in \Cref{sec:liouville}.
\item Stationary \(L^3\): excluded here by \cite{NRS1996}.
\item Uniformly \(L^3\)-tight: proved in \Cref{hyp:tight-liouville}, citing
Paper II.
\item Known structure/decay: excluded by the cited theorems in
\Cref{thm:known-structure-decay-liouville}.
\item Remaining class: proved in \Cref{hyp:no-remainder}, citing the residual
paper.
\end{itemize}

\subsection{Dependencies and discharged inputs}

The proof of the extraction theorem uses the local velocity concentration
input stated as \cref{prop:local-velocity-concentration}; this is the only
...
exhaustion, and both are proved in companion papers:
\cref{hyp:tight-liouville} is Paper II's endpoint \(L^3\) closure, and
\cref{hyp:no-remainder} is the residual closure theorem.  The structure/decay
class is restricted by definition to assumptions covered by the cited
Liouville theorems.  Thus the final Type I exclusion uses no assumption beyond
the Paper~0 admissibility condition, the extraction proved here, and the
closed companion inputs cited above.
...
Liouville classes and the remaining class.  \Cref{sec:liouville} records the
class exclusions and the uniformly tight input.  \Cref{sec:criterion} proves
the remaining-case criterion.  Appendix \ref{app:centered-symmetries} records
...
concentration theorem; see \cref{prop:local-velocity-concentration}.  Thus,
for a singular point, the additional admissibility requirement is the
no-escape condition \eqref{eq:velocity-no-escape}.  This is the only
...
\(\sigma\downarrow0\) gives \eqref{eq:velocity-no-escape} for the tail of the
...
\label{prop:local-velocity-concentration}
...
result in Paper 0 \cite{DuranPaper0}.  That result follows from the
...
The singularity enters the proof through the velocity concentration sequence in
\cref{def:admissible-type-I}, supplied by
\cref{prop:local-velocity-concentration}.  The no-escape condition says that
...
sense of \cref{def:admissible-type-I}.  Then there are
...
The preceding proposition is the point at which singularity enters the
compactness argument.  Without the velocity concentration and the
...
\]
For nontriviality, \cref{prop:nonzero} gives a positive local \(L^3\) lower bound
...
\cref{thm:extraction} along an admissible concentration sequence and satisfies
the local \(L^3\) nonvanishing condition \eqref{eq:nonvanishing}.
\end{definition}

\begin{definition}[Collection of normalized Seregin limits]
\label{def:seregin-collection}
...
ancient limit is nonzero in the sense of \eqref{eq:nonvanishing}.
\end{remark}

\begin{proof}[Proof of \cref{thm:extraction}]
Choose an admissible concentration sequence
\(\lambda_k\downarrow0\) from \cref{def:admissible-type-I}, and form the
rescaled sequence \eqref{eq:rescaled-sequence}.  The compact ancient limit is
...
\section{Liouville classes and the remaining class}\label{sec:classes}
...
\begin{definition}[Small-amplitude class]\label{def:small-class}
...
\begin{definition}[Stationary \(L^3\) class]\label{def:stationary-class}
...
\begin{definition}[Uniformly \(L^3\)-tight class]\label{def:tight-class}
...
Every \(V\in\calS\) is bounded.  Paper II shows that, for such profiles, the
tail condition \eqref{eq:L3-tightness} gives the endpoint sequence-\(L^3\)
...
\label{def:known-structure-decay-class}
...
\cite{DuranPaperIII};
\item \(U\) has bounded circulation \(\Gamma=rU_\theta\) and \(\Gamma\)
satisfies the coercive shell-control hypotheses in the scalar flattening
theorem of \cite{DuranPaperIII}.
\end{enumerate}
\end{definition}

\begin{remark}
Only hypotheses for which a cited Liouville theorem gives \(U\equiv0\) are
included in \(\calC_{\mathrm{known}}\).  Broader qualitative descriptions such
as controlled swirl, weighted velocity decay, Burgers-vortex asymptotics, or
bounded nonperiodic circulation are not included unless they imply one of the
listed hypotheses.
\end{remark}
...
\begin{definition}[Remaining class]\label{def:remaining-class}
...
\begin{proposition}[Exhaustion by defined classes]\label{prop:exhaustion}
...
\section{Liouville hypotheses and class exclusions}\label{sec:liouville}
...
\label{hyp:tight-liouville}
No nonzero normalized Seregin ancient limit belongs to
\(\calC_{L^3\mathrm{-tight}}\).
\end{theorem}

\begin{proof}
This is the tight-class Liouville theorem proved in Paper II
\cite[Corollary~\emph{Tight-class input for the residual criterion}]
{DuranPaperII}.  Paper II proves that, for bounded centered ancient profiles,
...
Liouville theorem and contradicts the Paper I compact velocity normalization.
\end{proof}

\begin{theorem}[Liouville-covered structure and decay exclusions]
\label{thm:known-structure-decay-liouville}
No nonzero normalized Seregin ancient limit belongs to
\(\calC_{\mathrm{known}}\).
\end{theorem}

\begin{proof}
By \cref{def:known-structure-decay-class}, membership in
...
and the bounded-circulation shell-control hypotheses are excluded by the
corresponding Liouville theorems in Paper III \cite{DuranPaperIII}.  Each
alternative gives \(V\equiv0\), contradicting the local \(L^3\) nonvanishing
condition \eqref{eq:nonvanishing}.  Hence no nonzero normalized Seregin
ancient limit lies in \(\calC_{\mathrm{known}}\).
\end{proof}

\begin{proposition}[Exclusion of the Liouville classes]
\label{prop:known-class-exclusion}
...
\cref{thm:classical-classes}.  The uniformly tight class is excluded by
\cref{hyp:tight-liouville}.  The structure/decay class is excluded by
\cref{thm:known-structure-decay-liouville}.
\end{proof}

\section{The remaining-case criterion}\label{sec:criterion}

\begin{theorem}[Residual closure theorem]\label{hyp:no-remainder}
...
\begin{proof}
This is the Paper I residual input proved in the residual paper
\cite[Corollary~\emph{Paper I residual input}]
{DuranResidual2026}.  The residual paper uses the same Paper I class
exhaustion.  Its notation \(\calR(\calS;I,J)\) records the ordered removal of
the small, stationary \(L^3\), uniformly \(L^3\)-tight, and Liouville-covered
structure/decay classes, together with the refined lower residual classes.  No
additional analytic hypothesis is introduced by the indices \(I,J\).  Its four
local exclusions give \(\calR(\calS;I,J)=\varnothing\), which is precisely the
emptiness of the Paper I remaining class \(\calR(\calS)\).
\end{proof}

\begin{corollary}[Rigidity in the remaining case]\label{hyp:remainder-rigidity}
For every collection \(\calS\) of normalized Seregin ancient limits generated
...
\begin{proof}
This is immediate from \cref{hyp:no-remainder}.
\end{proof}

\begin{proposition}[Equivalence on normalized Seregin limits]
\label{prop:remainder-equivalence}
For collections of normalized Seregin limits, \cref{hyp:no-remainder} and
\cref{hyp:remainder-rigidity} are equivalent.
\end{proposition}

\begin{proof}
If \cref{hyp:no-remainder} holds, the rigidity statement is vacuous.
Conversely, assume \cref{hyp:remainder-rigidity}.  If some normalized
...
\eqref{eq:nonvanishing}.  Hence the remaining class is empty.
\end{proof}

\begin{proof}[Proof of \cref{thm:remaining-criterion}]
Suppose that \((u,p,z_*)\) is an admissible finite-energy Type I singularity.
By \cref{thm:extraction}, there exists a normalized nonzero Seregin ancient
limit \(V\).  Let \(\calS\) be any collection of normalized limits containing
\(V\).

By \cref{prop:exhaustion}, either \(V\) belongs to one of the Liouville classes
or \(V\in\calR(\calS)\).  The first alternative is impossible by
\cref{prop:known-class-exclusion}.  The second alternative is impossible by
\cref{hyp:no-remainder}.  Hence the assumed admissible Type I singularity
cannot exist.
\end{proof}

The preceding theorem is the unconditional Type I conclusion for the stated
admissible finite-energy class.  The compactness construction, pressure
...
\cref{prop:local-velocity-concentration}.  The structure/decay class is
restricted to the assumptions in \cref{def:known-structure-decay-class}; any
other structured behavior lies in the remaining class and is covered by
\cref{hyp:no-remainder}.  No whole-space critical-norm estimate on the
original solution is used in this closure step.

\appendix

\section{Elementary symmetries of the centered equation}
\label{app:centered-symmetries}

\begin{lemma}[Logarithmic-time translation]\label{lem:tau-translation}
...
\]
Using the equation for \(V\), the equation defect for \(\widetilde V\) is
\[
