...
For whole-space finite-energy Type I terminal singularities, the velocity
no-escape condition used in the companion Type I blow-up analysis is automatic.
\end{abstract}
...
that local argument are defined below.  The final no-escape section adds a
whole-space finite-energy Type I hypothesis only to fix the standard Leray
pressure gauge and to handle fixed rescalings near the terminal time; its
uniform estimates are still uniformly local and do not use the size of the
global energy or global dissipation.
...
sequence used in the Type I blow-up analysis.  In the companion paper, the
remaining compactness input is a no-escape condition excluding the possibility
...
We record the interface used by the companion residual paper
\cite{DuranPaperIV}.  The point is that the local pointwise Type I alternative in
\cref{def:typeI-concentration} is already strong enough, after rescaling at
the singular point, to give the compactness input required by the terminal
concentration package.  Thus a finite-energy singular point satisfying the
local pointwise Type I bound enters the terminal profile analysis without adding a
separate whole-space no-escape hypothesis.
...
\begin{lemma}[Local pointwise Type I concentration enters the terminal dichotomy]
\label{lem:paper0-local-typeI-into-terminal-dichotomy}
Assume the companion residual paper's realized critical-tail rigidity input.
Under the hypotheses of
\cref{prop:paper0-local-typeI-terminal-compactness}, there exist
\(\eta_v>0\) and a sequence \(r_n\downarrow0\) such that
\begin{equation}\label{eq:paper0-local-typeI-bridge-velocity-concentration}
  \int_{Q_1(0,0)} |u_n|^3\,dx\,dt\ge\eta_v
  \qquad\text{for all }n.
\end{equation}
Consequently, after applying the terminal concentration package of the
companion residual paper \cite{DuranPaperIV} with a threshold
\(\varepsilon_{\rm sd}\le\eta_v\), the retained compact concentration issued
...
residual remainder.  With realized critical-tail rigidity made explicit in the
companion paper, and with active path-space recurrence proved there, the
companion package excludes mixed compact--diffuse remainders,
...
\begin{proof}
Since \(x_*\in\Sigma(T)\), \cref{cor:velocity-concentration} supplies
\(\eta_v>0\) and \(r_n\downarrow0\) with
\[
  C(z_*,r_n)
  =r_n^{-2}\int_{T-r_n^2}^{T}\int_{B_{r_n}(x_*)}|u|^3\,dx\,dt
  \ge\eta_v .
\]
After the rescaling \eqref{eq:paper0-local-typeI-bridge-rescaling}, this is
exactly \eqref{eq:paper0-local-typeI-bridge-velocity-concentration}.  The
compactness part follows from
\cref{prop:paper0-local-typeI-terminal-compactness}.  The local CKN measure in
the terminal package dominates the velocity contribution, so the origin
recentering is \(\varepsilon_{\rm sd}\)-concentrating whenever
\(\varepsilon_{\rm sd}\le\eta_v\).  The terminal exhaustion theorem of the
companion residual paper is paired: it records the active retained profile
locus together with a residual remainder.  Its diffuse-defect trichotomy and
critical-tail compactification route compact core plus diffuse/noncompact
remainders to the realized critical-tail rigidity theorem, while its active
path-space recurrence rules out infinite active cascades.  Therefore only a
single retained profile or a finite separated retained family remains at this
bridge stage.
\end{proof}

\begin{theorem}[Local pointwise Type I entry into the full exclusion pipeline]
\label{thm:paper0-local-typeI-entry-pipeline}
Assume the companion terminal concentration package and residual exclusions of
\cite{DuranPaperIV}, including the realized critical-tail rigidity input stated
there.  Let \((u,p)\) be a finite-energy suitable weak solution
...
concentration, and the companion terminal package applies to that profile.  In
particular, no finite-energy local pointwise Type I singular point exists once
the terminal package is included.
\end{theorem}

\begin{proof}
By \cref{lem:paper0-local-typeI-into-terminal-dichotomy}, the local pointwise Type I
...
For residual candidates, the terminal local stratification theorem of the
residual paper applies: finite separated retained families are excluded there,
infinite active branches are excluded by active path-space recurrence, mixed
compact--diffuse tails are routed through critical-tail compactification and
realized critical-tail rigidity, and a remaining single retained profile is forced through indecomposability,
parasitic-free mildness inheritance, and the endpoint Liouville contradiction.  Thus the
assumed local pointwise Type I singular point cannot exist in the completed pipeline.
\end{proof}

\begin{corollary}[Finite-energy singularities are locally Type II]
\label{cor:paper0-local-typeII-after-bridge}
In the completed Papers~0--IV pipeline, assuming the companion residual
paper's realized critical-tail rigidity input, every
finite-energy singular point falls into the local Type II alternative of
\cref{def:typeII-alternative}.
\end{corollary}

\begin{proof}
By \cref{thm:typeI-typeII-dichotomy}, every singular point lies in either the
local Type I alternative or the local Type II alternative.  The local Type I
alternative is excluded by
\cref{thm:paper0-local-typeI-entry-pipeline}.  Therefore only the local Type II
alternative remains.
\end{proof}
...
companion manuscript in the Type I chain, 2026.
