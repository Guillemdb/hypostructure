...
\cite{DuranPaperIV}.  The local pointwise Type I alternative in
...
companion residual paper also records the endpoint weak-Serrin bridge:
...
concentration sequences are admissible for the Paper I Seregin extraction.
...
\cref{def:paper0-admissible-local-bridge-sequence}.  The companion residual
paper proves, using the Albritton--Barker weak Serrin local Type I estimate,
that the local pointwise Type I envelope implies terminal velocity no-escape
for every rescaled sequence
\cite{AlbrittonBarker2019,DuranPaperIV}.  This is exactly the second requirement in
\cref{def:paper0-admissible-local-bridge-sequence}.
\end{proof}
...
residual remainder.  With realized critical-tail stratification and discharge
proved in the companion paper, and with active path-space recurrence proved there, the
...
The terminal exhaustion theorem of the
...
In the completed Papers~0--IV pipeline, the local Type I alternative is
