...
I alternative.  By endpoint compactness, after passing to a
subsequence the rescaled pairs converge to a suitable limit on \(Q_1\), with
strong \(L^3\)-convergence of the velocities on compact subcylinders and weak
\(L^{3/2}\)-compactness of the normalized pressures.  The velocity term
therefore passes to the limit strongly, while the pressure term is lower
semicontinuous under weak \(L^{3/2}\)-convergence.  Hence a positive amount of
the pressure-normalized critical density persists on \(Q_1\).  By inner
regularity of the integral, some compact cylinder
\(K_0\Subset Q_1\cap\{t<0\}\) still carries a positive amount of this density.
After decreasing the constant and discarding finitely many indices,
\eqref{eq:selected-lower-bound} holds.
\end{proof}
