...
local compactness, local energy stability, and non-triviality normalization
needed in later Liouville arguments.  The resulting renormalized ancient field
solves
\[
...
assertion is stated using a scale-selection normalization.  This formulation is
the one needed for subsequent compactness and Liouville arguments.
...
scales may be chosen so that \(U\not\equiv0\).  Equivalently, the
renormalized field \(V\) defined by \eqref{eq:self-sim-vars-intro} satisfies
\[
...
\end{equation}
for some \(R_0,\eta_0>0\).  Finally \(V\) is a smooth solution of
\eqref{eq:centered-intro} on \(\R^3\times\R\).
\end{theorem}
...
the pressure does not change the local energy inequality, because
\[
   \int_{\R^3} a_k(t)u_k(\cdot,t)\cdot\nabla\phi(\cdot,t)\,dx=0
\]
for divergence-free \(u_k\).  The terms containing
...
\begin{proof}
On each compact cylinder \(Q\Subset\R^3\times(-\infty,0)\), the sequence is
bounded in \(L^2_tH^1_x(Q)\) by \cref{lem:energy-diss}, and its time derivative
is bounded in \(L^{3/2}_tW^{-1,3/2}_x(Q)\) by \cref{lem:time-derivative}.
Aubin--Lions--Simon compactness \cite{Simon1987} gives strong compactness in
\(L^2(Q)\).  The uniform \(L^\infty(Q)\) bound from \cref{lem:inherited-bound}
upgrades this to strong \(L^q(Q)\) convergence for every finite \(q\).  A
diagonal argument over compact cylinders gives the local statement.  The
pressure convergence follows from \cref{lem:pressure}.
\end{proof}
...
\begin{lemma}[Concentration at a singular point]\label{lem:singular-concentration}
If \(z_*=(x_*,T)\) is singular, then for every sufficiently small \(r>0\),
\[
   C(u;z_*,r)+D(p;z_*,r)\ge \eps_*,
\]
where \(\eps_*\) is the constant in \cref{thm:eps-reg}.
\end{lemma}

\begin{proof}
If the inequality failed for some sequence \(r_j\downarrow0\), then
\cref{thm:eps-reg} would imply regularity in \(Q_{r_j/2}(z_*)\) for some
\(j\), contradicting the singularity of \(z_*\).
\end{proof}
...
There are scales \(\lambda_k\downarrow0\), radii \(R_0<\infty\), times
\(-S_0<-\sigma_0<0\), and a number \(c_0>0\) such that the rescaled sequence
\eqref{eq:rescaled-sequence}, after pressure gauges are fixed as above,
satisfies
\begin{equation}\label{eq:selected-lower-bound}
   \iint_{B_{R_0}\times[-S_0,-\sigma_0]}|u_k|^3\,dx\,dt\ge c_0
\end{equation}
for all \(k\).
\end{lemma}

\begin{proof}
Since \(z_*\) is singular, \(u\) is not essentially bounded in
\(Q_\rho(z_*)\) for any \(\rho>0\).  Let \(\{\rho_j\}_{j=1}^\infty\) be a
dense subset of \((0,1)\).  We choose \(\lambda_k\downarrow0\) inductively so
that, for \(1\le j\le k\),
\[
   \lambda_k \|u\|_{L^\infty(Q_{\rho_j\lambda_k}(z_*))}\ge k.
\]
Indeed, at the \(k\)-th step we choose \(\lambda_k\) smaller than the previous
scale; every cylinder \(Q_{\rho_j\lambda_k}(z_*)\), \(1\le j\le k\), is still
centered at the singular terminal point and hence has infinite essential
supremum.  For the centered rescalings
\eqref{eq:rescaled-sequence}, the preceding condition gives
\[
   \|u_k\|_{L^\infty(Q_{\rho_j})}\ge k,
   \qquad 1\le j\le k.
\]
By density and monotonicity of cylinders,
\[
   \limsup_{k\to\infty}\|u_k\|_{L^\infty(Q_\rho)}=\infty
   \qquad\hbox{for every }0<\rho<1.
\]

By \cref{lem:endpoint-compactness}, after passing to a diagonal subsequence,
\((u_k,p_k-b_{k,R})\) converges on each \(Q_R\) to a suitable limit \((U,P)\).
Using the convergence on \(Q_1\) obtained from the endpoint compactness on
\(Q_2\), \cref{prop:persistence} shows that \(U\) has a terminal singularity
at \((0,0)\).  In particular \(U\not\equiv0\) on
\(\R^3\times(-\infty,0)\).  Therefore there is a compact cylinder
\[
   B_{R_0}\times[-S_0,-\sigma_0]\Subset\R^3\times(-\infty,0)
\]
such that
\[
   \iint_{B_{R_0}\times[-S_0,-\sigma_0]}|U|^3\,dx\,dt=2c_0>0.
\]
Strong \(L^3\)-convergence on this cylinder gives
\eqref{eq:selected-lower-bound} for all sufficiently large \(k\).  Discarding
finitely many indices and relabeling the sequence completes the selection.
\end{proof}
...
\cref{prop:ancient-limit} is nonzero.  More precisely,
\[
   \iint_{B_{R_0}\times[-S_0,-\sigma_0]}|U|^3\,dx\,dt\ge c_0.
\]
\end{proposition}

\begin{proof}
By \cref{prop:compactness}, \(u_k\to U\) strongly in \(L^3\) on
\(B_{R_0}\times[-S_0,-\sigma_0]\).  Passing to the limit in
\eqref{eq:selected-lower-bound} gives the result.
\end{proof}
...
\[
   \|V\|_{L^\infty(\R^3\times\R)}\le M.
\]
Moreover there are \(R_1,\eta_1>0\) such that
\[
   \sup_{\tau\in\R}\int_{B_{R_1}}|V(y,\tau)|^3\,dy\ge \eta_1.
\]
\end{lemma}
...
\[
   ...
   \le M.
\]
For non-triviality, \cref{prop:nonzero} gives positive \(L^3\) mass on a
compact spacetime cylinder in \((x,t)\).  Under the change of variables
\((x,t)\mapsto(y,\tau)\), this compact cylinder maps into a bounded
\(y\)-region and a compact \(\tau\)-interval, with a smooth Jacobian bounded
above and below.  Hence \(V\) has positive \(L^3\) mass on some
\(B_{R_1}\times[\tau_1,\tau_2]\).  Consequently at some time
\(\tau\in[\tau_1,\tau_2]\),
\[
   \int_{B_{R_1}}|V(y,\tau)|^3\,dy\ge \eta_1
\]
for a positive \(\eta_1\).
\end{proof}
