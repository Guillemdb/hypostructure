...
\((x,t)\mapsto (x_*+\lambda x,T+\lambda^2t)\): if
\(\phi\in C_c^\infty\) is a test function for the rescaled variables, the
corresponding original test function is
\[
   \Phi(X,S)=\phi\left(\frac{X-x_*}{\lambda},
   \frac{S-T}{\lambda^2}\right).
\]
The identities
\(\partial_S\Phi=\lambda^{-2}\partial_t\phi\),
\(\nabla_X\Phi=\lambda^{-1}\nabla_x\phi\), and
\(\Delta_X\Phi=\lambda^{-2}\Delta_x\phi\), together with
\(dX\,dS=\lambda^5\,dx\,dt\), give exactly the rescaled local energy
inequality after multiplying by the common factor \(\lambda^{-1}\).  No
boundary term appears because the test functions are compactly supported
inside the corresponding cylinders.
...
\(\lambda^{-2}|p|^{3/2}\,dX\,dS\).  The normalization by spatial means is
also preserved:
\[
   (p^\lambda)_{B_r}(t)
   =
   \lambda^2(p)_{B_{\lambda r}(x_*)}(T+\lambda^2t).
\]
...
\((u_k,p_k-a_k(t))\), where \(a_k\) is any gauge supplied on the larger
cylinder by \cref{lem:pressure}.  Subtracting a function of time from the
pressure does not change the local energy inequality by
\cref{lem:gauges-harmless}.  For a.e. admissible pair of times
\(-2S<t_1<t_2<-\sigma/2\), and then by the usual limiting argument at the
endpoints of the support of \(\phi\), the inequality gives
\[
   2\iint |\nabla u_k|^2\phi
   \le
...
measure of \(B_{2R}\times[-2S,-\sigma/2]\).  For the cubic velocity term we
use the same \(L^\infty\) bound.  For the pressure term, Hölder's inequality
...
consideration and therefore \(u_i(t)u_j(t)\in L^{3/2}(\R^3)\cap L^1(\R^3)\)
locally in time.  In distributional form this means
\[
   -\Delta p=\partial_i\partial_j(u_i u_j)
\]
on each time slice, and the Riesz-transform formula determines \(p\) up to a
function of time.  The rescaled pressure has the same representation with
\(u_k\), again modulo a time-dependent function.
...
\]
More precisely, after changing \(p_k\) by an irrelevant function of \(t\), the
difference \(h_k=p_k-q_k\) satisfies
\(-\Delta h_k=0\) in \(B_{4R}\) for a.e. \(t\), because the localized source
coincides with \(u_{k,i}u_{k,j}\) on \(B_{4R}\).  Calderon--Zygmund boundedness
on \(L^{3/2}\) gives
\[
   \|q_k(t)\|_{L^{3/2}(B_{2R})}
