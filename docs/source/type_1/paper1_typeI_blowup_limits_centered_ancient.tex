...
The pressure-normalized CKN quantities, their parabolic scaling, the
epsilon-regularity/no-concentration implication, and the local Type I/Type II
rate dichotomy were established in the first-node paper
\cite[Sections 2--5]{Paper0LocalConcentration}.  The specific internal labels
used below are
\begin{align*}
   \texttt{prop:critical-scaling},\quad
   \texttt{thm:ckn},\\
   \texttt{thm:singular-positive},\quad
   \texttt{thm:blowup-rate-dichotomy}.
\end{align*}
We recall the notation needed below, but we do not reprove those foundational
facts.  The additional work in the present paper is the Type I compactness up
to ancient time, the pressure gauges needed for the whole-space limit, the
...
\begin{proof}
Suitability of the rescaled pair, invariance of \(C\) and \(D\), and
preservation of pressure gauges are the parabolic rescaling results
proved in the first-node paper
\cite[Section 2 and Proposition \texttt{prop:critical-scaling}]
{Paper0LocalConcentration}.  The remaining two quantities used here have the
same critical dimensional balance.  With \(X=x_*+\lambda x\) and
\(S=T+\lambda^2t\),
\begin{align*}
...
\begin{proof}
The existence of a positive CKN concentration sequence at a singular point is
proved in the first-node paper
\cite[Theorem \texttt{thm:singular-positive}]
{Paper0LocalConcentration}.  The present stronger
all-sufficiently-small-radii form follows directly from the same
epsilon-regularity contrapositive, using
\cite[Theorem \texttt{thm:ckn}]{Paper0LocalConcentration}.  Take
\(\eta=\eps_*\).  If there were arbitrarily small radii \(r\) with
\[
...
\begin{proof}
By \cref{lem:singular-concentration}, and equivalently by the positive
local-concentration theorem in the first-node paper
\cite[Theorem \texttt{thm:singular-positive}]
{Paper0LocalConcentration}, every sufficiently small radius carries positive
pressure-normalized density.  Choose any sequence \(r_j\downarrow0\) inside
that range, so that
\[
...
\cref{lem:singular-concentration}, the local Type I alternative from
\cite[Theorem \texttt{thm:blowup-rate-dichotomy}]
{Paper0LocalConcentration}, and the endpoint compactness supplied by
...
branch of the local blow-up-rate dichotomy proved in the first-node paper
\cite[Theorem \texttt{thm:blowup-rate-dichotomy}]
{Paper0LocalConcentration} along the selected sequence, while
...
Navier--Stokes},
companion first-node manuscript in this series,
