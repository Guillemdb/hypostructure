...
normalization.  Indeed, subtracting a function of time has zero spatial
gradient, leaves the distributional equation and the local energy inequality
...
\begin{proof}
Let \(\varepsilon_{\rm CKN}>0\) be the absolute epsilon-regularity threshold
for the pressure-normalized CKN criterion \cite{CKN1982,Lin1998}.  If
\(C(z_*,r)+D(z_*,r)<\varepsilon_{\rm CKN}\) along a sequence
\(r\downarrow0\), then \(u\) is locally bounded in a smaller cylinder and
\(z_*\) is regular.  Since \(z_*\) is singular, there are
\(\eta>0\) and radii \(r_k\downarrow0\) for which
\eqref{eq:positive-ckn} holds.  Along these same radii, either the
scale-invariant quantity
\[
   \sup_{Q_{\theta r_k}(z_*)}(T-t)^{1/2}|u|
\]
is bounded at some fixed local scale \(\theta\in(0,1)\), which is the local
Type I alternative, or no such uniform bound is available along the shrinking
cylinders.  The latter case is precisely the local Type II alternative.  This
uses only the CKN criterion and the definitions of the two local rate
alternatives.
\end{proof}
...
\begin{proof}
The distributional equation and the local energy inequality are invariant
under the Navier--Stokes parabolic scaling after testing with pulled-back
nonnegative cutoffs.  The cylinder Jacobian is \(\lambda^5\), while
\(|u|^3\), \(|p-(p)_{B_{\lambda r}}|^{3/2}\), and the prefactor \(r^{-2}\)
scale as \(\lambda^3\), \(\lambda^3\), and \(\lambda^{-2}\), respectively.
Consequently \(C\) and \(D\) are unchanged by the transformation.  If
\(p^{(\lambda)}\) is replaced by \(p^{(\lambda)}-b(t)\), then the additional
term has zero spatial gradient and its contribution to the local energy
identity vanishes because \(u^{(\lambda)}\) is divergence free.  It also
cancels from the oscillation defining \(D\).
\end{proof}
...
The estimates used by that construction in the present normalization are the
following standard local compactness estimates.  The Type I bound gives
uniform compact-cylinder \(L^\infty\) control of the rescaled velocities.
The local energy inequality gives uniform local \(L^\infty_tL^2_x\) and
\(L^2_t\dot H^1_x\) bounds.  After subtracting spatial pressure averages on
fixed balls, the pressure is bounded in \(L^{3/2}_{\mathrm{loc}}\).  The
equation then bounds \(\partial_t u^{(k)}\) in a negative Sobolev space, so
the Aubin--Lions--Simon lemma gives strong local velocity convergence, while
the pressure converges weakly in \(L^{3/2}_{\mathrm{loc}}\).  These
convergences pass the equation, divergence-free constraint, and local energy
inequality to an ancient suitable weak limit; standard local regularity under
the inherited Type I bound gives smoothness away from the terminal time.

The point specific to the present formulation is the normalization used below.
The positive local CKN density is supplied by \Cref{thm:local-concentration}.
Its persistence follows from choosing the concentration sequence so that a
fixed subcylinder carries positive pressure-normalized density and then using
the strong local \(L^3\) convergence of velocities together with weak lower
semicontinuity for the gauged \(L^{3/2}\)-pressure norm.  Under the change of
variables \((x,t)=(e^{-\tau/2}y,-e^{-\tau})\), the compact negative-time
cylinder carrying positive mass maps to
...
\(L^{3/2}\)-pressure compactness after admissible gauges is part of the
extraction, and the centered change of variables has bounded Jacobian and
bounded pressure scaling factor on every compact \(\tau\)-interval.
\end{definition}
...
concentration normalization, namely a compact cylinder and a lower bound which
persist in the ancient limit by the scale selection and compactness argument
in \Cref{prop:seregin-extraction}.  Then let
...
and after centering they are composed with the smooth time map
\(t=-e^{-\tau}\) and multiplied by the smooth factor \(e^{-\tau}\).
\end{definition}
...
in \Cref{prop:seregin-extraction}.  The sole choice made in forming
...
\begin{proof}
Differentiate \(U(x,t)=e^{\tau/2}V(e^{\tau/2}x,\tau)\), where
\(\tau=-\log(-t)\).  The terms \(\partial_tU\), \((U\cdot\nabla)U\),
\(\nabla\Pi\), and \(\Delta U\) all carry the common factor
\(e^{3\tau/2}\) after expressing them in \((y,\tau)\).  The derivative of the
self-similar variables contributes
\(-\frac12(V+y\cdot\nabla V)\), and the pressure transformation gives
\(\nabla_yP\).  Since
\(\nabla_x\cdot U=e^\tau\nabla_y\cdot V\), the divergence-free condition is
also preserved.  Dividing the transformed equation by the common factor gives
\eqref{eq:centered}.
\end{proof}
...
\]
Smoothness follows from the inherited Type I bound and standard local
parabolic regularity for suitable weak solutions.  Applying the Leray
projection to \eqref{eq:centered} gives the whole-space projected Duhamel
formulation
\[
   V(\tau)=S(\tau-\sigma)V(\sigma)
   -\int_\sigma^\tau
      S(\tau-s)\mathbb P\nabla\cdot(V\otimes V)(s)\,ds,
\]
where \(S\) is the centered Stokes semigroup.  This formulation gives a
bounded mild representative.  Since the centered equation is autonomous,
\(V^s(y,\tau)=V(y,\tau+s)\) is again a bounded mild ancient solution with the
same \(L^\infty\) norm and introduces no additional normalization.
\end{proof}
...
local energy and pressure bounds on compact negative-time cylinders by the
extraction estimates in \Cref{prop:seregin-extraction}.  On every compact
...
axisymmetric and weighted-decay branches.  The stated properties are
preserved by the self-similar transformation and by time translation because
the axis of symmetry is unchanged, the swirl component scales by the same
homogeneous factors as the velocity, axial periods scale in the physical
variables and become stationary constraints in centered variables, and the
weighted \(L^2\) condition is imposed directly on \(V(\tau)\).  The no-swirl,
\(L_t^\infty L_x^p\)-swirl, periodic bounded-\(\Gamma\), controlled-swirl,
and weighted-decay exclusions are used below only through the single
structured Liouville input stated below.
\end{remark}
...
fields.  The Ornstein--Uhlenbeck formula for \(S(s)\) is
\[
   (S(s)f)(y)
   =
   e^{-s/2}
...
\]
The corresponding projected-divergence estimate is the centered form of
Kato's \(L^p\) estimate \cite{Kato1984}.  Thus there is an absolute constant
\(C_0\) such that, for
...
Fix \(\tau\in\mathbb R\) and \(T>0\).  The mild identity for the projected
centered equation on \([\tau-T,\tau]\) gives
\[
   V(\tau)
   =
   S(T)V(\tau-T)
...
\[
   ...
   \qquad \nabla\cdot W=0 .
\]
The preceding calculation has identified the stationary mild trajectory with
a stationary self-similar profile.  The theorem of
Ne\v{c}as--R\r{u}\v{z}i\v{c}ka--\v{S}verak \cite{NRS1996} then applies to
the profile equation with \(W\in L^3(\mathbb R^3)\), and gives
\(W\equiv0\).  Again
