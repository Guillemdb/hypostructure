...
Liouville theorem.  The positive concentration and the Type I/Type II local
dichotomy are used as proved local facts; the only additional inputs are the
stated nonclassical Liouville criteria.
\end{abstract}
...
The argument below is a finite exhaustion argument.  It uses the following
inputs and no others.  First, local CKN concentration at a singular point
supplies a positive pressure-normalized density.  Second, the Type I bound and
Seregin compactness turn a concentration sequence into a nonzero bounded
...
structured, and residual classes are eliminated only under the hypotheses
explicitly stated below.  Thus the final theorem is conditional exactly in
...
\]
\item \(U\) is axisymmetric and, for some \(1\le p<\infty\),
\[
...
solutions of the centered equation.  The small ancient Liouville theorem
therefore gives \(V\equiv0\).  In that case the Leray pressure is spatially
constant, modulo the allowed gauge \(a(\tau)\), and the integral in
\eqref{eq:nonvanishing} is zero.  This contradicts the inherited positive
normalization.

Assume next that \(V\in\mathcal X_{\mathrm{stat}\text{-}L^3}\).  Then
\(V(y,\tau)=W(y)\) for some \(W\in L^3(\mathbb R^3)\).  Substituting
\(\partial_\tau V=0\) into \eqref{eq:centered} gives the stationary
self-similar Navier--Stokes equation for \(W\):
\[
   (W\cdot\nabla)W+\nabla P
   =
   ...
   \qquad \nabla\cdot W=0 .
\]
The theorem of Necas--Ruzicka--Sverak applies exactly to this
\(L^3\)-stationary profile and gives \(W\equiv0\).  Again the nonvanishing
condition \eqref{eq:nonvanishing} is contradicted.
\end{proof}
...
\cite{KNSS2009}, the \(L_t^\infty L_x^p\)-swirl theorem of
Lei--Zhang--Zhao \cite{LZZ2017}, the periodic bounded-\(\Gamma\) theorem of
Lei--Ren--Zhang \cite{LRZ2022}, and any additional admissible
...
By \Cref{prop:exhaustion}, exactly the following exhaustive assertion holds:
either \(V\) belongs to at least one of the five non-residual classes, or
\[
   V\in\mathcal R(\mathcal S;I,J).
\]
If \(V\) belongs to a non-residual class, then
\Cref{prop:closed-branch-exclusion} rules it out.  This proposition uses the
...
\Cref{hyp:no-residual} says that no such element exists.  Both alternatives
