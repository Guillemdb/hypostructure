...
The compactness and invariant-measure construction for time translates was
proved in the preceding compact-dynamics paper
\cite{DuranCompactAncient}.  That construction yields a stationary
statistical identity, but it does not imply that the underlying trajectories
are stationary: the barycenter of an invariant measure carries a quadratic
covariance term.  The additional input considered here is an equilibrium
principle excluding recurrent nonstationary compact motion.  This paper
records a precise conditional form of that principle.
...
We use the same solution class, topology, and compact-hull conventions as in
the compact-dynamics paper \cite[Sections~2--5]{DuranCompactAncient}.  They
are recalled here only to fix notation for the present argument.  The locally
...
Navier--Stokes equation are those proved in the lemma ``Local smoothing and
pressure reconstruction'' of \cite{DuranCompactAncient}.
...
\(\mathbb P\operatorname{div}(V\otimes V)\) is interpreted as in
\cite[Definition ``Bounded mild ancient solution'"]{DuranCompactAncient}, by
duality against smooth test fields and as a tempered-distribution Fourier
multiplier on each time slice.  No boundedness of the Riesz transforms on
\(L^\infty\) is used in this paper.
...
\cite{DuranCompactAncient}.  The result is stated for the same locally smooth
topology and the same compact invariant sets used here.  It proves continuity
of each time-translation map, joint continuity of the action, and the identity
\(\Theta_t^{-1}=\Theta_{-t}\), hence the asserted homeomorphism property.
\end{proof}
...
\(\operatorname{div}W=0\) gives \(\operatorname{div}w=0\).  By the lemma
``Local smoothing and pressure reconstruction'' in
\cite{DuranCompactAncient}, every bounded mild ancient solution in the compact
...
\begin{proof}
This is the Krylov--Bogolyubov averages theorem proved in
\cite[Theorem ``Krylov--Bogolyubov averages'"]{DuranCompactAncient}.  The
hypotheses match because \cref{def:compact-hull} uses the same compact
locally smooth hull and time-translation flow.  That theorem gives
subsequential weak limits of the orbit averages, invariance under every
\(\Theta_t\), and the full-support conclusion in the minimal case.
\end{proof}
...
Necas--Ruzicka--Sverak \cite{NRS1996}, recalled in this series as the
stationary self-similar rigidity theorem in
\cite{DuranTightAncient}.  Their theorem is stated for weak
...
reduction developed in \cite{DuranMinimalAncient}.  The reduction itself is
not reproved here; it is isolated as the hypothesis below so that this paper
only supplies the compact-equilibrium implication.
...
the Type I blow-up procedure cited in \cite{DuranTypeIBlowup}, a nonzero
bounded mild ancient solution belonging to a collection \(A\) satisfying
\cref{hyp:minimal-compact}.  Then no Type I singularity occurs in that class.
\end{corollary}

\begin{proof}
The nonzero centered ancient limit is the Type I blow-up output used in
\cite{DuranTypeIBlowup}; the local concentration/nonvanishing input underlying
the surrounding singularity dichotomy is recorded in \cite{DuranPaper0TypeII}.
Thus the blow-up construction would give a nonzero element of \(A\), while
...
\bibitem{DuranPaper0TypeII}
G.~Cordero,
\emph{Local CKN Concentration and the Type I/Type II Dichotomy for
Navier--Stokes},
manuscript in this series,
\path{docs/source/type_1/unconditional_typeI_chain/paper0_local_concentration_entry_typeII.tex}.

\bibitem{DuranTypeIBlowup}
G.~Duran Ballester,
\emph{Type I Blow-up Limits for the Three-dimensional Navier--Stokes
Equations and the Centered Ancient Equation},
manuscript in this series,
\path{docs/source/type_1/paper1_typeI_blowup_limits_centered_ancient.tex}.

\bibitem{DuranTightAncient}
G.~Duran Ballester,
\emph{Uniformly Tight Ancient Solutions of the Navier--Stokes Equations in
Self-similar Variables},
manuscript in this series,
\path{docs/source/type_1/paper2_tight_ancient_stationary_rigidity.tex}.

\bibitem{DuranMinimalAncient}
G.~Duran Ballester,
\emph{Minimal Ancient Solutions for the Navier--Stokes Equations in
Self-similar Variables},
manuscript in this series,
\path{docs/source/type_1/paper3_minimal_ancient_solutions_reduction.tex}.

\bibitem{DuranCompactAncient}
G.~Duran Ballester,
\emph{Compact Ancient Navier--Stokes Dynamics and Invariant Measures in
Self-similar Variables},
manuscript in this series,
\path{docs/source/type_1/paper4_compact_ancient_dynamics_invariant_measures.tex}.
