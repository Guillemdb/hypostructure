...
incompressible Navier--Stokes equations in standard PDE language.  The argument
starts from a suitable weak solution near a putative singular point and uses the
local Caffarelli--Kohn--Nirenberg concentration theorem, a local Type I tangent
criterion, the local single-core Type II criterion, the localized
scale-translation representation,
the local multibubble and cascade exclusion, the compact-cylinder Caccioppoli
regularity theorem, and the scale-collapse dichotomy.  Every positive local Type
II concentration is exhausted by the single-core, multibubble/cascade,
rough-core, or scale-collapse
alternatives, and each alternative is excluded by the corresponding theorem.
Consequently no Type II singularity occurs at the point.
...
\[
  ...
  \qquad \nabla\cdot u=0,
\]
in a neighborhood of a possible singular point \(z_0=(x_0,T)\).  This paper assembles the local Type II part of the analysis from the local concentration reduction together with the local Type II components stated below.

The local formulation starts from parabolic rescalings.  For \(r>0\), set
\[
...
Let \((u,p)\) be a suitable weak solution near \((x_0,T)\).  If \((x_0,T)\)
is in the local Type II regime of \cref{def:typeII}, then \((x_0,T)\) is not
singular.
\end{theorem}

Here the Type II regime means that the Type I tangent alternative is absent.
The remaining local alternatives are exhausted and excluded by the five local
Type II components.

\section{Local concentration and Type I--Type II split}
...
A positive local concentration sequence is called a local Type II sequence if no
selected-window rescaling has a nonzero bounded parabolic tangent on a compact
cylinder.
\end{definition}
...
\section{Local PDE components}

This section states the local PDE components used in the assembly theorem.

\begin{theorem}[Local single-core criterion]\label{thm:single-core}
A local Type II sequence with one
...
parabolic tangent, contradicting the definition of a local Type II sequence.
\end{proof}
...
No local multibubble, cascade, or gauge-degenerate Type II configuration occurs.
\end{theorem}
...
reduce every such configuration either to the single-core criterion or to a
nonzero bounded parabolic tangent, both of which are impossible in the local Type
II regime.
\end{proof}

\begin{theorem}[Compact-cylinder Caccioppoli regularity]\label{thm:rough-core}
For the renormalized Type II sequences arising from the local construction, the
local pressure reconstruction modulo functions of time, the bounded critical
local norm on the cylinders under consideration, and the bounded or
window-integrable modulation coefficients give uniform local windowed \(H^1\)
control by the local Caccioppoli estimate.  Consequently the rough-core
alternative, meaning failure of this local windowed \(H^1\) bound in a bounded
renormalized core, cannot occur.
\end{theorem}
...
the modulation terms are controlled by the established coefficient bounds.  This
...
\]
The finite-cost scale-collapse rescaling is excluded by the scale-drift cost
comparison.  Alternatively, on thick scale-collapse windows with compactness,
modulation convergence, and a stationary omega-limit, the rescaling produces a
nonzero stationary or generalized self-similar profile, which is excluded by the
corresponding Liouville theorem.
\end{theorem}
...
theorem.  The structural alternative follows by translating thick windows, using
compactness and modulation convergence to pass to an autonomous reduced limit,
and then applying the stationary omega-limit and Liouville theorem.
\end{proof}

\begin{theorem}[Local Type II alternative decomposition]\label{thm:alternatives}
Every positive local Type II concentration sequence belongs to one of the
following alternatives: compact single-core finite cost, multibubble/cascade or
gauge degeneracy, rough-core loss of local windowed \(H^1\), or scale collapse.
\end{theorem}

\begin{proof}
The compact-cylinder dichotomy gives either compact-cylinder suitable bounds or
additional bounded-cylinder concentration.  The latter is the
multibubble/cascade alternative.  Under compact-cylinder suitable bounds,
localized scale-translation degeneracy is the gauge-degenerate alternative; the
nondegenerate case gives the single-core renormalized variables.  In this
single-core case the selected-window compact argument either has finite
localized Type II cost, loses local windowed \(H^1\) control in a bounded
renormalized core, or enters the scale-collapse alternative.  These cases are
...
If \((x_0,T)\) is in the local Type II regime of \cref{def:typeII}, then
\((x_0,T)\) is not singular.
\end{theorem}
...
with positive local CKN concentration.  By the definition of the local Type II regime, no selected-window rescaling has a nonzero bounded parabolic tangent.
...
\cref{thm:scale-collapse} excludes the scale-collapse alternative.

By the local Type II alternative decomposition, \cref{thm:alternatives}, these
are all positive local Type II concentration cases.  Each case has been
excluded, contradicting the assumed Type II singularity.
\end{proof}

\begin{corollary}[Exclusion after Type I separation]
No singular point of a suitable weak solution belongs to the local Type II
regime.
\end{corollary}
...
The local concentration theorem reduces singularity formation to positive CKN
concentration.  The Type I tangent alternative is separated off by definition of
the local Type II regime.  The five local Type II theorems then exhaust the
remaining possibilities: compact single-core finite cost, multibubble/cascade
or gauge degeneracy, rough-core loss of local windowed \(H^1\), and scale
collapse.  Each possibility is excluded.  Hence no local Type II singularity
remains.
