...
\begin{abstract}
This document consolidates the Type II regularity argument into one working draft. The source material has been placed in a single LaTeX file, with duplicated scaffolding removed and shared notation, references, and bibliography handled by one preamble.
\end{abstract}
...
This section isolates a local compactness mechanism for Type II concentration in
...
Then, using the pressure-normalized quantities \(C,D\) from the local concentration dichotomy, there exist
...
\begin{proof}
This is the local blowup-rate dichotomy used as the entry point for the Type II analysis.
\end{proof}
...
\begin{proof}
The local concentration dichotomy supplies the positive CKN concentration sequence and the exclusion of the local Type I alternative at the chosen concentration point.  The
...
formulation of the Type II branch used in this section: any branch with a nonzero
bounded selected-window limit is assigned to the bounded-window alternative,
not to \cref{paper1:def:typeII}.
\end{proof}

\begin{proposition}[Imported local representation input]\label{paper1:prop:representation-input}
...
\begin{quote}
\textbf{Abstract.}
This section gives the local representation theorem for the retained
...
\begin{definition}[Single-core assumption package]\label{paper2:def:SC}
...
\subsection{Domain bookkeeping and support}\label{paper2:sec:domains}
...
\begin{definition}[Retained nondegenerate repaired-gauge stratum]\label{paper2:def:admissible-gauge-path}
...
\begin{proposition}[Gauge regularity on the retained stratum]\label{paper2:prop:preliminary-correction}
...
of Definition~\ref{paper2:def:admissible-gauge-path}. The final assertion is exactly
...
\subsection{Final representation package}\label{paper2:sec:final}
...
The modulation identity is exactly Theorem~\ref{paper2:thm:mod-sys}; uniqueness follows from invertibility of
...
\begin{theorem}[Theorem C: compact-window representation output]\label{paper2:thm:C}
Under the assumptions of Theorems~\ref{paper2:thm:A} and~\ref{paper2:thm:B}, for every compact
window \(J\subset I_*\) and each fixed \(R>0\), setting
\(Q_R^J:=B_R\times J\), the
following are available for later sections:
\begin{enumerate}
...
\end{enumerate}
No step beyond these items is used in later sections.
\begin{proof}
...
Proposition~\ref{paper2:prop:stability}. This is the package exported by the previous theorems.
\end{proof}
\end{theorem}
...
This section uses the classical theory of suitable weak solutions of the
...
This section is a local reduction theorem.  It uses the analytic inputs already established in \cref{sec:single-core-criterion,sec:repaired-gauge-construction,sec:rough-core-exclusion,sec:scale-collapse-cost} and proves only the multibubble, cascade, and terminal-frame decoupling steps.
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item From \cref{sec:scale-collapse-cost}, the collapse cost exclusion \cref{paper5:thm:collapse-core-floor,paper5:thm:cost-exclusion} and autonomous reduced-limit theorem \cref{paper5:thm:autonomous-limit} wherever a scale-rigid or scale-collapsing branch is produced.
\end{enumerate}
...
\begin{definition}[Active profile package]\label{paper3:def:active-package}
...
\cref{paper3:def:active-package}.  If the branch is not already a single active
...
\begin{definition}[Admissible terminal active package]\label{paper3:def:admissible-terminal}
...
\cref{paper3:def:active-package}.
...
\cref{paper3:thm:terminal-decoupling}, provided the terminal package generated in the
...
\cref{paper3:thm:terminal-decoupling}.  Therefore one of three outcomes holds:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textbf{Single-core branch:}
  after discarding \(S_n\), the branch is governed by one active component, so it
  lands in the compact single-core branch of \cref{paper3:ass:single-core};
...
non-uniqueness alternative in \cref{paper3:def:local-multibubble} rather than a new
...
\cref{paper3:def:local-multibubble}.  By the repaired-gauge representation in \cref{sec:repaired-gauge-construction}, nondegeneracy is exactly the hypothesis needed to enter the
...
scale-rigid alternative is excluded by \cref{paper3:ass:scale-rigidity}, which packages
...
\cref{paper3:def:local-multibubble} is impossible in the local Type II regime.
\end{proof}
...
core alternative is excluded by \cref{paper3:thm:multi} under the hypotheses stated
there.
\end{corollary}
...
The final exclusion is \cref{paper3:thm:multi}.  Hence no failure of the single
...
This section is written in the classical setting of suitable weak solutions (Leray \cite{Leray1934}, Caffarelli--Kohn--Nirenberg \cite{CaffarelliKohnNirenberg1982}, Scheffer \cite{Scheffer1976}, Lin \cite{Lin1998}).
\end{quote}

\subsection{Imported inputs and notation}

This section uses the renormalized local energy inequality obtained in \cref{sec:repaired-gauge-construction}. It does not rederive the renormalized suitable structure or the pressure reconstruction. All estimates
...
\]
More quantitatively, the bound is the one in \cref{paper4:thm:caccioppoli}. If a
later argument uses pressure means on a smaller ball, \cref{paper4:lem:pressure-means}
converts those normalizations to the outer-ball normalization used here.
\end{corollary}

\begin{proof}
This is \cref{paper4:thm:caccioppoli}. The final sentence follows from
...
\subsection{Inputs exported to later sections}

The following consequences are used later in the document.
...
\(\int a_-(\tau)M_R(\tau)\,d\tau=\infty\); thus finite scale-collapsing cost or finite absolute cost is incompatible with collapse.  In addition, we record the autonomous-window limit passage under an explicitly stated compactness input; that input is not derived from the cost hypotheses in this section.
\end{quote}
...
We split the scale-collapse analysis into two separate levels:
\begin{enumerate}[label=(\arabic*)]
  \item Part A (Sections~\ref{paper5:sec:cost}--\ref{paper5:sec:finite-cost}): unconditional cost
    exclusion arguments that do not use compactness.
  \item Part B (Section~\ref{paper5:sec:autonomous}): a conditional compactness statement under explicitly
    stated compactness hypotheses.
\end{enumerate}

Throughout this section, we assume the renormalized representation established earlier: on the
...
All integrals in this section are interpreted in \([0,\infty]\). When we write
...
not the cost bound alone; it is the selected-window suitable package supplied by the renormalized representation, pressure reconstruction, and local Caccioppoli estimates in \cref{sec:repaired-gauge-construction,sec:rough-core-exclusion}.

\paragraph{Dependencies used in this section.}
In standard PDE terms, \cref{sec:repaired-gauge-construction,sec:rough-core-exclusion} supply the following ingredients on selected compact renormalized windows:
\begin{enumerate}[label=(\arabic*)]
...
\begin{proposition}[Derivation of the selected-window estimates]
...
of the repaired-gauge representation theorem \cref{paper2:thm:C} on the windows \([\tau_n,\tau_n+T]\).  Assume that on
...
Assumption~\ref{paper5:ass:selected-window-estimates} follow from the repaired-gauge representation,
...
\subsection{State-space stratification of the scale-collapse branch}
...
estimate and autonomous-reduction route established earlier in this document.  The purpose of this section is
...
Let a represented Type II branch be obtained from the selected-core construction developed above.  Then there exist \(R>0\), a cutoff \(\chi\in C_c^\infty(B_R)\), \(\chi\ge0\), a
...
\begin{theorem}[Scale-collapse state-space stratification]\label{paper5:thm:scale-collapse-stratification}
...
Assumption~\ref{paper5:ass:selected-window-estimates}, or the branch is in the compactness defect.  If
...
\begin{theorem}[Stationary-rigidity blocker]\label{paper5:thm:stationary-rigidity-blocker}
...
\begin{proof}
Apply Theorem~\ref{paper5:thm:scale-collapse-stratification}.  The finite-cost alternatives are exactly
...
Theorem~\ref{paper5:thm:stationary-rigidity-blocker} blocks the branch.  If not, the unresolved
...
assemble the selected-window estimates from \cref{sec:repaired-gauge-construction,sec:rough-core-exclusion} into a nontrivial autonomous
finite-window limit \((U,\Pi)\) satisfying the full distributional autonomous equation.

Section~\ref{paper5:sec:branch-closure} records the same state-space stratification as the markdown
...
those regimes this document records the precise thin-drift, modulation, compactness, or
...
Navier--Stokes equations from the local results proved in \cref{sec:single-core-criterion,sec:repaired-gauge-construction,sec:multibubble-cascade-exclusion,sec:rough-core-exclusion,sec:scale-collapse-cost}.
...
state-space alternatives to which they apply.  The proof of the assembly below uses only those
...
\begin{proof}
This is the local concentration and blow-up dichotomy used as the entry point for \cref{sec:typeII-assembly}.
\end{proof}

\subsection{Local Theorems Used in the Assembly}

We record the local results established above in the form used below.  They are quoted here as PDE statements to keep the final assembly self-contained.
...
\begin{proof}
This is the compact-window representation package of \cref{paper2:thm:C}.
\end{proof}
...
\begin{proof}
This is exactly \cref{paper1:thm:single-core-criterion}.
\end{proof}
...
\begin{proof}
This is the local multibubble and cascade exclusion theorem \cref{paper3:thm:multi}, together with its removal corollary \cref{paper3:cor:multi-removal}.
\end{proof}
...
\begin{proof}
This is the rough-core exclusion on compact windows, \cref{paper4:cor:rough-core}.
\end{proof}
...
\begin{proof}
This is the scale-collapse cost exclusion \cref{paper5:thm:cost-exclusion} and absolute scale-cost exclusion \cref{paper5:cor:absolute-cost-exclusion}.
\end{proof}
...
state-space alternatives isolated in \cref{sec:scale-collapse-cost} occurs:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\subsection{State-Space Assembly}
...
\cref{paper6:def:local-typeII-sequence}, after passing to subsequences and selected
...
alternative.  By the local concentration dichotomy, \cref{paper6:thm:paper0}, there is a positive local Type II
concentration sequence \(r_n\downarrow0\) at \((x_0,T)\).  The local
state-space decomposition, \cref{paper6:thm:state-decomposition}, places this sequence,
...
\begin{proof}
Let \(x_0\in\Sigma(T)\).  By the local concentration dichotomy, \cref{paper6:thm:paper0}, a concentration point is
