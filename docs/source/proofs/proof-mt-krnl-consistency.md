...
Let $\mathcal{S} = (\mathcal{X}, \nabla, \Phi_\bullet, \tau, \partial_\bullet)$ be a structural flow datum (in the sense of {prf:ref}`def-structural-flow-datum`) satisfying the following:

**Assumption (Strict Dissipation):** For all $x \in \mathcal{X}$ and $t > 0$,
$$\Phi(S_t x) < \Phi(x) \quad \text{unless } S_t x = x.$$
...
- Otherwise, certify dispersion via the Scattering-Compactness dichotomy (Theorem {prf:ref}`mt-scattering-compactness`)
...
$$\|S_t x - x^*\|_{\mathcal{X}} \leq C \cdot t^{-\frac{1}{2\theta - 1}} \quad \text{for } t \gg 1.$$
