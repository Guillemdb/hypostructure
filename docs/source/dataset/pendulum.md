...
The Hamiltonian vector field $X_H = (\frac{p}{ml^2}, -\frac{g}{l}\sin\theta)$ is $C^\infty$ on the phase space. By Picard-Lindelöf, solutions exist locally. Since trajectories remain on compact level sets (energy conservation), they cannot escape to infinity in finite time. By standard ODE extension theorems, solutions exist globally for all $t \in \mathbb{R}$.
